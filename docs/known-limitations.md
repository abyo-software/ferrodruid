...
> Applies to: FerroDruid v1.2.0 · Last reconciled: 2026-07-12
>
...
- `[v1.0 plan]` — item from the original v1.0 roadmap; where still open
  at v1.2.0 it remains a committed roadmap item.
...
- **Open residuals:**
  - **CL-1-R2 — read this one.** The exactly-once segment writer
    (atomic segment + offset-sidecar publish, crash-safe resume) is
    proven by the integration suite, but the in-tree
    `KafkaConsumerTask::run` supervisor path **still runs
    at-least-once** until the writer contract is wired into the
    supervisor runtime. Duplicates are possible on crash-restart today.
...
| ID | Limitation | Severity | Status (v1.2.0) |
|---|---|---|---|
| CL-1 | Kafka ingestion production validation | `[v1.0 plan]` | Single-broker EOS / rebalance / broker-kill / schema-drift closed (2026-06-30). Default runtime path still at-least-once (CL-1-R2); multi-broker + soak + managed cloud open (CL-1-R1). |
...
| FG-1..FG-5 | Feature gaps | mixed | See §15 — notably **no automatic compaction** (FG-2) and no nested columns (FG-3). |
