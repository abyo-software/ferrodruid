...
- Metadata: SQLite, PostgreSQL, MySQL (via `sqlx`)
- mmap-based zero-copy segment serving
- Tiered storage (Hot / Warm / Cold / Frozen) with 9 load-rule types

### Ingestion

- Native batch (JSON, CSV) via `index_parallel`-style spec
- Kafka supervisor + real consumer (offset tracking, buffer/flush)
