...
#
# Wave 47-C — Real Druid 33.0.0 ⇄ FerroDruid wire/JSON diff harness.
#
# Mirrors `run_compat_v35.sh` but uses the
# `docker-compose.druid33.yml` stack on host port 33888.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORKSPACE_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
COMPOSE_FILE="$SCRIPT_DIR/docker-compose.druid33.yml"
DRUID_PORT=33888
DRUID_BASE="http://localhost:${DRUID_PORT}"
TEST_NAME="druid_33_vs_ferrodruid_diff"

cd "$SCRIPT_DIR"

cleanup() {
    echo "Shutting down Druid 33 stack..."
    docker compose -f "$COMPOSE_FILE" down -v 2>/dev/null || true
}
trap cleanup EXIT

echo "=== Starting Apache Druid 33.0.0 (micro-quickstart) ==="
docker compose -f "$COMPOSE_FILE" up -d

echo "=== Waiting for Druid 33 to be healthy (up to 10 min on first pull / amd64-on-arm64 emulation) ==="
attempts=0
max_attempts=120
until curl -sf "${DRUID_BASE}/status/health" > /dev/null 2>&1; do
    attempts=$((attempts + 1))
    if [ "$attempts" -ge "$max_attempts" ]; then
        echo "ERROR: Druid 33 did not become healthy after $max_attempts attempts"
        docker compose -f "$COMPOSE_FILE" logs druid | tail -80
        exit 1
    fi
    sleep 5
done
echo "Druid 33 is ready."

echo "=== Submitting batch ingestion task to Druid 33 ==="
...
echo "=== Waiting for Druid 33 ingestion to complete (up to 5 min) ==="
attempts=0
max_attempts=60
while true; do
    attempts=$((attempts + 1))
...
echo "=== Verifying data is queryable in Druid 33 ==="
...
echo "=== Running FerroDruid <-> Druid 33 diff test ==="
cd "$WORKSPACE_ROOT"
cargo test -p ferrodruid-rest --test druid_diff_test "$TEST_NAME" \
    -- --ignored --nocapture 2>&1 \
    | tee "$SCRIPT_DIR/RESULTS_wave47c_v33_run_stdout.txt"
