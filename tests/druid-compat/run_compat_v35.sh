...
#
# Wave 47-B — Real Druid 35.0.1 ⇄ FerroDruid wire/JSON diff harness.
#
# Mirrors `run_compat.sh` (which targets Druid 30.0.1) but uses the
# `docker-compose.druid35.yml` stack on host port 28888.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORKSPACE_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
COMPOSE_FILE="$SCRIPT_DIR/docker-compose.druid35.yml"
DRUID_PORT=28888
DRUID_BASE="http://localhost:${DRUID_PORT}"
TEST_NAME="druid_35_vs_ferrodruid_diff"

cd "$SCRIPT_DIR"

cleanup() {
    echo "Shutting down Druid 35 stack..."
    docker compose -f "$COMPOSE_FILE" down -v 2>/dev/null || true
}
trap cleanup EXIT

echo "=== Starting Apache Druid 35.0.1 (micro-quickstart) ==="
docker compose -f "$COMPOSE_FILE" up -d

echo "=== Waiting for Druid 35 to be healthy (up to 10 min on first pull / amd64-on-arm64 emulation) ==="
attempts=0
max_attempts=120
until curl -sf "${DRUID_BASE}/status/health" > /dev/null 2>&1; do
    attempts=$((attempts + 1))
    if [ "$attempts" -ge "$max_attempts" ]; then
        echo "ERROR: Druid 35 did not become healthy after $max_attempts attempts"
        docker compose -f "$COMPOSE_FILE" logs druid | tail -80
        exit 1
    fi
    sleep 5
done
echo "Druid 35 is ready."

echo "=== Submitting batch ingestion task to Druid 35 ==="
...
echo "=== Waiting for Druid 35 ingestion to complete (up to 5 min) ==="
attempts=0
max_attempts=60
while true; do
    attempts=$((attempts + 1))
...
echo "=== Verifying data is queryable in Druid 35 ==="
...
echo "=== Running FerroDruid <-> Druid 35 diff test ==="
cd "$WORKSPACE_ROOT"
cargo test -p ferrodruid-rest --test druid_diff_test "$TEST_NAME" \
    -- --ignored --nocapture 2>&1 \
    | tee "$SCRIPT_DIR/RESULTS_wave47b_v35_run_stdout.txt"
