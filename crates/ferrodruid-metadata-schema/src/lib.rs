...
/// Byte cap on each IDENTITY column (`id` / `dataSource` / `start` /
/// `end` / `version`) of a `druid_segments` row transferred from an
/// untrusted source DB — the same unbounded-alloc species
/// [`MAX_SEGMENT_PAYLOAD_BYTES`] closes for the payload.  A legitimate
/// identity column is a datasource name, an ISO-8601 instant, or a
/// version string (all well under a kilobyte; Druid's own schema is
/// `VARCHAR(255)`), so a multi-MB value is hostile or corrupt.  Rows
/// over the cap are length-gated AT THE SQL LEVEL like the payload (the
/// oversized bytes are NEVER transferred — the SELECT NULLs the
/// offending column and returns the max byte length in `identity_len`)
/// and become a loud per-row `Err`, never an unbounded allocation or a
/// run abort.  64 KiB is a generous ceiling far above any legitimate
/// segment identity.
const MAX_IDENTITY_COLUMN_BYTES: i64 = 64 * 1024;
...
/// The length-gated IDENTITY-column SELECT-list expressions for a
/// backend (the identity mirror of [`payload_columns`]): each of the
/// five identity columns is returned only when its byte length is
/// within [`MAX_IDENTITY_COLUMN_BYTES`] (else `NULL` — the oversized
/// bytes never leave the DB), and the max byte length across all five
/// comes back as `identity_len` so the reader can refuse the row
/// loudly.  Every length expression `COALESCE`s a NULL column to `''`
/// (byte length 0), so a genuinely-NULL identity column still flows to
/// the existing NULL-column decode error and can never NULL-poison the
/// max.
fn identity_columns(backend: &SourceBackend) -> String {
    // `start` / `end` are reserved words, quoted per backend exactly as
    // in [`build_segments_sql`].
    let (q_open, q_close) = match backend {
        SourceBackend::MySql => ("`", "`"),
        SourceBackend::Postgres | SourceBackend::SqlitePath(_) => ("\"", "\""),
    };
    // Byte-length expression per backend (`CAST(... AS BLOB)` on SQLite
    // so a TEXT column is measured in bytes, matching the payload gate).
    let len_expr = |col: &str| match backend {
        SourceBackend::Postgres => format!("octet_length(COALESCE({col}, ''))"),
        SourceBackend::MySql => format!("OCTET_LENGTH(COALESCE({col}, ''))"),
        SourceBackend::SqlitePath(_) => format!("length(CAST(COALESCE({col}, '') AS BLOB))"),
    };
    let start_col = format!("{q_open}start{q_close}");
    let end_col = format!("{q_open}end{q_close}");
    let cols: [(&str, &str); 5] = [
        ("id", "id"),
        ("dataSource", "ds"),
        (start_col.as_str(), "seg_start"),
        (end_col.as_str(), "seg_end"),
        ("version", "version"),
    ];
    let mut projections: Vec<String> = cols
        .iter()
        .map(|(col, alias)| {
            format!(
                "CASE WHEN {len} <= {MAX_IDENTITY_COLUMN_BYTES} THEN {col} ELSE NULL END \
                 AS {alias}",
                len = len_expr(col)
            )
        })
    ...
    let lens = cols
        .iter()
        .map(|(col, _)| len_expr(col))
        .collect::<Vec<_>>()
        .join(", ");
    // The projected max MUST decode as `i64`: PG `GREATEST` of
    // `octet_length` int4s is int4 (the same landmine as `payload_len`,
    // hence the `::bigint` cast); MySQL `CAST(... AS SIGNED)` is a
    // signed 64-bit integer; SQLite's scalar `MAX` is already i64.
    let max_expr = match backend {
        SourceBackend::Postgres => format!("GREATEST({lens})::bigint"),
        SourceBackend::MySql => format!("CAST(GREATEST({lens}) AS SIGNED)"),
        SourceBackend::SqlitePath(_) => format!("MAX({lens})"),
    };
    projections.push(format!("{max_expr} AS identity_len"));
    projections.join(", ")
}
...
/// and the identity columns likewise (see [`identity_columns`]).
...
fn build_segments_sql(
    ...
    let used_literal = match backend {
        SourceBackend::Postgres | SourceBackend::MySql => "TRUE",
        SourceBackend::SqlitePath(_) => "1",
    };
    let identity_cols = identity_columns(backend);
    let payload_cols = payload_columns(backend);
    let mut sql = format!(
        "SELECT {identity_cols}, {payload_cols} \
...
{
    // IDENTITY GATE (re-audit Low) — checked before anything else is
    // read.  The identity columns are length-gated AT THE SQL LEVEL like
    // the payload (each is NULLed in the SELECT when it exceeds
    // [`MAX_IDENTITY_COLUMN_BYTES`]; the max byte length always comes
    // back in `identity_len`), so the oversized bytes never leave the
    // DB.  An over-cap row — or one whose `identity_len` cannot be read
    // as `i64` (the same fail-closed rule as `payload_len`) — becomes a
    // loud per-row `Err` carrying whatever identity fields survived
    // their own gate, never an unbounded allocation or a run abort.
    let identity_violation: Option<String> = match row.try_get::<Option<i64>, _>("identity_len") {
        Ok(Some(n)) if n > MAX_IDENTITY_COLUMN_BYTES => Some(format!(
            "an identity column (id/dataSource/start/end/version) is {n} bytes, exceeding the \
             {MAX_IDENTITY_COLUMN_BYTES} byte identity-column cap — skipped (an implausibly \
             large segment identity; the oversized bytes were NOT transferred)"
        )),
        Ok(Some(_)) => None,
        Ok(None) => Some(
            "identity_len is NULL — refusing to read possibly-oversized identity columns"
                .to_string(),
        ),
        Err(e) => Some(format!(
            "could not read identity_len as i64 ({e}) — refusing to read possibly-oversized \
             identity columns"
        )),
    };
    if let Some(reason) = identity_violation {
        // Best-effort identity for reporting: each column was
        // individually gated in the SELECT, so what survives is safe to
        // read; the offending column comes back NULL (empty here).
        let gated = |name: &str| {
        ...
        return DruidSourceSegmentRow {
            id,
            data_source: gated("ds"),
            start: gated("seg_start"),
            end: gated("seg_end"),
...
mod tests {
    ...
    #[tokio::test]
    async fn read_druid_source_rejects_oversized_identity_columns() {
        // Re-audit Low: the payload is length-gated at the SQL level, but
        // the identity TEXT columns were NOT — a hostile/corrupt row with
        // a multi-MB identity column was read unbounded into memory (the
        // same unbounded-alloc species the payload cap closed).
        let dir = tempfile::tempdir().unwrap();
        let (pool, path) = make_source_sqlite(dir.path()).await;
        // A normal row that must still decode alongside the bad ones.
        insert_source_row(
            &pool,
            "ok_row",
            "wiki",
        ...
        .await;
        let over_cap = usize::try_from(MAX_IDENTITY_COLUMN_BYTES).unwrap() + 1;
        // A row whose `version` identity column is one byte over the cap.
        sqlx::query(
            "INSERT INTO druid_segments \
             (id, dataSource, created_date, \"start\", \"end\", partitioned, version, used, payload) \
             VALUES ('big_version', 'wiki', 'c', '2015-09-12T00:00:00.000Z', \
             '2015-09-13T00:00:00.000Z', 1, ?, 1, ?)",
        )
        .bind("v".repeat(over_cap))
        .bind(local_payload_json("wiki", "/x"))
        .execute(&pool)
        .await
        .unwrap();
        // A row whose `id` ITSELF is oversized: the error cannot name the
        // id, but the row must still surface as a loud per-row Err.
        sqlx::query(
            "INSERT INTO druid_segments \
             (id, dataSource, created_date, \"start\", \"end\", partitioned, version, used, payload) \
             VALUES (?, 'wiki', 'c', '2015-09-12T00:00:00.000Z', \
             '2015-09-13T00:00:00.000Z', 1, 'v1', 1, ?)",
        )
        .bind("z".repeat(over_cap))
        .bind(local_payload_json("wiki", "/x"))
        .execute(&pool)
        .await
        .unwrap();
        ...
        let report = read_druid_source(&format!("sqlite://{path}"), None, None)
            .await
            .expect("oversized identity rows must not abort the whole read");
        assert_eq!(
            report.segments.len(),
            3,
            "all rows are returned (oversized-identity ones as per-row Errs, not dropped)"
        );
        let by_id = |id: &str| {
            report
                .segments
        ...
        // The big-version row: refused loudly, its oversized bytes never
        // materialized in the returned row.
        let big_version = by_id("big_version");
        let err = big_version
            .payload
            .as_ref()
            .expect_err("an oversized identity column must be a per-row Err, never read unbounded");
        assert!(
            err.contains("identity") && err.contains("exceed") && err.contains("cap"),
            "the reason names the identity byte cap: {err}"
        );
        assert!(
            big_version.version.len() <= usize::try_from(MAX_IDENTITY_COLUMN_BYTES).unwrap(),
            "the oversized identity bytes must NOT be transferred \
             (got {} bytes)",
            big_version.version.len()
        );
        // The oversized-id row: the id column itself was gated to NULL,
        // so it surfaces as the empty-id row with a loud per-row Err.
        let big_id = report
            .segments
            .iter()
            .find(|s| s.id.is_empty())
            .expect("the oversized-id row is returned with its id gated out");
        let err = big_id
            .payload
            .as_ref()
            .expect_err("an oversized id must be a per-row Err");
        assert!(
            err.contains("identity") && err.contains("cap"),
            "the reason names the identity byte cap: {err}"
        );
    }
    ...
    fn build_segments_sql_quotes_reserved_words_per_backend() {
        ...
        for sql in [&pg, &my, &sq] {
            ...
            // The identity columns are length-gated in the SELECT list
            // too (re-audit Low): the max identity byte length is
            // projected, and each identity column is CASE-gated at the
            // 64 KiB identity cap.
            assert!(sql.contains("AS identity_len"), "{sql}");
            assert!(sql.contains("<= 65536"), "{sql}");
        }
        // The projected `identity_len` decodes as i64 on every backend
        // (the payload_len int4 landmine, applied to the identity gate).
        assert!(pg.contains("::bigint AS identity_len"), "{pg}");
        assert!(
            my.contains("CAST(GREATEST(") && my.contains("AS SIGNED) AS identity_len"),
            "{my}"
        );
    }
