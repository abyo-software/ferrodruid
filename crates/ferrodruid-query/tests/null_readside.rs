...
/// is stored as `LongNullable` (exact i64 values + null-row bitmap), which
/// the vectorized fast paths do not match, so the query falls back to the
/// general row path — the aggregator itself must yield null for the
/// all-null group.
#[test]
...
// ===========================================================================
// compat-10 — nullable LONG columns are null-faithful AND i64-exact through
// the generic slow path (values beyond ±2^53 must not round; nulls must be
// skipped by sums and form their own group key).
// ===========================================================================

/// `longSum` over a `LongNullable` column holding 2^53+1: the sum must be
/// the EXACT i64 total of the non-null rows (null skipped), proving the
/// slow path never routes the value through f64. Also pins that the
/// all-null group still reports SQL null.
#[test]
fn longsum_nullable_long_is_i64_exact_beyond_2p53() {
    let base = base_millis();
    let s = |v: &str| Some(v.to_string());
    let segment = SegmentDataBuilder::new()
        .add_timestamp_column((0..4).map(|i| base + i * 1000).collect())
        .add_string_column_nullable("site", vec![s("a"), s("a"), s("a"), s("b")])
        .add_long_column_nullable(
            "v",
            true,
            // 2^53+1 (not f64-representable) + 2 + null → exact 2^53+3.
            vec![Some(9_007_199_254_740_993), Some(2), None, None],
        )
        .build()
        .expect("segment builds");
    let query = json!({
    ...
    assert_eq!(
        groupby_pairs(&result, "site", "s"),
        vec![
            // 9007199254740993 + 2 = 9007199254740995; an f64 round-trip
            // would yield ...992 + 2 = ...994.
            (json!("a"), json!(9_007_199_254_740_995_i64)),
            (json!("b"), json!(null)),
        ],
        "longSum must be i64-exact (null skipped) and null for the all-null group"
    );
}

/// GroupBy KEYED on a `LongNullable` dimension: the beyond-2^53 value must
/// group under its EXACT i64 key, and the null rows must form their own
/// null-keyed group (never merged into the `0` placeholder value).
#[test]
fn groupby_on_nullable_long_dimension_exact_key_and_null_group() {
    let base = base_millis();
    let segment = SegmentDataBuilder::new()
        .add_timestamp_column((0..5).map(|i| base + i * 1000).collect())
        .add_long_column_nullable(
            "code",
            false,
            vec![
                ...
                Some(0), // a REAL 0 row — distinct from the null rows
                None,
            ],
        )
        .build()
    ...
    let result = run(&query, &segment);
    assert_eq!(
        groupby_pairs(&result, "code", "cnt"),
        vec![
            // The REAL 0 row groups alone — the two null rows must NOT
            // leak into it via the 0 placeholder in the value vector.
            (json!(0), json!(1)),
            (json!(9_007_199_254_740_993_i64), json!(2)), // EXACT i64 key
            (json!(null), json!(2)),                      // distinct null group
        ],
        "nullable-long groupBy must key exactly and keep a distinct null group"
    );
}
