...
///
/// compat-11 MV fail-loud: an ARRAY value (the shape a genuine multi-value
/// `StringMulti` row materialises to) must never scalarise into a join key
/// — pre-fix it became the JSON text `"[\"a\",\"b\"]"` and could spuriously
/// equal a right-side literal.  Until element-wise MV joins land, it is a
/// [`DruidError::Query`] naming `key_column`.
///
/// # Errors
///
/// Returns [`DruidError::Query`] when `value` is a (multi-value) array.
fn join_key(key_column: &str, value: &serde_json::Value) -> Result<Option<String>> {
    match value {
        serde_json::Value::Null => Ok(None),
        serde_json::Value::String(s) => Ok(Some(s.clone())),
        serde_json::Value::Bool(b) => Ok(Some(b.to_string())),
        serde_json::Value::Number(n) => Ok(Some(n.to_string())),
        serde_json::Value::Array(_) => Err(DruidError::Query(format!(
            "join on a multi-value dimension `{key_column}` is not supported yet \
             (element-wise MV joins are a follow-on)"
        ))),
        // Objects are not valid scalar join keys (legacy stringly form).
...
impl RightRelation {
    ...
    fn build(
        ...
        for row in &rows {
            // A multi-value (array) right key fails loud (see `join_key`).
            let key = match row.get(right_key).map(|v| join_key(right_key, v)) {
                Some(k) => k?,
                None => None,
            };
...
pub fn execute_join(
    ...
    for left in left_rows {
        // A multi-value (array) left key fails loud (see `join_key`).
        let key = match left.get(&join.condition.left_key) {
            Some(v) => join_key(&join.condition.left_key, v)?,
            None => None,
        };
...
/// compat-11 MV fail-loud, SCHEMA-based: reject a join whose left key
/// COLUMN is a genuine multi-value (`StringMulti`) column of the left probe
/// segment, before any row is rendered.
///
/// The per-value `Value::Array` rejection in [`join_key`] only fires when a
/// rendered row is actually an array — but a `StringMulti` column whose
/// rows all happen to be singletons renders each row as a scalar string
/// (`["US"]` → `"US"`, see [`crate::helpers::column_value_at`]) and joined
/// "successfully", so enforcement was data-dependent: the query only
/// errored once a multi-element row arrived.  The column IS multi-value by
/// schema, so the join is rejected up front regardless of the current row
/// values; the per-row check stays as a defense-in-depth backstop for rows
/// arriving from non-segment sources ([`execute_join`] over materialized
/// rows).
///
/// A scan virtual column of the same name is exempt: it shadows the segment
/// column in every rendered row
/// ([`crate::virtual_columns::VirtualColumns::augment_row`] overwrites) and
/// is itself guarded by `ensure_no_multi_value_refs` — consistent with the
/// aggregation-guard exemption.
///
/// # Errors
///
/// Returns [`DruidError::Query`] naming the multi-value join-key column.
fn ensure_left_key_not_multi_value(
    join: &JoinDataSource,
    left_scan: &ScanQuery,
    left_segment: &SegmentData,
) -> Result<()> {
    let key = &join.condition.left_key;
    let shadowed = left_scan
        .virtual_columns
        .as_deref()
        .unwrap_or_default()
        .iter()
        .any(|vc| match vc {
            crate::virtual_columns::VirtualColumnSpec::Expression { name, .. } => name == key,
        });
    if !shadowed
        && matches!(
            left_segment.columns.get(key),
            Some(ferrodruid_segment::column::ColumnData::StringMulti(_))
        )
    {
        return Err(DruidError::Query(format!(
            "join on a multi-value dimension `{key}` is not supported yet \
             (element-wise MV joins are a follow-on)"
        )));
    }
    Ok(())
...
pub fn execute_join_scan(
    ...
    // Schema-based MV join-key rejection — BEFORE any row is rendered, so a
    // singleton-only `StringMulti` key column cannot slip through as scalars.
    ensure_left_key_not_multi_value(join, left_scan, left_segment)?;
...
mod tests {
    ...
    /// compat-11 MV fail-loud: a multi-value (array) join key must ERROR,
    /// never scalarise into the JSON text `"[\"a\",\"b\"]"` (which could
    /// spuriously equal a right-side literal).  This is exactly the value
    /// shape `materialise_segment_rows` produces for a `StringMulti` row.
    #[test]
    fn mv_left_join_key_fails_loud() {
        let join = JoinDataSource {
            right: JoinRight::Inline {
                column_names: vec!["code".to_string(), "country".to_string()],
                // A malicious/accidental right literal equal to the
                // stringified array — pre-fix this MATCHED left row 0.
                rows: vec![vec![json!("[\"a\",\"b\"]"), json!("bogus")]],
            },
            right_prefix: "r.".to_string(),
            condition: JoinCondition {
                left_key: "tags".to_string(),
                right_key: "code".to_string(),
            },
            join_type: JoinType::Inner,
        };
        ...
        let lookups = LookupManager::new();
        let err = execute_join(&join, &left, &lookups)
            .expect_err("MV join key must fail loud, not stringify");
    ...
    /// The right side of a join is guarded the same way: an array value in
    /// the right key column must ERROR instead of indexing as JSON text.
    #[test]
    fn mv_right_join_key_fails_loud() {
        let join = JoinDataSource {
            right: JoinRight::Inline {
                column_names: vec!["code".to_string(), "country".to_string()],
                rows: vec![vec![json!(["a", "b"]), json!("bogus")]],
        ...
        let lookups = LookupManager::new();
        let err = execute_join(&join, &left_rows(), &lookups)
            .expect_err("MV right join key must fail loud, not stringify");
        let msg = err.to_string();
        assert!(msg.contains("multi-value"), "{msg}");
    ...
    /// [`build_left_segment`] plus a genuine `StringMulti` column `tags`
    /// whose rows are ALL singletons — each row renders as a scalar string
    /// (`["US"]` → `"US"`), so the per-row array check alone never fires.
    fn build_mv_left_segment() -> SegmentData {
        use ferrodruid_segment::column::{ColumnData, StringMultiColumnData};

        let mut segment = build_left_segment();
        let tags = StringMultiColumnData::from_rows(&[
            vec!["US".to_string()],
            vec!["JP".to_string()],
            vec!["US".to_string()],
            vec!["XX".to_string()],
        ...
        segment
            .columns
            .insert("tags".to_string(), ColumnData::StringMulti(tags));
        segment.dimensions.push("tags".to_string());
        segment
    }

    /// compat-11 MV fail-loud (schema-based): a join whose left key COLUMN
    /// is `StringMulti` must ERROR at setup even when every row happens to
    /// be a singleton — pre-fix the singleton rows rendered as scalars and
    /// joined "successfully", so enforcement was data-dependent (the query
    /// only broke once a multi-element row arrived).
    #[test]
    fn mv_singleton_join_key_column_fails_loud_by_schema() {
        let segment = build_mv_left_segment();
        let join = JoinDataSource {
            right: inline_right(),
            right_prefix: "r.".to_string(),
            condition: JoinCondition {
        ...
        let lookups = LookupManager::new();
        let err = execute_join_scan(&join, &scan_all(), &segment, &lookups, None)
            .expect_err("MV join-key column must be rejected by schema, not per-row values");
        let msg = err.to_string();
        assert!(msg.contains("multi-value"), "{msg}");
        assert!(msg.contains("tags"), "{msg}");
        assert!(msg.contains("join"), "{msg}");
    ...
    /// A scan virtual column shadowing the MV name is exempt: the rendered
    /// rows carry the scalar VC value (`augment_row` overwrites), never the
    /// raw MV column — consistent with the aggregation-guard exemption.
    #[test]
    fn mv_join_key_shadowed_by_virtual_column_still_works() {
        let segment = build_mv_left_segment();
        let mut scan = scan_all();
        scan.virtual_columns = Some(vec![
            serde_json::from_value(json!({
                "type": "expression",
                "name": "tags",
                "expression": "cc"
            }))
            .expect("parse virtual column"),
        ]);
        let join = JoinDataSource {
            right: inline_right(),
            right_prefix: "r.".to_string(),
        ...
        let lookups = LookupManager::new();
        let result = execute_join_scan(&join, &scan, &segment, &lookups, None)
            .expect("VC-shadowed join key must not be rejected");
        // The VC copies `cc`, so US, JP, US match (XX dropped by INNER).
