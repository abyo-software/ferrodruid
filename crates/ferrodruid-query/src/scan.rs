...
impl ScanQuery {
    ...
    pub fn execute(&self, segment: &SegmentData) -> Result<ScanResult> {
        ...
        // compat-11 MV fail-loud: an expression over a genuine multi-value
        // (`StringMulti`) column has no element-wise semantics yet — error
        // once at plan time instead of silently stringifying each row's
        // array.  (Scanning the MV column ITSELF stays fully working: it
        // renders arrays/scalars/null, oracle-verified.)
