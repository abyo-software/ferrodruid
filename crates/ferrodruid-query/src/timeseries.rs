...
use crate::helpers::{
    ...
    deserialize_intervals, ensure_aggregations_not_multi_value, feed_aggregator_row_from_map,
...
impl TimeseriesQuery {
    ...
    pub fn execute(&self, segment: &SegmentData) -> Result<Vec<TimeseriesResult>> {
        ...
        // typed-decidable filter. Mirrors the groupBy fast path: no per-row
        // row-map build, no boxed serde_json aggregator dispatch. Falls back
        // to the row loop below for any unsupported shape.
    ...
    /// Handles typed `count`/`longSum`/`doubleSum` aggregators with a
    /// typed-decidable filter and no virtual columns. Returns
    /// `Ok(Some(results))` when it handled the query, `Ok(None)` to fall back
    /// to the row-oriented loop. Output matches that loop for accepted shapes.
    ...
    fn try_vectorized_all(
        ...
        // The filter must be typed-decidable (a per-filter-type property, so
        // probing row 0 is sufficient); otherwise fall back.
        ...
        // Compile the filter once into borrowed column slices + pre-parsed
        // bounds. `try_vectorized_all` only runs when the filter is typed-
        // decidable (probed at row 0 above), so compilation succeeds here; the
        // `matches_typed` per-row fallback is retained defensively.
