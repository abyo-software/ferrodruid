...
use crate::helpers::{
    ...
    ensure_dimension_specs_not_multi_value_coerced, feed_aggregator_row_from_map,
    finalize_agg_value, parse_intervals, pruned_row_range, substitute_cardinality_partials,
...
impl TopNQuery {
    ...
    pub fn execute_with_limit(
        ...
        // stringifying each row's array.  (The topN DIMENSION over an MV
        // column stays fully working: it explodes, oracle-verified.)
        ...
        // compat-11 R3: a topN DimensionSpec over an MV column with a
        ...
        ensure_dimension_specs_not_multi_value_coerced(
            segment,
            std::slice::from_ref(&self.dimension),
            &virtual_columns,
        )?;
        ...
        // sum/count aggregators, no virtual columns, and a filter that
        // compiles to a total typed predicate — typed-decidable for every
        // row).
        ...
        for (row_idx, &ts) in timestamps.iter().enumerate().take(segment.num_rows()) {
            ...
            // compat-11: a MULTI-VALUE dimension row (a JSON array in the
            // row map) EXPLODES — the row contributes one entry per
            // element, each receiving the row's full aggregation
            // contribution (Druid MV topN semantics).  Scalar rows yield
            // exactly one key, preserving the unchanged single-value
            // behaviour; an empty exploded list (every element rejected
            // by a wrapper, or a rejecting wrapper on a scalar) drops the
            // row.
            let dv_keys = crate::groupby::explode_dim_value(&dv, &compiled_dim);
            if dv_keys.is_empty() {
                continue;
            }

            for dv_key in dv_keys {
                ...
                let aggs = dim_aggs.entry(dv_key).or_insert_with(|| {
    ...
    fn try_vectorized_topn(
        ...
        if !virtual_columns.is_empty() || !is_all_granularity(&self.granularity) {
            return Ok(None);
        }
        // W-B legacy null mode: this path keys on raw dictionary codes /
        // column slices, bypassing the legacy canonicalization (`''`≡null
        // merged groups, numeric nulls as 0).  Legacy is a migration-compat
        // mode, not a perf mode — take the row path, which reads through
        // `column_value_at`.  Perf twins are a follow-on.
        if ferrodruid_common::legacy_null_mode() {
            return Ok(None);
        }
        let DimensionSpec::Default { output_type, .. } = &self.dimension else {
            return Ok(None);
        };
        ...
        let dim = match segment.columns.get(dimension_spec_name(&self.dimension)) {
            Some(ColumnData::Long(v)) => {
                // Re-audit Medium (2026-07-19): LONG outputType over a LONG
                // column is the identity coercion and STRING is the typed
                // pass-through — both key/emit the raw i64.  FLOAT/DOUBLE
                // re-key at float/double precision: row path (which coerces
                // via `CompiledDimSpec::apply_typed`).
                if !matches!(output_type, ColumnType::String | ColumnType::Long) {
                    return Ok(None);
                }
                DimKind::Long(v.as_slice())
            }
            Some(ColumnData::String(sc)) => {
                ...
                // Re-audit Medium (2026-07-19): raw dictionary strings are
                // only valid keys under the pass-through STRING outputType;
                // a coercing (numeric) outputType must merge distinct
                // strings into one numeric group ("01"/"1" → 1) — row path.
        ...
        // Re-audit Low (2026-07-19, same species as the groupBy dense
        // finding): the accumulate closure below indexes the dim slice
        // directly (`v[row]` / `encoded_values[row]`), so a dim column
        // shorter than the scan range (corrupt / hand-built / foreign
        // attach segment) would panic out of bounds — a request-triggered
        // DoS.  Fall back to the row path, which resolves missing rows as
        // null gracefully.
        let dim_len = match &dim {
            DimKind::Long(v) => v.len(),
            DimKind::Str(sc) => sc.encoded_values.len(),
        };
        if dim_len < num_rows {
            return Ok(None);
        }
        ...
        // parse per row. Q3 carries no filter, so this is `Some(None)` there.
    ...
    match metric {
        TopNMetricSpec::Numeric { metric: name } => {
            entries.sort_by(|(da, a, _), (db, b, _)| {
                let va = a.get(name).map_or(0.0, metric_rank_value);
                let vb = b.get(name).map_or(0.0, metric_rank_value);
...
/// The ranking value of one TopN metric cell: a plain number ranks as
/// itself; a SKETCH partial-state envelope ranks by its `estimate`
/// convenience field (W-A, v1.5.0 — Druid ranks a `hyperUnique` metric by
/// its estimated cardinality; the same rule serves theta/HLL envelopes).
/// Everything else — JSON null, strings, a mix-error envelope (which
/// carries no estimate) — ranks as 0.0, exactly how the historical
/// `as_f64().unwrap_or(0.0)` ranked non-numeric cells.
///
/// Delegates to the ONE shared cell resolution
/// ([`crate::helpers::numeric_agg_cell`]) so topN ranking, groupBy HAVING,
/// and groupBy limitSpec ordering can never diverge on how a sketch cell
/// reads.
fn metric_rank_value(v: &serde_json::Value) -> f64 {
    crate::helpers::numeric_agg_cell(v).unwrap_or(0.0)
}
...
mod tests {
    ...
    /// single-value STRING column: Druid coerces each value to LONG and
    /// MERGES `"01"` and `"1"` into one numeric entry `1` with a
    /// number-typed key (re-audit Medium 2026-07-19; pre-fix the topN
    /// path ignored `outputType` on single-value columns and kept two
    /// distinct string entries).
    #[test]
    fn topn_output_type_long_coerces_and_merges_string_column() {
        ...
        assert_eq!(
            entries.len(),
            2,
            "\"01\" and \"1\" must merge into the single numeric entry 1"
        );
        let top = &entries[0];
        assert_eq!(
            top.get("code"),
            Some(&serde_json::json!(1)),
            "the merged entry keys as the JSON NUMBER 1 and ranks first (count 2)"
        );
        assert_eq!(top.get("cnt").and_then(|v| v.as_i64()), Some(2));
        assert_eq!(entries[1].get("code"), Some(&serde_json::json!(2)));
        assert_eq!(entries[1].get("cnt").and_then(|v| v.as_i64()), Some(1));
    }

    /// Druid-compat regression (High, 2026-07-19): topN mirror of the
    /// groupBy case — under `outputType: LONG`, `"1.9"` must key as
    /// SQL-null (Druid `BigDecimal.longValueExact` semantics), NOT
    /// truncate into the numeric entry 1.
    #[test]
    fn topn_output_type_long_nonintegral_string_keys_null() {
        use ferrodruid_segment::SegmentDataBuilder;
        let segment = SegmentDataBuilder::new()
            .add_timestamp_column(vec![100, 200, 300])
            .add_string_column("code", vec!["1".into(), "1.9".into(), "2".into()])
            .build()
            .expect("build segment");
        let q = TopNQuery {
            data_source: DataSource::Table { name: "t".into() },
        ...
        assert_eq!(
            entries.len(),
            3,
            "expected entries null / 1 / 2 — \"1.9\" must NOT merge into entry 1"
        );
        let cnt_of = |key: &serde_json::Value| {
            entries
                .iter()
                .find(|e| e.get("code") == Some(key))
                .and_then(|e| e.get("cnt"))
                .and_then(serde_json::Value::as_i64)
        };
        assert_eq!(
            cnt_of(&serde_json::Value::Null),
            Some(1),
            "\"1.9\" keys as the SQL-null entry"
        );
        assert_eq!(
            cnt_of(&serde_json::json!(1)),
            Some(1),
            "entry 1 must count ONLY the exact \"1\" row"
        );
        assert_eq!(cnt_of(&serde_json::json!(2)), Some(1));
    }
    ...
    /// Regression (2026-07-19 High): same row-dependent typed-decidability
    /// species as the timeseries `try_vectorized_all` defect, on the topN
    /// vectorized fast path.  `And`/`Or` short-circuit in `matches_typed`,
    /// so probing row 0 alone must never certify a filter as
    /// typed-decidable for the whole scan.
    ///
    /// Segment `price` (Double) = [5, 20, 30], `cat` (String, null-free)
    /// = [a, b, b]; filter `And[Bound(price > 10, numeric),
    /// Selector(cat = b)]`.  Row 0 fails the Bound so
    /// `matches_typed(segment, 0)` short-circuits to `Some(false)`
    /// without visiting the undecidable Selector; the old row-0 probe
    /// then admitted the filter and the accumulate loop dropped rows 1-2
    /// (`None`-undecided), returning `[]` instead of the correct single
    /// entry (cat = b, cnt = 2).
    #[test]
    fn topn_row_dependent_and_filter_falls_back_correctly() {
        use ferrodruid_segment::SegmentDataBuilder;
        let segment = SegmentDataBuilder::new()
            .add_timestamp_column(vec![100, 200, 300])
        ...
        let filter: crate::filter::FilterSpec = serde_json::from_str(
            r#"{"type":"and","fields":[
        ...
        let q = TopNQuery {
            data_source: DataSource::Table { name: "t".into() },
            intervals: vec!["1970-01-01T00:00:00.000Z/1970-01-01T00:00:01.000Z".into()],
            granularity: GranularitySpec::Simple("all".into()),
            dimension: ferrodruid_common::types::DimensionSpec::Default {
                dimension: "cat".into(),
        ...
        let results = q.execute(&segment).expect("execute");
        assert_eq!(results.len(), 1, "one granularity=all bucket expected");
        let entries = &results[0].result;
        assert_eq!(
            entries.len(),
            1,
            "exactly one entry (cat=b) expected, got {entries:?}"
        );
        assert_eq!(entries[0].get("cat"), Some(&serde_json::json!("b")));
        assert_eq!(
            entries[0].get("cnt"),
            Some(&serde_json::json!(2)),
            "price>10 AND cat=b matches exactly rows 1-2, got {entries:?}"
        );
    }
}
