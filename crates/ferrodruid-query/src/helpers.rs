...
/// Gate + compile an optional filter for a vectorized fast-path scan,
/// requiring typed decidability for **every** row.
///
/// Returns:
///
/// * `Some(None)` — no filter: scan unconditionally.
/// * `Some(Some(compiled))` — the WHOLE filter tree compiled, so the
///   per-row predicate is total ([`CompiledFilter::eval`] yields a
///   definitive bool for every row of this segment).
/// * `None` — the filter is not fully typed-compilable: the caller MUST
///   fall back to the row-map path for the entire segment.
///
/// A `matches_typed` probe at row 0 is NOT a sound gate here (the
/// pre-2026-07-19 defect): `And`/`Or` short-circuit inside
/// [`FilterSpec::matches_typed`], so typed decidability is
/// **row-dependent** — row 0 can decide `Some(false)`/`Some(true)` via a
/// typed clause without ever visiting an undecidable sibling, while later
/// rows reach that sibling and return `None`. A vectorized accumulate loop
/// that treats `!= Some(true)` as reject then silently drops exactly those
/// rows (wrong aggregates / empty results). The structural
/// [`FilterSpec::compile_typed`] succeeds only when every node of the tree
/// is typed-supported against this segment, which is precisely the
/// every-row guarantee the fast path needs.
#[must_use]
#[allow(clippy::option_option)] // outer = "may the fast path run", inner = "is there a filter"
pub fn compile_vectorized_filter<'a>(
    filter: Option<&FilterSpec>,
    segment: &'a SegmentData,
) -> Option<Option<CompiledFilter<'a>>> {
    match filter {
        None => Some(None),
        Some(f) => f.compile_typed(segment).map(Some),
    }
}
...
///
/// Both spellings of every granularity agree: the string form (e.g.
/// `"none"`) and the object form (e.g. `{"type":"none"}`) produce the
/// same bucket for the same timestamp.
pub fn bucket_timestamp(ts_millis: i64, granularity: &GranularitySpec) -> i64 {
    let period_ms = match granularity {
        GranularitySpec::Simple(s) => match s.to_lowercase().as_str() {
            "all" => return 0,
            // Druid's `none` granularity does NOT bucket — every distinct
            // millisecond is its own bucket, identical to the object form
            // `Granularity::None` below.  (Re-audit Medium 2026-07-19: the
            // string spelling previously collapsed to the single epoch
            // bucket like `"all"`, so `"granularity":"none"` — the form
            // most clients emit — aggregated everything into one
            // 1970-01-01 row while `{"type":"none"}` bucketed per
            // millisecond.)
            "none" => return ts_millis,
            "second" => 1_000,
            "minute" => 60_000,
            "five_minute" => 300_000,
            "ten_minute" => 600_000,
...
///
/// # Legacy null mode (W-B, `useDefaultValueForNull=true`)
///
/// Under [`ferrodruid_common::legacy_null_mode`] this function is the ONE
/// read-side canonicalization point (every row-map path — filters,
/// aggregator feeds, group keys, scan output — reads through it), and it
/// reproduces what a legacy Druid *reads* (all oracle-measured, Druid
/// 27.0.0, `oracle/legacy27*` fixtures):
///
/// * numeric NULL reads as the default `0`/`0.0` (native scan shows the
///   literal zeros; `SUM(x)` over an all-null column = 0; `x IS NULL`
///   matches nothing);
/// * a STRING `""` value reads as JSON **null** — the canonical merged
///   ''/null value.  Druid's own legacy read renders BOTH `''` and
///   absent rows as null in native scan/groupBy/topN output while its
///   SQL layer re-renders that null as `""` (both pinned); FerroDruid
///   mirrors that split: canonical null here, `""` restored at the SQL
///   output layer;
/// * a multi-value (`StringMulti`) cell canonicalizes its `""` ELEMENTS
///   the same way (H2): a singleton `[""]` row reads as canonical null,
///   and a multi-element row's `""` element reads as a null element —
///   so a legacy `''`/null selector matches by the any-element rule and
///   scan/explosion render the element as null.  Consistent residual:
///   string-domain predicates (bound/like/regex/search/bloom) see no
///   candidate for such an element, exactly as they see none for a
///   scalar canonical-null cell.
    ...
    match col {
        ...
        ColumnData::LongNullable(v, nulls) => {
            if !legacy && is_null_long_row(nulls, row_idx) {
                // ANSI: the bitmap marks SQL NULL. Legacy: the value vector
                // already stores the coerced default 0 at NULL rows — read
                // it as-is (an ANSI-written segment thereby answers with
                // legacy semantics, matching a legacy Druid's read).
        ...
        ColumnData::Float(v) => v
            ...
            .and_then(|&x| {
                if legacy && x.is_nan() {
                    // Legacy: the in-band NaN null reads as the 0.0 default.
        ...
        ColumnData::String(sc) => {
            ...
            if sc.is_null_row(row_idx) {
                ...
                match sc.dictionary.get(ord) {
                    // Legacy: `''` IS the null value — canonicalize to JSON
                    // null on read (Druid 27 renders both `''` and absent
                    // rows as null in every native surface; the SQL layer
                    // restores `""`).
                    Some(s) if legacy && s.is_empty() => serde_json::Value::Null,
        ...
        //
        // W-B legacy (H2): a `''` ELEMENT canonicalizes exactly like a
        // scalar `''` cell — a singleton `[""]` row reads as canonical
        // null and a multi-element row's `''` element reads as a null
        // element — so the merged `''`/null selector matches by the
        // usual any-element rule and native scan/explosion render null.
        // ANSI reads are byte-identical to the pre-W-B arm.
        ColumnData::StringMulti(mc) => {
            let render = |ord: u32| {
                mc.dictionary
                    .get(ord as usize)
                    .map(|s| {
                        if legacy && s.is_empty() {
                            serde_json::Value::Null
                        } else {
            ...
            match mc.row_ordinals(row_idx) {
                None | Some([]) => serde_json::Value::Null,
                Some([ord]) => render(*ord),
                Some(ords) => {
                    serde_json::Value::Array(ords.iter().map(|&ord| render(ord)).collect())
                }
            }
        }
        ...
        ColumnData::ComplexTheta(rows) => rows
            ...
            .unwrap_or(serde_json::Value::Null),
        // Migrated Druid `hyperUnique` metric (W-A, v1.5.0): render the
        // row's decoded register state as the druidHyperUnique
        // partial-state envelope — the same shape `HyperUniqueAggregator`'s
        // own `get` emits — so the aggregator's merge feed folds the
        // per-row sketches by register-wise max (Druid's own fold), and
        // the SQL `APPROX_COUNT_DISTINCT` accumulator can adopt it.
        ColumnData::ComplexHyperUnique(rows) => rows
            .get(row_idx)
            .map(ferrodruid_aggregator::druid_hyper_unique_envelope)
            .unwrap_or(serde_json::Value::Null),
    }
}
...
/// True when `value` is a SKETCH partial-state envelope — a JSON object
/// carrying the `@sketch` tag (a decoded-Druid hyperUnique, a theta or HLL
/// sketch, or the loud mix-error envelope).
pub fn is_sketch_envelope(value: &serde_json::Value) -> bool {
    value
        .as_object()
        .is_some_and(|obj| obj.contains_key("@sketch"))
}

/// The numeric comparison value of one aggregation output cell — the ONE
/// shared resolution used by every path that compares or ranks a metric
/// cell (topN metric ranking, groupBy HAVING, groupBy limitSpec ordering),
/// so the symmetric paths can never diverge on how a sketch cell reads:
///
/// * a plain JSON number is itself;
/// * a SKETCH partial-state envelope (see [`is_sketch_envelope`]) resolves
///   to its `estimate` convenience field — Druid compares/ranks a
///   `hyperUnique` / `thetaSketch` aggregation by its estimated
///   cardinality, and the `estimate` field is recomputed by every envelope
///   constructor and JSON merge, so it always equals the finalized value;
/// * everything else — JSON null, strings, a mix-error envelope (which
///   deliberately carries no estimate), non-envelope objects — is `None`,
///   letting each caller keep its historical non-numeric behavior (rank as
///   `0.0` in ordering paths; fail the comparison in HAVING).
pub fn numeric_agg_cell(value: &serde_json::Value) -> Option<f64> {
    if let Some(n) = value.as_f64() {
        return Some(n);
    }
    let obj = value.as_object()?;
    obj.get("@sketch")?;
    obj.get("estimate").and_then(serde_json::Value::as_f64)
}

/// Extract the value to feed into an aggregator for a given row.
///
/// # Legacy null mode (W-B H1, physically-absent columns)
///
/// Under [`ferrodruid_common::legacy_null_mode`] a column the segment
/// lacks ENTIRELY (schema evolution) feeds the aggregator its legacy
/// default instead of `None` — so an absent column is indistinguishable
/// from a present column whose every cell is null:
///
/// * numeric aggregators ([`legacy_agg_input_kind`] = `Long`/`Double`)
///   read the coerced default `0`/`0.0`, mirroring Druid's numeric
///   value selector over a missing column
///   (`NilColumnValueSelector.getLong() == 0` under
///   `replaceWithDefault`) and this crate's own read of a present
///   legacy null cell;
/// * every other aggregator (HLL / theta / cardinality / string
///   first-last / …) reads canonical JSON null — the merged `''`/null
///   value a present `''` cell also feeds — mirroring Druid's
///   dimension-flavored read of a missing column (null string), so
///   e.g. approximate `COUNT DISTINCT` counts the ONE `''` value.
///
/// ANSI behavior (absent column → `None` → aggregator skip) is
...
pub fn extract_agg_value(
    ...
    match spec.field_name() {
        ...
        Some(field) => match segment.columns.get(field) {
            Some(col) => Some(column_value_at(col, row_idx)),
            None if ferrodruid_common::legacy_null_mode() => {
                Some(ferrodruid_common::null_mode::legacy_canonical_cell(
                    legacy_agg_input_kind(spec),
                    None,
                ))
            }
            None => None,
...
/// The [`ferrodruid_common::null_mode::LegacyColumnKind`] an aggregator
/// reads its input column as, used by [`extract_agg_value`] to pick the
/// legacy default for a PHYSICALLY-ABSENT column (W-B H1).
///
/// Mirrors Druid's selector flavors: the long family reads through a
/// long value selector (missing → `0`), the double/float family —
/// including variance and the quantiles sketch, whose inputs are
/// doubles — through a double selector (missing → `0.0`), and every
/// other aggregator through a dimension-flavored (string) read
/// (missing → null string).  `Filtered` defers to its inner spec.
fn legacy_agg_input_kind(spec: &AggregatorSpec) -> ferrodruid_common::null_mode::LegacyColumnKind {
    use ferrodruid_common::null_mode::LegacyColumnKind as K;
    match spec {
        AggregatorSpec::LongSum { .. }
        | AggregatorSpec::LongMin { .. }
        | AggregatorSpec::LongMax { .. }
        | AggregatorSpec::LongFirst { .. }
        | AggregatorSpec::LongLast { .. } => K::Long,
        AggregatorSpec::DoubleSum { .. }
        | AggregatorSpec::FloatSum { .. }
        | AggregatorSpec::DoubleMin { .. }
        | AggregatorSpec::DoubleMax { .. }
        | AggregatorSpec::FloatMin { .. }
        | AggregatorSpec::FloatMax { .. }
        ...
        | AggregatorSpec::QuantilesDoublesSketch { .. } => K::Double,
        AggregatorSpec::Filtered { aggregator, .. } => legacy_agg_input_kind(aggregator),
        // EXHAUSTIVE on purpose (compat-11 lesson): a future aggregator
        // must consciously pick its missing-column kind, not silently
        // inherit the string default.
        AggregatorSpec::Count { .. }
        | AggregatorSpec::StringFirst { .. }
        | AggregatorSpec::StringLast { .. }
        | AggregatorSpec::Cardinality { .. }
        | AggregatorSpec::HllSketchBuild { .. }
        | AggregatorSpec::HllSketchMerge { .. }
        | AggregatorSpec::ThetaSketch { .. }
        | AggregatorSpec::HyperUnique { .. }
        | AggregatorSpec::ArrayAgg { .. }
        | AggregatorSpec::StringAgg { .. }
        | AggregatorSpec::BloomFilter { .. }
        | AggregatorSpec::Grouping { .. } => K::String,
    }
}
...
///   (`["01"]` and `["1"]` both become the numeric group `1`).
///   Single-value columns now perform exactly this coercion
///   ([`crate::dim_spec::CompiledDimSpec::apply_typed`], re-audit Medium
///   2026-07-19), and `explode_dim_value` routes each element through it,
///   but the MV explosion shape remains UNVERIFIED against the Druid
///   oracle — keep rejecting it until an oracle run confirms the
...
mod tests {
    ...
    /// Druid buckets granularity `none` per MILLISECOND (identity) for
    /// BOTH spellings — the string `"none"` and the object
    /// `{"type":"none"}`.  Re-audit Medium (2026-07-19): the string
    /// spelling previously collapsed to the single epoch bucket like
    /// `"all"`, so the two spellings of the same granularity produced
    /// different results and the string form diverged from Druid.
    #[test]
    fn granularity_none_string_buckets_per_millisecond_like_object_form() {
        let simple = GranularitySpec::Simple("none".to_string());
        let object = GranularitySpec::Full(ferrodruid_common::types::Granularity::None);
        for ts in [
            0_i64,
            1,
            999,
            ...
            assert_eq!(
                bucket_timestamp(ts, &simple),
                ts,
                "string \"none\" must be the identity bucket (per-millisecond)"
            );
            assert_eq!(
                bucket_timestamp(ts, &simple),
                bucket_timestamp(ts, &object),
                "string and object spellings of `none` must agree"
            );
        }
        // Case-insensitive like every Simple spelling.
        assert_eq!(
            bucket_timestamp(42, &GranularitySpec::Simple("NONE".into())),
            42
        );
        // "all" keeps the single epoch bucket.
        assert_eq!(
            bucket_timestamp(
                1_704_067_200_123,
    ...
    /// The shared metric-cell resolution: numbers are themselves, sketch
    /// envelopes resolve to their `estimate`, and everything else (null /
    /// string / estimate-less mix-error envelope / non-envelope object)
    /// is None — the contract topN ranking, groupBy HAVING, and groupBy
    /// limitSpec ordering all rely on identically.
    #[test]
    fn numeric_agg_cell_resolves_numbers_and_sketch_estimates_only() {
        use serde_json::json;

        assert_eq!(numeric_agg_cell(&json!(7.5)), Some(7.5));
        assert_eq!(numeric_agg_cell(&json!(3)), Some(3.0));
        assert_eq!(
            numeric_agg_cell(&json!({"@sketch": "theta", "bytes": "AAAA", "estimate": 12.0})),
            Some(12.0)
        );
        assert_eq!(
            numeric_agg_cell(
                &json!({"@sketch": "druidHyperUnique", "bytes": "AAAA", "estimate": 40.4})
            ),
            Some(40.4)
        );
        // A mix-error envelope deliberately carries no estimate.
        assert_eq!(
            numeric_agg_cell(&json!({"@sketch": "mixError", "message": "boom"})),
            None
        );
        assert_eq!(numeric_agg_cell(&serde_json::Value::Null), None);
        assert_eq!(numeric_agg_cell(&json!("12")), None);
        // A non-envelope object never numerifies, even with an `estimate`
        // field (only the `@sketch` tag marks an envelope).
        assert_eq!(numeric_agg_cell(&json!({"estimate": 5.0})), None);

        assert!(is_sketch_envelope(&json!({"@sketch": "mixError"})));
        assert!(!is_sketch_envelope(&json!({"estimate": 5.0})));
        assert!(!is_sketch_envelope(&json!(1.0)));
    }
}
