        ...
        match self {
            Self::Selector { dimension, value } => {
                ...
                match (row_val, value) {
                    ...
                    (Some(rv), Some(fv)) => values_equal(rv, fv),
                }
            }
            ...
            Self::In { dimension, values } => {
                ...
                match row_val {
                    ...
                    Some(rv) => values.iter().any(|v| values_equal(rv, v)),
                }
            }
            ...
            Self::Bound {
                ...
                if is_numeric {
                    ...
                    if let Some(lo) = lower {
                        if lower_strict.unwrap_or(false) {
                            if row_str.as_str() <= lo.as_str() {
                                return false;
                            }
                        } else if row_str.as_str() < lo.as_str() {
                            return false;
                        }
                    }
                    if let Some(hi) = upper {
                        if upper_strict.unwrap_or(false) {
                            if row_str.as_str() >= hi.as_str() {
                                return false;
                            }
                        } else if row_str.as_str() > hi.as_str() {
                            return false;
                        }
                    }
                    true
            ...
            Self::Range {
                ...
                match row_val {
                    ...
                    Some(rv) => {
                        let is_numeric =
                            matches!(match_value_type.as_str(), "LONG" | "DOUBLE" | "FLOAT");
                        if is_numeric {
                            let rv_f = value_to_f64(rv);
                            let rv_f = match rv_f {
                                Some(v) => v,
                                None => return false,
                            };
            ...
            Self::Search { dimension, query } => {
                ...
                search_query_matches(query, &row_str)
            }
            ...
            Self::BloomFilter {
                ...
                let Some(row_val) = row.get(dimension) else {
                    return false;
                };
                if row_val.is_null() {
                    return false;
                ...
                let probe = value_to_string(row_val);
                filter.test(&probe)
            }
