...
impl FilterSpec {
    ...
    pub fn matches(&self, row: &HashMap<String, serde_json::Value>) -> bool {
        match self {
            Self::Selector { dimension, value } => {
                ...
                // W-B legacy null mode: a `''` selector VALUE is the null
                // selector — `selector(col, "")` and `selector(col, null)`
                // are one filter (oracle native_filter_selector_empty_strcol
                // = native_filter_selector_null_strcol = 3 on the fixture;
                // ANSI: 2 vs 1).  Row values arrive already canonicalized
                // (`''` reads as JSON null under legacy), so normalizing
                // the filter value completes the equivalence.
                let value: Option<&serde_json::Value> = match value {
                    Some(v) if v.as_str() == Some("") && ferrodruid_common::legacy_null_mode() => {
                        None
                    }
                    other => other.as_ref(),
                };
                match (row_val, value) {
                    ...
                    (None, Some(fv)) | (Some(serde_json::Value::Null), Some(fv)) => fv.is_null(),
                    ...
                    // W-B legacy (H2): a `''` element canonicalizes to a
                    // null ELEMENT (`column_value_at`), and the merged
                    // `''`/null selector matches it by the usual
                    // any-element rule.  ANSI arrays carry no null
                    // elements from the read path, so ANSI behavior is
                    // unchanged.
                    (Some(serde_json::Value::Array(a)), None) => {
                        a.is_empty()
                            || (ferrodruid_common::legacy_null_mode()
                                && a.iter().any(serde_json::Value::is_null))
                    }
            ...
            Self::In { dimension, values } => {
                let row_val = row.get(dimension);
                // W-B legacy null mode: a `''` entry in the IN list matches
                // the merged ''/null rows exactly like a null entry (oracle
                // native_filter_in_strcol_a_empty =
                // native_filter_in_strcol_a_null = 4; ANSI: 3 vs 2).
                let null_in_list = |vs: &[serde_json::Value]| {
                    let legacy = ferrodruid_common::legacy_null_mode();
                    vs.iter()
                        .any(|v| v.is_null() || (legacy && v.as_str() == Some("")))
                };
                match row_val {
                    None | Some(serde_json::Value::Null) => null_in_list(values),
                    // Defensive `[]`-as-null (see the Selector arm).
                    Some(serde_json::Value::Array(a)) if a.is_empty() => null_in_list(values),
                    // compat-11: an MV row matches when ANY element is in
                    // the set (Druid IN-on-MV semantics).
                    // W-B legacy (H2): a canonicalized `''`(=null) MV
                    // element matches a `''` IN entry through the merged
                    // `''`/null rule (a bare null entry already matches a
                    // null element via `row_value_equals`, in any mode).
                    Some(rv) => {
                        let legacy_null_element = ferrodruid_common::legacy_null_mode()
                            && matches!(rv, serde_json::Value::Array(a)
                                if a.iter().any(serde_json::Value::is_null))
                            && null_in_list(values);
                        legacy_null_element || values.iter().any(|v| row_value_equals(rv, v))
                    }
                }
            }
            ...
            Self::Null { column } => {
                match row.get(column) {
                    ...
                    // W-B legacy (H2): an MV row containing the
                    // canonicalized `''`(=null) element is null-matching
                    // under the merged `''`/null semantics (any-element
                    // rule, same as the null selector; an empty array is
                    // defensive — it normally materialises as JSON null).
                    // ANSI arrays never matched here and still don't.
                    Some(serde_json::Value::Array(a)) if ferrodruid_common::legacy_null_mode() => {
                        a.is_empty() || a.iter().any(serde_json::Value::is_null)
                    }
                    _ => false,
                }
            }
    ...
    ///
    /// **Typed decidability is ROW-DEPENDENT, not a per-filter-type
    /// property**: `And`/`Or` short-circuit below, so a decidable clause
    /// can settle one row's answer without ever visiting an undecidable
    /// sibling that a *different* row would reach (2026-07-19 High). A
    /// `Some(..)` at one row therefore certifies NOTHING about other
    /// rows — never use a single-row probe to gate a whole-segment scan
    /// that treats `None` as a non-match. Vectorized fast paths must
    /// gate on the structural [`Self::compile_typed`] instead (see
    /// `crate::helpers::compile_vectorized_filter`); per-row callers
    /// must route `None` to the row-map path as shown below.
    ///
    ...
    pub fn matches_typed(&self, segment: &SegmentData, row_idx: usize) -> Option<bool> {
        // W-B legacy null mode: typed evaluation reads raw column slices
        // and would diverge from the legacy row-map semantics (numeric
        // NULLs must read as the 0/0.0 defaults, e.g. a NaN-null double
        // MUST pass `bound(d >= 0)`).  Legacy is a migration-compat mode,
        // not a perf mode — force every filter onto the row-map path,
        // which reads through the ONE legacy canonicalization point
        // (`column_value_at`).  Perf twins are a documented follow-on.
    ...
    pub fn compile_typed<'a>(&self, segment: &'a SegmentData) -> Option<CompiledFilter<'a>> {
        // W-B legacy null mode: same gate as [`Self::matches_typed`] — the
        // compiled fast path reads raw slices and must not run under
        // legacy semantics; `None` sends every caller (including
        // `compile_vectorized_filter`) to the row-map fallback.
        if ferrodruid_common::legacy_null_mode() {
            return None;
        }
        match self {
...
fn values_equal(a: &serde_json::Value, b: &serde_json::Value) -> bool {
    ...
    // W-B legacy null mode: two JSON numbers compare NUMERICALLY, so the
    // coerced double default `0.0` matches the SQL literal `0` (oracle
    // ext_count_d_eq_0.json: legacy `d = 0` matches the coerced rows; the
    // ANSI string-coercion fallback below renders them "0.0" vs "0" and
    // would miss).  ANSI behavior is unchanged (`serde_json::Number` for
    // 0.0 vs 0 differ, so the historical string comparison decides there).
    if ferrodruid_common::legacy_null_mode()
        && let (serde_json::Value::Number(na), serde_json::Value::Number(nb)) = (a, b)
    {
        return legacy_numbers_equal(na, nb);
    }
...
/// W-B legacy equality between a canonicalized numeric row value and a
/// role-split wire STRING literal (H2).
///
/// The role-split wire filter (`ferrodruid-rpc`'s `EqualsFilter`)
/// carries its literal as a string; under the latch a numeric cell must
/// compare NUMERICALLY against it (`0.0` matches `"0"`), through the
/// SAME exact-integer rule as the single-binary Selector path
/// ([`legacy_numbers_equal`]) — never through a lossy f64 round-trip —
/// so the two paths cannot diverge on a legacy filter answer.  The
/// literal parses like a JSON number (i64, then u64, then f64); a
/// non-numeric literal never equals a numeric cell.
#[must_use]
pub fn legacy_number_equals_literal(n: &serde_json::Number, literal: &str) -> bool {
    let lit = if let Ok(i) = literal.parse::<i64>() {
        ...
        match serde_json::Number::from_f64(f) {
            Some(x) => x,
            // NaN / infinity spellings have no JSON-number identity.
            None => return false,
        }
    } else {
        return false;
    };
    legacy_numbers_equal(n, &lit)
}

/// Exact numeric equality for the W-B legacy branch of [`values_equal`].
///
/// Integer operands compare as integers — NEVER through f64, which
/// collapses neighbours beyond 2^53 (the compat-10 silent-precision-loss
/// species: `9007199254740992 == 9007199254740993` must be FALSE, or
/// selector/IN return extra rows).  The legacy null->0 coercion only
/// needs *numeric* equality across widths (`0` matches `0.0`), so:
///
/// * i64 ↔ i64 and u64 ↔ u64 compare exactly;
/// * disjoint integer ranges (a beyond-`i64::MAX` u64 vs a negative
///   i64) are never equal;
/// * a float operand equals an integer only when it is an in-range
///   integral double that converts back to the same integer;
/// * float ↔ float keeps the plain IEEE comparison (`serde_json`
///   numbers are never NaN).
fn legacy_numbers_equal(a: &serde_json::Number, b: &serde_json::Number) -> bool {
    ...
    if !a.is_f64() && !b.is_f64() {
        // Both integers, but in disjoint ranges: never equal.
        return false;
    }
    if a.is_f64() && b.is_f64() {
        return a.as_f64() == b.as_f64();
    }
    // Exactly one float operand: exact integer-vs-double comparison.
    let (int_side, float_side) = if a.is_f64() { (b, a) } else { (a, b) };
    let Some(f) = float_side.as_f64() else {
        return false;
    };
    if let Some(i) = int_side.as_i64() {
        return f64_equals_i64(f, i);
    }
    if let Some(u) = int_side.as_u64() {
        return f64_equals_u64(f, u);
    }
    false
}

/// True iff the double `f` is exactly the integer `i`.
///
/// In-range (|f| < 2^63) integral doubles truncate to i64 exactly, so
/// the round-trip comparison is precision-loss-free; anything integral
/// but out of range can never equal an i64.
fn f64_equals_i64(f: f64, i: i64) -> bool {
    // -(2^63) == i64::MIN (exact); the exclusive 2^63 end excludes overflow.
...
/// True iff the double `f` is exactly the (beyond-`i64::MAX`) integer `u`.
fn f64_equals_u64(f: f64, u: u64) -> bool {
    // The exclusive 2^64 end excludes overflow.
    #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let eq =
        f.fract() == 0.0 && (0.0..18_446_744_073_709_551_616.0).contains(&f) && (f as u64) == u;
    eq
}
