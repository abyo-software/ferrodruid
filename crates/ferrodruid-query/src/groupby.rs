...
use ferrodruid_segment::column::{ColumnData, is_null_double};
...
use crate::helpers::{
    ...
    deserialize_intervals, feed_aggregator_row, feed_aggregator_row_from_map, finalize_agg_value,
...
impl GroupByQuery {
            ...
            if let Some(ref columns) = limit_spec.columns {
                for col_spec in columns.iter().rev() {
                    ...
                    results.sort_by(|a, b| {
                        ...
                        // Multi-shard exact union (2026-07-11): exact-
                        // cardinality outputs are envelope-shaped partials
                        // at this point (substituted after HAVING in
                        // `aggregate_active_dims`), so ordering on such a
                        // column must read the envelope's exact count, not
                        // the envelope object (which would sort as 0 /
                        // stringify as JSON).  The unchecked probe is
                        // clone-free and O(1); it is safe here because the
                        // envelopes were emitted by THIS executor a few
                        // lines earlier — peer envelopes are validated at
                        // the merge/finalize trust boundaries instead.
                        let ca = va
                        ...
                        // W1-J finding-C: Druid/Calcite default ordering is
                        // NULLS FIRST in ASC (and therefore NULLS LAST in
                        // DESC, since DESC reverses the ASC order).  Without
                        // an explicit null branch the lex `to_string()`
                        // fallback below stringifies `null` to `"null"`,
                        // which sorted GROUPING SETS / CUBE / ROLLUP
                        // null-groups *after* real values whose first byte is
                        // less than `'n'` (e.g. `"Hauptseite"`) — and *before*
                        // values starting with characters greater than `'n'`
                        // — diverging from Druid on the cl4_grouping_sets /
                        // cube / rollup harness queries.  Treat a missing
                        // key (`None`) the same as `Value::Null` because
                        // subtotal rows store omitted dimensions as JSON
                        // null but may also omit the key entirely on
                        // upstream paths that have not been updated.
                        let na = va.is_none_or(serde_json::Value::is_null);
                        let nb = vb.is_none_or(serde_json::Value::is_null);
                        let ord = match (na, nb) {
                            (true, true) => std::cmp::Ordering::Equal,
                            // null < non-null in ASC ⇒ matches Druid NULLS
                            // FIRST.  The trailing `if desc { reverse }`
                            // flips this so DESC puts nulls last as Druid
                            // does.
                            (true, false) => std::cmp::Ordering::Less,
                            (false, true) => std::cmp::Ordering::Greater,
                            (false, false) => {
                                if numeric {
                                    let fa = va.and_then(|v| v.as_f64()).unwrap_or(0.0);
                                    let fb = vb.and_then(|v| v.as_f64()).unwrap_or(0.0);
                                    fa.partial_cmp(&fb).unwrap_or(std::cmp::Ordering::Equal)
                                } else {
                                    // Wave 40-B: typed dimensions emit their
                                    // JSON form (Number / Bool) rather than
                                    // String, so a lex sort must fall
                                    // through to `to_string()` rather than
                                    // only honour `as_str()`.  Nulls have
    ...
    fn aggregate_active_dims(
        ...
        'rows: for (row_idx, &ts) in timestamps.iter().enumerate().take(segment.num_rows()) {
            ...
            if row_map_free {
                ...
                for &idx in active {
                    ...
                    let is_default = matches!(dim_spec, DimensionSpec::Default { .. });
                    let group_key = if is_default {
                        segment.columns.get(name).map_or(GroupKey::Null, |col| {
    ...
    fn try_vectorized_active_dims(
        ...
        for &idx in active {
            let dim_spec = &self.dimensions[idx];
            if !matches!(dim_spec, DimensionSpec::Default { .. }) {
                return Ok(false);
            }
            match segment.columns.get(dim_spec_input_name(dim_spec)) {
                ...
                Some(ColumnData::Long(v)) => dim_cols.push(DimCol::Long(v.as_slice())),
                _ => return Ok(false),
            }
        }
        ...
        let mut accs: Vec<Acc> = agg_plans
            ...
            .collect();
        // Hoist the granularity bucket for the constant "all"/"none" case: its
        // `to_lowercase()` otherwise allocates a String on every row.
        let const_bucket: Option<i64> = match &self.granularity {
            GranularitySpec::Simple(s)
                if s.eq_ignore_ascii_case("all") || s.eq_ignore_ascii_case("none") =>
            {
                Some(0)
            }
            _ => None,
        ...
        let dense: Option<(Vec<usize>, usize)> = const_bucket.and_then(|_| {
            ...
            for j in (0..dim_cols.len()).rev() {
                ...
                let card = match &dim_cols[j] {
                    DimCol::Str(sc) => sc.dictionary.len(),
        ...
        // gid -> key ([bucket, dim_code_0, ..]); indexes `accs` and drives emit.
        let mut group_keys: Vec<Vec<i64>> = Vec::new();
        // Group key = [bucket, dim_code_0, .., dim_code_{k-1}] (all i64).
        let mut key_buf: Vec<i64> = Vec::with_capacity(dim_cols.len() + 1);
            ...
            for dc in &dim_cols {
                let code = match dc {
                    DimCol::Str(sc) => sc
                        .encoded_values
                        .get(row)
                        .map_or(i64::MIN, |&c| i64::from(c)),
                    DimCol::Long(v) => v.get(row).copied().unwrap_or(i64::MIN),
                };
                key_buf.push(code);
            }
...
mod tests {
    ...
    fn groupby_unsupported_post_agg_errors() {
        ...
        let q = parse_groupby(
            r#"{
                ...
                "aggregations": [{"type":"count","name":"cnt"}],
                "postAggregations": [
                    {"type":"hyperUniqueCardinality","name":"bad","fieldName":"cnt"}
                ]
            }"#,
        );
    ...
    fn limit_spec_sort_puts_nulls_first_in_asc() {
        ...
        // Drive the same sort code path the LimitSpec uses for an
        // ASC string dimension by inlining its logic — we keep the
        // unit test pure (no segment, no query plumbing) so a future
        // regression in the comparator is immediately visible.
