...
impl VirtualColumns {
    ...
    /// [`DruidError::Query`] if any expression is malformed, any spec uses
    /// an unsupported virtual-column type (only `expression` is supported),
    /// or any spec is named `__time` (reserved, see below).
    ///
    /// `None` / an empty list compile to an empty (no-op) set.
    ///
    /// ## `__time` is reserved (multi-segment ordering High, 2026-07-21)
    ///
    /// `__time` is Druid's reserved row-timestamp column and a virtual
    /// column may not redefine it (Druid rejects such specs at query
    /// validation).  Beyond wire fidelity this is load-bearing for
    /// correctness: [`Self::augment_row`] runs AFTER the scan executor's
    /// physical-time sort and would overwrite the emitted `__time` cell,
    /// while the broker's time-ordered multi-segment merge re-sorts on
    /// that projected cell — a shadowing virtual `__time` (e.g. the
    /// constant `0`) tied every cross-segment row so shard-arrival order,
    /// not physical time, decided which rows survived the LIMIT.
    /// Rejecting the name at the single compile choke point closes the
    /// hole for every query type (scan / timeseries / groupBy / topN).
                ...
                VirtualColumnSpec::Expression {
                    ...
                    if name == "__time" {
                        return Err(DruidError::Query(
                            "virtual column must not be named '__time': '__time' is the \
                             reserved row-timestamp column and cannot be redefined"
                                .to_string(),
                        ));
                    }
...
mod tests {
    ...
    /// Multi-segment ordering High (2026-07-21): a virtual column named
    /// `__time` must be rejected at compile time.  `__time` is Druid's
    /// reserved row-timestamp column; allowing a virtual column to shadow
    /// it made the broker's time-ordered scan merge sort on the SHADOWED
    /// (post-`augment_row`) value while each segment executor sorted on
    /// the PHYSICAL timestamp — a constant virtual `__time` tied every
    /// cross-segment row and shard-arrival order decided which rows
    /// survived the LIMIT.
    #[test]
    fn reserved_time_column_name_rejected() {
        let err = VirtualColumns::compile(&specs(json!([
            {"type": "expression", "name": "__time", "expression": "0"}
        ])))
        .expect_err("virtual column named __time must be rejected");
        match err {
