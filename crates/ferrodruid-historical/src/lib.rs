...
fn column_type_of(col: &ferrodruid_segment::column::ColumnData) -> ColumnType {
    ...
    match col {
        ...
        ColumnData::ComplexHyperUnique(_) => ColumnType::Complex("hyperUnique".to_string()),
    }
}
...
impl Historical {
    ...
    fn execute_query_on_snapshot(
        ...
        // W-C S3: iterate segments in segment-id order, NOT HashMap order.
        // Per-segment partials feed the broker merge, and HashMap iteration
        // gave the fan-out a per-process-random shard order — observed as
        // restart variance in the proving run ([03,01,02] / [01,02,03] / …)
        // and wrong `ORDER BY __time LIMIT k` rows.  Druid segment IDs
        // embed `interval-start_interval-end_version_partition`, so
        // lexicographic ID order approximates timeline order and, more
        // importantly, makes every merge tiebreak (equal `__time` rows,
        // scan column-union order, groupBy arrival order) deterministic
        // across runs.
        let mut ordered_segments: Vec<(&String, &SegmentEntry)> = segments.iter().collect();
        ordered_segments.sort_unstable_by(|a, b| a.0.cmp(b.0));

        for (seg_id, entry) in ordered_segments {
...
fn column_shape(col: &ferrodruid_segment::column::ColumnData) -> (&'static str, usize) {
    ...
    match col {
        ...
        // Same distinctness rule for the decoded hyperUnique column
        // (W-A, v1.5.0).
        ColumnData::ComplexHyperUnique(v) => ("COMPLEX<hyperUnique>", v.len()),
    }
}
...
fn estimate_segment_bytes(segment: &SegmentData) -> u64 {
    ...
    for (name, col) in &segment.columns {
        ...
        let column_bytes = match col {
            ...
            // Decoded per-row hyperUnique column (W-A, v1.5.0): the Vec of
            // sketch structs plus each sketch's boxed 2048-byte register
            // page (a fixed-size heap allocation per row).
            ferrodruid_segment::column::ColumnData::ComplexHyperUnique(v) => {
                allocation_bytes::<ferrodruid_segment::column::DruidHyperUnique>(v.capacity())
                    .saturating_add(
                        u64::try_from(v.len())
                            .unwrap_or(u64::MAX)
                            .saturating_mul(2048),
...
mod tests {
    ...
    fn spill_rejects_comma_in_column_name() {
        ...
        // The writer now rejects the unrepresentable name up front (before it
        // can produce an unreadable meta.smoosh); the older round-trip re-read
        // net still applies to other non-round-tripping segments. Either
        // diagnostic satisfies the invariant: the comma segment never loads.
        assert!(
            format!("{err}").contains("round-trip")
                || format!("{err}").contains("cannot represent"),
            "error must explain the name/round-trip fidelity failure: {err}"
        );
