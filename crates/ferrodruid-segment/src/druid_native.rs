...
//!   `druid_writes_ferrodruid_reads_auto_long`).
//! * `stringDictionary`:
...
pub(crate) fn decode_native_column(data: &[u8]) -> Result<ColumnData> {
    ...
    let column = match part.part_type.as_str() {
        "longV2" => ColumnData::Long(read_long_part(data, &mut pos)?),
        "doubleV2" => {
...
/// The shared head of every numeric part: `[u32 BE part_len][u8 version=2]
/// [u32 BE num_values][u32 BE values_per_block][u8 compression]`.
...
fn read_numeric_part_head(data: &[u8], pos: &mut usize, what: &str) -> Result<NumericPartHead> {
...
/// Read a plain numeric part (`doubleV2` / `floatV2`; `longV2` goes through
/// [`read_long_part`]) and return the concatenated little-endian value bytes
/// (`num_values * width` bytes).
fn read_numeric_part(data: &[u8], pos: &mut usize, width: usize, what: &str) -> Result<Vec<u8>> {
    let head = read_numeric_part_head(data, pos, what)?;
...
/// Read a `longV2` part into `i64` values, handling both `longEncoding:
/// longs` (plain 8-byte little-endian values, the upstream default) and
/// `longEncoding: auto` (table/delta bit-packed values, marked by an
/// encoding flag on the compression byte — see the module docs).
fn read_long_part(data: &[u8], pos: &mut usize) -> Result<Vec<i64>> {
    let what = "longV2";
    let head = read_numeric_part_head(data, pos, what)?;
    let part = &data[..head.part_end];
...
// ---------------------------------------------------------------------------
// Complex (sketch) part — `thetaSketch` only (compat-8 sketch #2)
// ---------------------------------------------------------------------------

/// Decode a `complex` part.  Only `typeName: "thetaSketch"` is decodable:
/// the part is a generic-indexed container of per-row Apache DataSketches
/// compact Theta images, each decoded into a union-only Druid-origin
/// [`ferrodruid_sketches::ThetaSketch`] (see the module docs).  A null or
/// zero-length row blob decodes as an EMPTY Druid-origin sketch
/// (cardinality 0).  Every other complex type fails loudly — their
/// serialized forms are not reducible to a `(theta, retained_hashes)`
/// state, so under the migrate `--allow-unreadable-columns` gate they are
/// dropped with a manifest instead.
    ...
    if type_name != "thetaSketch" {
        return Err(DruidError::Segment(format!(
            "druid-native: unsupported complex column typeName `{type_name}` (only \
             `thetaSketch` decodes; hyperUnique/HLLSketch/quantiles sketches remain \
             unreadable)"
        )));
    }
...
mod tests {
    ...
    /// Only `thetaSketch` decodes: every other complex typeName — the
    /// DataSketches types whose serialized forms are NOT reducible to a
    /// `(theta, retained_hashes)` state — stays a loud rejection (dropped
    /// with a manifest under the migrate `--allow-unreadable-columns`
    /// gate).
    #[test]
    fn non_theta_complex_types_stay_rejected() {
        for type_name in ["hyperUnique", "HLLSketch", "quantilesDoublesSketch"] {
            let desc = format!(
