...
impl SegmentDataBuilder {
    ...
    /// stored as `DOUBLE` with NaN nulls (documented degradation: integral
    /// values render as `10.0` rather than `10`, and the column reports
    /// `DOUBLE` in metadata).  A null-free input stays a true `LONG` column.
    pub fn add_long_column_nullable(
        ...
        if values.iter().all(Option::is_some) {
            ...
            let vals: Vec<f64> = values
                .into_iter()
                .map(|v| v.map_or(crate::column::NULL_DOUBLE, |x| x as f64))
                .collect();
            self.add_double_column(name, is_metric, vals)
        }
    }
...
mod tests {
    ...
    fn builder_nullable_columns() {
        ...
        match seg.column("code_sparse") {
            ...
            other => panic!("null-bearing long column must fall back to DOUBLE, got {other:?}"),
        }
