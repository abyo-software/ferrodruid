...
use crate::smoosh::{MAX_SMOOSH_ENTRIES, SMOOSH_MAX_CHUNK_SIZE};
...
impl SmooshWriter {
    ...
    /// # Errors
    ///
    /// Fails if the archive would declare more logical files than the
    /// reader's [`MAX_SMOOSH_ENTRIES`] cap (the reader rejects such a
    /// `meta.smoosh` at reload, so a "successful" write would be durable
    /// data loss discovered only at restart — the native writer guards
    /// this in `build_native_smoosh_writer`; this is the same guard at the
    /// shared choke-point), or if the single emitted chunk would exceed
    /// the smoosh format's advertised maximum chunk size
    /// ([`SMOOSH_MAX_CHUNK_SIZE`], 2^31-1) — a larger `00000.smoosh`
    /// cannot be memory-mapped by Druid or a correct smoosh reader, and
    /// multi-chunk splitting is out of milestone scope.
    fn finish(self) -> Result<(String, Vec<Vec<u8>>)> {
        // Entry-count guard FIRST: the reader's `parse_meta` refuses any
        // archive declaring more than `MAX_SMOOSH_ENTRIES` logical files.
        // The private v9 writer emits 2 control entries + 2 per column
        // (descriptor + data), so a very wide segment (~8190+ columns)
        // used to write "successfully" and then fail
        // `num_files … exceeds cap` on the next reload.
        if self.files.len() > MAX_SMOOSH_ENTRIES {
            return Err(DruidError::Segment(format!(
                "smoosh writer: archive would declare {} logical files, exceeding the \
                 reader's limit of {MAX_SMOOSH_ENTRIES} — refusing to write a segment \
                 its own reader rejects at reload",
                self.files.len()
            )));
        }
...
/// Validate the declared column names against what the private v9 layout
/// (and its smoosh / index.drd readers) can represent.
///
/// Port of the native writer's `validate_column_names` guard
/// ([`crate::druid_native_writer`]) to the PRIVATE write path — the one
/// the production spill paths (Kafka consumer, Historical persist,
/// overlord publish) actually use.  Pre-fix, a dimension named `"a,b"`
/// wrote "successfully" but emitted the meta line `a,b,0,<start>,<end>`,
/// which `parse_meta` reads back as filename `"a"` with file_number `"b"`
/// ("bad file_number") — the whole segment unreadable at reload; a column
/// named `index.drd` / `version.bin` silently shadowed the control entry
/// (last-wins entry map).  Either way: data acked as durable, lost at
/// restart.  The charset + reserved-name check itself lives in
/// [`crate::smoosh::validate_smoosh_entry_name`], shared with the format
/// that imposes it.
fn validate_column_names(segment: &SegmentData) -> Result<()> {
    let mut seen: HashSet<&str> = HashSet::new();
    for name in segment.dimensions.iter().chain(segment.metrics.iter()) {
        // Charset + reserved-control-filename representability.
        crate::smoosh::validate_smoosh_entry_name(name)?;
        // NOTE: unlike the strict native writer, the private v9 writer does
        // NOT reject a dimension/metric literally named `__time`. The column
        // assembly above dedupes it (it is emitted once as the implicit time
        // column), so a `__time`-in-dimensions segment — a tolerated in-memory
        // shape from the Historical spill path — round-trips cleanly rather
        // than being rejected. `seen` still catches a genuine duplicate below.
        ...
        // A duplicate declaration would emit the same smoosh entry twice;
        // the reader's entry map is last-wins, so the first silently
        // vanishes.
        if !seen.insert(name.as_str()) {
            return Err(DruidError::Segment(format!(
                "duplicate column name `{name}` — a duplicate smoosh entry silently \
                 shadows the first (the meta entry map is last-wins)"
            )));
        }
    }
    Ok(())
...
fn build_smoosh_writer(segment: &SegmentData) -> Result<SmooshWriter> {
    ...
    // A declared dimension/metric whose column data is missing from the
    // `columns` map used to be silently SKIPPED here while still being
    // listed in index.drd — a "successful" write whose own reader fails
    // `smoosh: file not found` at reload (fail-open durable data loss).
    // The native writer refuses this exact shape
    // (`build_native_smoosh_writer`: "declared in index.drd but has no
    // data"); fail loud identically on the production private path.
    for dim in &segment.dimensions {
        // `__time` is always emitted first as the implicit time column; a
        // dimension list that also names `__time` (a tolerated in-memory
        // shape produced by the Historical spill path, where the schema layer
        // excludes it) must not emit a second, shadowing smoosh entry.
        if dim == "__time" {
            continue;
        }
        if !segment.columns.contains_key(dim.as_str()) {
            return Err(DruidError::Segment(format!(
                "v9 writer: dimension `{dim}` is declared in index.drd but has no \
                 column data — refusing to write a segment its own reader rejects \
                 at reload"
            )));
        }
        col_names.push(dim);
    ...
    for met in &segment.metrics {
        ...
        if !segment.columns.contains_key(met.as_str()) {
            return Err(DruidError::Segment(format!(
                "v9 writer: metric `{met}` is declared in index.drd but has no \
                 column data — refusing to write a segment its own reader rejects \
                 at reload"
            )));
        }
    ...
    for col_name in col_names {
        // Presence was verified while collecting `col_names` above; keep
        // the lookup fail-loud rather than silently skipping.
        let col = segment.columns.get(col_name).ok_or_else(|| {
            DruidError::Segment(format!(
                "v9 writer: column `{col_name}` vanished between validation and emit \
                 (internal invariant)"
            ))
        })?;
...
mod tests {
    ...
    /// `meta.smoosh` is a comma/newline-delimited text index, so a column
    /// name containing `,`, `\n`, `\r`, or `/` cannot be represented: the
    /// pre-fix writer emitted e.g. the meta line `a,b,0,<start>,<end>` for
    /// a dimension named `"a,b"`, which `parse_meta` reads back as
    /// filename `"a"` with file_number `"b"` ("bad file_number") — the
    /// whole segment is unreadable at reload, i.e. acked-durable data
    /// loss on the production spill path (Kafka consumer / Historical
    /// persist).  The write itself must fail LOUD instead, exactly like
    /// the native writer's `validate_column_names` guard.
    #[test]
    fn writer_rejects_column_names_smoosh_meta_cannot_represent() {
        for bad in ["a,b", "a\nb", "a\rb", "a/b"] {
            let segment = SegmentDataBuilder::new()
                .add_timestamp_column(vec![1000, 2000])
                .add_string_column(bad, vec!["x".to_string(), "y".to_string()])
                .build()
                .expect("build");

            let err = match write_segment_v9_to_memory(&segment) {
                Err(e) => e,
                Ok((meta, chunks)) => {
                    // Pre-fix demonstration: the "successful" write is not
                    // reopenable — surface exactly what reload would see.
                    let reopen = SmooshReader::from_parts(&meta, chunks)
                        .and_then(|s| SegmentData::from_smoosh(&s));
                    panic!(
                        "column name {bad:?} was accepted at write time; \
            ...
            assert!(
                msg.contains("smoosh") && msg.contains("cannot represent"),
                "error for {bad:?} must name the smoosh charset limit, got: {msg}"
            );

            // The on-disk writer refuses identically and leaves nothing.
            let dir = tempfile::tempdir().expect("tempdir");
            let dest = dir.path().join("seg");
            assert!(
                write_segment_v9(&segment, &dest).is_err(),
                "on-disk writer must refuse column name {bad:?}"
            );
            assert!(!dest.exists(), "no segment dir may be left behind");
        }
    }

    /// A column named after one of the segment's own control files
    /// (`version.bin`, `index.drd`, `meta.smoosh`, `metadata.drd`,
    /// `factory.json`) silently SHADOWED the control entry pre-fix (the
    /// smoosh entry map is last-wins and columns are appended after the
    /// control entries), bricking reload; the private writer must refuse
    /// it exactly like the native writer's reserved-name guard.
    #[test]
    fn writer_rejects_reserved_control_filenames_as_column_names() {
        for bad in [
            "version.bin",
            "index.drd",
            "meta.smoosh",
            ...
            let segment = SegmentDataBuilder::new()
                .add_timestamp_column(vec![1000, 2000])
                .add_long_column(bad, true, vec![7, 8])
                .build()
                .expect("build");
            ...
            let err = match write_segment_v9_to_memory(&segment) {
                Err(e) => e,
                Ok((meta, chunks)) => {
                    // Pre-fix demonstration: the shadowed control entry
                    // makes the reopen fail (or reads garbage bookkeeping).
                    let reopen = SmooshReader::from_parts(&meta, chunks)
                        .and_then(|s| SegmentData::from_smoosh(&s));
                    panic!(
                        "reserved column name {bad:?} was accepted at write \
                         time; reopen result: {reopen:?}"
                    );
                }
            };
            let msg = err.to_string();
            assert!(
                msg.contains("reserved") && msg.contains(bad),
                "error must name the reserved collision for {bad:?}, got: {msg}"
            );
        }
    }
    ...
    /// The private writer must refuse an archive declaring more logical
    /// files than the reader's `MAX_SMOOSH_ENTRIES` cap — pre-fix a
    /// ~8190+-column segment (2 control entries + descriptor&data per
    /// column) wrote "successfully" and the reader rejected it with
    /// `num_files … exceeds cap` on the next reload: durable data loss
    /// discovered only at restart.  The native writer already guards
    /// this; the guard now lives at the shared `finish` choke-point.
    #[test]
    fn finish_rejects_archive_over_smoosh_entry_cap() {
        use crate::smoosh::MAX_SMOOSH_ENTRIES;

        let mut over = SmooshWriter::new();
        for i in 0..=MAX_SMOOSH_ENTRIES {
            over.add_file(format!("f{i}"), Vec::new());
        }
        let err = over.finish().expect_err("over-cap archive must be refused");
        let msg = err.to_string();
        assert!(
            msg.contains("exceeding the reader's limit"),
            "error must name the reader cap, got: {msg}"
        );

        // Boundary: exactly at the cap still writes and reopens.
        let mut at_cap = SmooshWriter::new();
        for i in 0..MAX_SMOOSH_ENTRIES {
            at_cap.add_file(format!("f{i}"), Vec::new());
        }
        let (meta, chunks) = at_cap.finish().expect("at-cap archive must write");
        SmooshReader::from_parts(&meta, chunks).expect("at-cap archive must reopen");
    }

    /// A declared dimension/metric whose column data is missing from the
    /// `columns` map must be a LOUD write error — pre-fix the private
    /// writer silently skipped it while still listing the name in
    /// index.drd, writing a segment whose own reader fails
    /// `smoosh: file not found` at reload (the native writer already
    /// refuses this exact shape).
    #[test]
    fn writer_rejects_declared_column_without_data() {
        let mut columns = HashMap::new();
        columns.insert("__time".to_string(), ColumnData::Long(vec![10, 20]));
        ...
        let segment = SegmentData {
            ...
            dimensions: vec!["ghost".to_string()],
            metrics: vec![],
            columns,
            time_sorted: true,
        };
        ...
        let err = write_segment_v9_to_memory(&segment)
            .expect_err("declared dimension without data must be refused");
        let msg = err.to_string();
        assert!(
            msg.contains("ghost") && msg.contains("no") && msg.contains("data"),
            "error must name the dataless column, got: {msg}"
        );

        // Same for a metric, and the on-disk writer refuses identically.
        let mut columns = HashMap::new();
        columns.insert("__time".to_string(), ColumnData::Long(vec![10, 20]));
        let segment = SegmentData {
            version: 9,
            ...
            dimensions: vec![],
            metrics: vec!["phantom".to_string()],
            columns,
            time_sorted: true,
        };
        let err = write_segment_v9_to_memory(&segment)
            .expect_err("declared metric without data must be refused");
        ...
        assert!(
            write_segment_v9(&segment, &dest).is_err(),
            "on-disk writer must refuse too"
        );
        assert!(!dest.exists(), "no segment dir may be left behind");
    }
