...
fn read_segment_v9_inner(
    ...
    // A ComplexTheta column decodes its own row count from its blob;
    // nothing inside the blob ties that count to the segment's canonical
    // row count, so a truncated (or padded) sketch column used to reopen
    // fine, with the missing rows silently read as NULL sketches — a
    // cardinality-under-counting segment that even the strict
    // rewrite-reopen guard accepted.  Enforce the same row-count
    // agreement every other column family gets from its own length
    // checks, in BOTH strict and lenient mode: the mismatch is data
    // corruption, not a decode failure that lenient loading may drop.
    for (name, col) in &columns {
        if let ColumnData::ComplexTheta(rows) = col
            && rows.len() != num_rows
        {
            return Err(DruidError::Segment(format!(
                "v9 segment: complex theta column `{name}` holds {} sketch rows but \
                 the segment has {num_rows} rows",
                rows.len()
            )));
        }
    }
