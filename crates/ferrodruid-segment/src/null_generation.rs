...
pub fn classify_column(name: &str, column: &ColumnData) -> ColumnNullGeneration {
    let (handling, legacy_signature) = match column {
        ...
        // Multi-value string columns (compat-11) are written only by the
        // modern FerroDruid writer (upstream-Druid MV segments fail closed
        // at decode), and their null representation is STRUCTURAL — a
        // zero-element row — so a trailing null bitmap confirms modern
        // handling; otherwise mirror the single-value `""` signature
        // (an `""` dictionary entry actually referenced by some element).
        ColumnData::StringMulti(s) => {
            if s.null_rows().is_some() {
                (NullHandling::ConfirmedModern, false)
            } else {
                let empty_present = s.dictionary.get(0) == Some("") && s.ordinals.contains(&0);
                (NullHandling::Unconfirmed, empty_present)
            }
        }
        // A plain LONG column has no in-band null encoding (null-bearing
        // long-typed columns are stored as `LongNullable`, see
        ...
        // A `LongNullable` layout is written only by a modern (SQL-null)
        // writer — the explicit null-row bitmap IS the modern null marker.
        // A (theoretical) empty bitmap carries no null evidence, so it
        // stays unconfirmed with the same `0` legacy signature as LONG.
        ColumnData::LongNullable(v, nulls) => {
            if nulls.is_empty() {
                (NullHandling::Unconfirmed, v.contains(&0))
            } else {
                (NullHandling::ConfirmedModern, false)
            }
        }
        ...
        // Complex (sketch) columns — opaque OR decoded per-row theta —
        // carry no null convention this detector understands; stay honest
        // and report them unconfirmed, without a signature to warn about.
        ColumnData::Complex(_) | ColumnData::ComplexTheta(_) => (NullHandling::Unconfirmed, false),
    };
