...
//!   [u32 BE values_per_block][u8 0xff][generic-indexed blocks of raw LE
//!   values]`, where `values_per_block` is the canonical 64 KiB buffer over
//!   the value width (8192 for LONG/DOUBLE, 16384 for FLOAT).
...
/// Compression-buffer size, in bytes, of one numeric block — real Druid
/// sizes numeric blocks to a fixed 64 KiB buffer, so the declared
/// values-per-block is `65536 / value_width`: `0x2000` (8192) for 8-byte
/// LONG/DOUBLE parts and `0x4000` (16384) for 4-byte FLOAT parts.
/// Verified against the captured Druid 31.0.2 fixtures:
/// `druid31_added.col` / `druid31_added_d.col` / `druid31_time.col`
/// declare vpb=8192 while `druid31_delta_f.col` (FLOAT) declares
/// vpb=16384.  (An earlier revision hard-coded 8192 for every width —
/// a shape real Druid never writes for floats.)
const NUMERIC_BLOCK_BUFFER_BYTES: usize = 65_536;
...
/// [u32 BE num_values][u32 BE values_per_block][u8 0xff]
/// [generic-indexed blocks]`, where `values_per_block` is the canonical
/// 64 KiB buffer divided by the value width (8192 for LONG/DOUBLE,
/// 16384 for FLOAT — see [`NUMERIC_BLOCK_BUFFER_BYTES`]).
fn write_numeric_v2_part(raw_le: &[u8], width: usize, num_values: usize) -> Result<Vec<u8>> {
    let what = "numeric part";
    // Internal invariants of the callers below, checked rather than assumed.
    if width == 0 || !NUMERIC_BLOCK_BUFFER_BYTES.is_multiple_of(width) {
        return Err(DruidError::Segment(format!(
            "druid-native writer: {what}: value width {width} cannot tile the 64 KiB \
             numeric block buffer"
        )));
    }
    ...
    // Real Druid sizes numeric blocks to a fixed 64 KiB buffer, so the
    // per-block VALUE count depends on the width (byte-anchored per width
    // against the captured Druid 31.0.2 fixtures).
    let values_per_block = NUMERIC_BLOCK_BUFFER_BYTES / width;
    let blocks: Vec<Option<&[u8]>> = raw_le
        .chunks(NUMERIC_BLOCK_BUFFER_BYTES)
        .map(Some)
        .collect();
...
fn write_native_column(name: &str, col: &ColumnData) -> Result<Vec<u8>> {
    match col {
        ...
        ColumnData::Complex(_)
        | ColumnData::ComplexTheta(_)
        | ColumnData::ComplexHyperUnique(_) => Err(DruidError::Segment(format!(
            "druid-native writer: column `{name}` is a complex/sketch column — not \
                 writable in the native v9 layout yet (milestone scope-out; hyperUnique \
                 write-back / reverse migration is out of W-A scope)"
        ))),
    }
}
...
mod tests {
    ...
    /// The declared values-per-block must match real Druid's fixed 64 KiB
    /// compression buffer per value WIDTH: 8192 for 8-byte LONG/DOUBLE,
    /// 16384 for 4-byte FLOAT.  The repo's own captured fixtures anchor
    /// this (`druid31_delta_f.col` declares vpb=16384; `druid31_added.col`
    /// / `druid31_added_d.col` declare vpb=8192) — an earlier revision
    /// hard-coded 8192 for floats, a shape real Druid never writes.
    #[test]
    fn numeric_part_values_per_block_matches_druid_per_width() {
        // Extract the `[u32 BE values_per_block]` field of a column blob:
        // `[u32 BE json_len][json][u32 BE part_len][u8 ver][u32 BE
        // num_values][u32 BE vpb]…`.
        fn declared_vpb(blob: &[u8]) -> u32 {
            let json_len = u32::from_be_bytes([blob[0], blob[1], blob[2], blob[3]]) as usize;
            let p = 4 + json_len;
            u32::from_be_bytes([blob[p + 9], blob[p + 10], blob[p + 11], blob[p + 12]])
        }

        let long_blob = write_long_column_native(&[1, 2, 3]).expect("long");
        assert_eq!(declared_vpb(&long_blob), 8192, "LONG: 64 KiB / 8 bytes");

        let double_blob = write_double_column_native("d", &[1.0, 2.0]).expect("double");
        assert_eq!(declared_vpb(&double_blob), 8192, "DOUBLE: 64 KiB / 8 bytes");

        let float_blob = write_float_column_native("f", &[1.0_f32, 2.0]).expect("float");
        assert_eq!(declared_vpb(&float_blob), 16384, "FLOAT: 64 KiB / 4 bytes");
    }

    /// A float column spanning multiple 16384-value blocks still
    /// round-trips exactly through the Druid-verified reader (the block
    /// split boundary moved when vpb was corrected from 8192 to 16384).
    #[test]
    fn float_v2_writer_multi_block_round_trips_exact_values() {
        let values: Vec<f32> = (0..40_000).map(|i| i as f32 * 0.25 - 1000.0).collect();
        let blob = write_float_column_native("f", &values).expect("write");
        assert!(is_native_column(&blob));
        match decode_native_column(&blob).expect("reader must decode our bytes") {
            ColumnData::Float(decoded) => assert_eq!(decoded, values),
