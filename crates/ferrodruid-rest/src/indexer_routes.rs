...
///
/// ASYNC, matching Apache Druid: responds `{"task": "<id>"}` with 200 as
/// soon as the task is accepted (durable RUNNING row persisted, batch
/// execution spawned in the background) — it does NOT wait for the task
/// to finish. Clients learn the verdict by polling
/// `GET /druid/indexer/v1/task/{id}/status` (`handle_get_task_status`).
/// An error response therefore always means "nothing ran": once the id
/// is returned, the polled status is the only verdict channel.
///
