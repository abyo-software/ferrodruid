                ...
                .map(|e| {
                    ...
                    for col in output_columns {
                        ...
                        if time_col == Some(col.name.as_str()) {
                            ...
                            obj.insert(col.name.clone(), legacy_string_cell(col, v));
                        }
                    }
                ...
                .map(|row| {
                    ...
                    for col in output_columns {
                        ...
                        if let Some(v) = row.get(native_key) {
                            ...
                            obj.insert(col.name.clone(), legacy_string_cell(col, value));
...
fn sql_row_object_with_time(
    ...
    for col in output_columns {
        ...
        out.insert(col.name.clone(), legacy_string_cell(col, v));
    }
...
// W-B legacy null mode null→`""` VARCHAR rendering (`legacy_string_cell`):
// the former private helper here now lives in `ferrodruid_sql::types` (see
// the `use` at the top of this file) so the single-binary formatter and
// the role-split broker formatter
// (`ferrodruid_rpc::sql_bridge::sql_response_rows`) share ONE
// canonicalization point (W-B H1 role-split divergence fix).
