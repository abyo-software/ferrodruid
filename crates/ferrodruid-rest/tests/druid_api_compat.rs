...
// ===========================================================================
// Indexer task status endpoint compat (compat-4)
// ===========================================================================

/// `GET /druid/indexer/v1/task/{id}/status` is the endpoint every Druid
/// client polls to learn a task's terminal state. It must be ROUTED (not a
/// 404) and return Druid's `{"task": id, "status": {...}}` envelope where
/// `status.status` and `status.statusCode` both carry the state. (This
/// harness's overlord has no executor, so the submitted task parks at
/// `PENDING` — which is exactly what we assert here: the route + envelope
/// shape. The SUCCESS path is verified end-to-end against the real binary.)
#[tokio::test]
async fn get_task_status_returns_druid_status_envelope() {
    let app = setup_wikipedia_app().await;
    ...
    let spec = json!({
        ...
        "spec": {
            "dataSchema": {
                "dataSource": "status_probe",
                "timestampSpec": {"column": "t", "format": "iso"},
                "dimensionsSpec": {"dimensions": []},
                "granularitySpec": {"rollup": false}
            },
            "ioConfig": {
                "inputSource": {"type": "inline", "data": "{\"t\":\"2024-01-01T00:00:00Z\"}"},
                "inputFormat": {"type": "json"}
            }
        }
    });
    let spec_bytes = serde_json::to_vec(&spec).expect("serialize spec");
    let (submit_status, submit) = post_raw(
        app.clone(),
    ...
    assert_eq!(
        submit_status,
        StatusCode::OK,
        "task submission must be accepted: {submit}"
    );
    let task_id = submit["task"]
        .as_str()
        .expect("submit response carries a task id")
        .to_string();

    let (status, body) = get_json(
        app.clone(),
        &format!("/druid/indexer/v1/task/{task_id}/status"),
    )
    .await;
    assert_eq!(
        status,
        StatusCode::OK,
        "the /status endpoint must be routed, not a 404: {body}"
    );
    assert_eq!(
        body["task"].as_str(),
        Some(task_id.as_str()),
        "top-level `task` must echo the id: {body}"
    );
    let st = &body["status"];
    assert_eq!(st["id"].as_str(), Some(task_id.as_str()));
    // This harness has no executor, so the submitted task parks queued. Druid
    // splits lifecycle into two fields: the non-terminal top-level
    // `statusCode`/`status` is RUNNING (never an out-of-domain "PENDING"),
    // and the queue position is carried by `runnerStatusCode` (Codex R1 H5).
    assert_eq!(
        st["statusCode"].as_str(),
        Some("RUNNING"),
        "a queued task's statusCode must be the non-terminal RUNNING: {body}"
    );
    assert_eq!(
        st["status"].as_str(),
        Some("RUNNING"),
        "the deprecated `status` alias must mirror statusCode: {body}"
    );
    assert_eq!(
        st["runnerStatusCode"].as_str(),
        Some("PENDING"),
        "the queue position (PENDING) belongs in runnerStatusCode: {body}"
    );

    // An unknown task id must be a 404, not an empty 200.
    let (missing_status, _) =
        get_json(app.clone(), "/druid/indexer/v1/task/no-such-task-id/status").await;
