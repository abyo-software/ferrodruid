...
/// it, park the [`JoinHandle`] in `ops` (the Overlord's
/// `kafka_lifecycle_ops` registry), and relay its result to the caller over
/// a oneshot.
///
/// Cancellation safety (Codex R20 → R22 → R23): a lifecycle op chains
/// destructive-then-restorative steps — the earliest-replay cleanup
/// ([`drop_streaming_segments_task`], destroys rows) followed by the
/// consumer start + registration (whose replay REBUILDS them). A caller
/// cancelled at an await point (an HTTP client disconnect dropping the axum
/// handler future) must never tear those apart. R20 made the cleanup ALONE
/// uncancellable; R22 made orphaned cleanups gate the next lifecycle op;
/// R23 closed the remaining seam: with only the cleanup spawned, a caller
/// cancelled AFTER the cleanup completed but BEFORE the consumer start left
/// the rows deleted with nothing bound to replay them — a follow-up LATEST
/// create for the same (datasource, topic, bootstrap) then consumed from
/// the topic tail and the deleted rows were rebuilt by nobody (silent
/// permanent loss). Spawning the whole tail removes that boundary by
/// construction: a cancelled caller either ran nothing (cancelled before
/// the spawn — nothing destructive has happened) or the full op runs to
/// completion in the background.
///
/// The registry lock is taken BEFORE the spawn and released after the push,
/// with no await point in between, so caller cancellation cannot lose a
/// spawned op's handle. `create_supervisor`, `resume_kafka_supervisors` AND
/// `shutdown_supervisor` drain the registry right after taking the
/// lifecycle lock — shutdown must too (R23): an in-flight op may be about
/// to persist + register the very consumer the shutdown exists to stop;
/// without the drain, shutdown would report "not found" and the op would
/// then register a consumer nothing ever stops.
///
/// The result travels back on a oneshot; a receive error (the op panicked
/// or was aborted before sending) is a fail-closed `Err` naming `label` —
/// the caller must report failure. Op bodies take only the datasource
/// publish lock, metadata handles, and the `kafka_supervisors` registration
/// lock — never this registry or the lifecycle lock — so the drain cannot
/// deadlock.
pub(crate) async fn run_lifecycle_op<T, F>(
    ops: &Mutex<Vec<JoinHandle<()>>>,
    label: &str,
    op: F,
) -> Result<T>
where
    ...
    T: Send + 'static,
{
    ...
    {
        // Registry lock BEFORE the spawn: spawn→push has no await point, so
        // caller cancellation cannot lose a spawned op's handle.
...
mod tests {
    ...
    async fn repost_or_cross_type_replacement_of_running_is_refused() {
        ...
        // the running Kafka consumer) — and must not be persisted.
        let kinesis = json!({"id": "sup-x", "type": "kinesis"});
        let err2 = overlord
