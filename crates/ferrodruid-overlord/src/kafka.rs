...
/// `min_start` sentinel for a COMMITTED-PASSTHROUGH frontier entry (ship4 —
/// the last horn of the ambiguous-recreation problem): a partition whose ONLY
/// durable evidence is an ambiguous topicId-mismatch row (see
/// [`compute_resume_frontier`]) gets an entry with this `min_start`, no
/// coverage, and no `recreated` flag. In
/// [`resume_offset`](ferrodruid_ingest_kafka::streaming::resume_offset)'s
/// `min(committed, min_start)` the sentinel never pulls a real committed
/// offset down, so the authoritative seek target IS the committed offset —
/// which the seek path then validates against the live `[low, high]`
/// watermark window
/// ([`clamp_resume_target`](ferrodruid_ingest_kafka::streaming::clamp_resume_target)):
/// in-range resumes exactly from it (bounded), out-of-range FLOORS at the
/// live `low` (re-consume, never an `auto.offset.reset` skip-to-latest).
/// With NO committed offset at all, the sentinel itself is the target and is
/// guaranteed out-of-range, so the same clamp floors at `low` — the C1
/// discipline (a durable-managed partition never falls back to
/// `auto.offset.reset`) applied to the ambiguous case.
const COMMITTED_PASSTHROUGH_MIN_START: i64 = i64::MAX;
...
///   CONSERVATIVELY, to every durable row EXCEPT a definite cluster
///   mismatch and a topicId-MISMATCH row (definite or ambiguous — see the
///   topic identity section: a mismatched row's span names another offset
///   space, so even its `start` is not a safe floor). A phantom row (blob
///   missing) whose identity is unknowable must still pull the resume below
///   its hole — otherwise the committed offset (already past the span) is
///   trusted and the missing data is skipped forever (loss). Flooring an
///   actually-foreign row with NO mismatch evidence merely re-consumes from
///   an early offset (bounded at-least-once duplication, never loss).
...
///   log from `low`". The recreation CLASSIFICATION requires the row's OWN
///   `clusterId` to positively confirm the current cluster (ship2, the R24
///   H1 / R26 H1 gate applied per-row);
/// * **AMBIGUOUS topic-id mismatch** (both known, different, but the row's
///   cluster identity NOT positively confirmed — current cluster id
///   unresolved, or the row's clusterId stamp absent/malformed): a
///   bootstrap cluster REPOINT A→B and a same-cluster recreation present
///   identically and are told apart only by the cluster identity, so the
///   row never marks a recreation (topic-level detection stays gated on
///   cluster confirmation). EITHER reading places its span in a FOREIGN
///   (repoint) or DEAD (recreation) offset space, so the row contributes
///   NEITHER role (the ship2 High fix): a foreign `start` planted in
///   `min_start` becomes the resume SEEK TARGET whenever no committed
///   offset exists, and on a repoint an A-side start past B's live low
///   watermark silently skips the live records below it (loss the R5-H1
///   clamp cannot catch — the target is in-range). Beyond that exclusion
///   the mismatch is NON-ACTIONABLE for resume (the ship3 High fix): the
///   partition is NOT put on the persistent recreated/low-watermark model
///   — it resumes from its COMMITTED consumer offset, bounded (the
///   committed offset advances as records are consumed; no per-restart
///   re-consume). Pre-ship3, marking it `recreated` persistently floored
///   EVERY restart at the live `low` — and an unresolved cluster identity
///   is exactly the mode in which publishes OMIT `kafkaOffsets` (R4 H4),
///   so no `loaded` coverage ever advanced the floor: the same records
///   were re-consumed and re-published on every restart indefinitely
///   (unbounded duplication). But the committed offset must still be
///   WATERMARK-VALIDATED (the ship4 High fix): absent other durable
///   evidence the partition gets a COMMITTED-PASSTHROUGH entry
///   ([`COMMITTED_PASSTHROUGH_MIN_START`], no coverage, no `recreated`
///   flag) rather than NO entry — with no entry the consumer performed no
///   seek at all, so an OUT-OF-RANGE committed offset (exactly what an
///   unconfirmed recreation/truncation leaves behind) hit broker-side
///   OFFSET_OUT_OF_RANGE and librdkafka's `auto.offset.reset`: under
///   `latest` (useEarliestOffset=false) every retained `[low, high)`
///   record was skipped forever. Through the passthrough entry the
///   authoritative seek resolves to the committed offset and the R5-H1
///   watermark clamp bounds it: in-range → resume from it (bounded, the
///   ship3 guarantee kept); out-of-range or absent → floor at the live
///   `low` (bounded at-least-once re-consume of the retained window,
///   never a skip — and once the re-consumed records commit, the next
///   restart is in-range again, so this never becomes the per-restart
///   ship3 loop). The skip-vs-dup tradeoff stays resolved to the bounded
///   side, with one documented residual: IF the mismatch was in fact a
///   recreation and the committed offset lands IN-range in the new
///   numbering, records below it in the new numbering are not re-consumed
///   (mis-alignment, undetectable without a resolved identity). A
///   fundamental limitation of unresolvable Kafka cluster identity;
///   resolvable (PLAINTEXT / 2.8+) brokers never hit this — the mismatch
///   is cluster-confirmed there and takes the recreation path below;
...
pub(crate) fn compute_resume_frontier(
    ...
    // ship2 + ship3: a topicId mismatch on a row whose cluster identity is
    // NOT positively confirmed (current id unresolved / stamp absent or
    // malformed) is AMBIGUOUS — repoint vs recreation — but EITHER way the
    // row's span names offsets of a FOREIGN (repoint) or DEAD (recreation)
    // offset space, so it is excluded from BOTH roles (ship2). Beyond that
    // it is NON-ACTIONABLE (ship3): its partitions are NOT forced onto the
    // recreated/low-watermark model and resume from their committed
    // consumer offsets like normal partitions (see the topic gate).
    let mut ambiguous_topic_mismatch = 0usize;
    ...
    // Partitions named by AMBIGUOUS-mismatch rows (ship2/ship3 → ship4): the
    // rows contribute neither floor nor advance, but a partition with NO
    // other frontier evidence still needs a COMMITTED-PASSTHROUGH entry so
    // its committed offset is watermark-validated instead of falling to
    // librdkafka's `auto.offset.reset` (see below).
    let mut ambiguous_partitions: std::collections::BTreeSet<i32> =
        std::collections::BTreeSet::new();
    for r in rows {
        if r.data_source != data_source
        ...
        // Topic identity (Codex R7 H1 → R8 H1/H2 → R16 H2 → ship2, KIP-516):
        // FOUR-state — the id only ever BITES on positive evidence of a
        // recreation, and (ship2) only on a row whose OWN clusterId
        // positively confirms the current cluster (the R24 H1 / R26 H1
        // gate `detect_topic_recreation` already applies; an ambiguous
        // pairing contributes NEITHER role and is otherwise NON-ACTIONABLE
        // — committed-offset resume, see the mismatch arm). A
        // cluster-confirmed definite MISMATCH marks a DEAD generation: the row
        ...
        let topic_advance_ok = match (classify_payload_stamp(&r.payload, "topicId"), topic_id) {
            (PayloadStamp::Present(row_tid), Some(tid)) if row_tid != tid => {
                // ship2 (R24 H1 / R26 H1 sibling): a topicId mismatch is
                // recreation evidence ONLY on a row whose OWN clusterId
                // POSITIVELY confirms the current cluster (`cluster_advance`
                // is true exactly on a definite Present==current match) —
                // the same gate `detect_topic_recreation` applies. A
                // same-cluster recreation and a bootstrap cluster REPOINT
                // A→B both present as "row tid != current tid" and are told
                // apart only by the cluster identity: the ambiguous pairing
                // (current id unresolved, or the row's stamp
                // absent/malformed) never marks a topic-level recreation.
                // But EITHER reading places the row's span in an offset
                // space that is NOT the live partition's (foreign cluster
                // on a repoint, dead generation on a recreation), so its
                // `start` may not play the FLOOR role either (ship2 High
                // fix): planted in `min_start`, a foreign start becomes the
                // resume SEEK TARGET whenever no committed offset exists —
                // on a repoint A→B with no B-side commit, an A-side start
                // of 5 seeks partition B at 5 past B's live low watermark
                // and permanently SKIPS the live records below it (silent
                // loss, the exact skip the R5-H1 clamp cannot catch because
                // the target is IN-range). The row is excluded from BOTH
                // roles. Beyond that exclusion the ambiguous pairing is
                // NON-ACTIONABLE (ship3 High fix): it must NOT mark the
                // partition `recreated` — that flag persists across every
                // restart while the identity stays unresolved, flooring the
                // resume at the live `low` each time, and with the identity
                // unresolved publishes OMIT `kafkaOffsets` (R4 H4) so no
                // coverage ever advances the floor: the same records were
                // re-consumed and re-published on EVERY restart (unbounded
                // duplication). The partition resumes from its committed
                // consumer offset like a normal partition instead (bounded;
                // the R5-H1 watermark clamp remains the last-resort bound
                // for an out-of-range committed value — see the warn below
                // for the documented mis-alignment residual).
                if cluster_advance {
                ...
                ambiguous_topic_mismatch += 1;
                // ship4: remember the named partitions — one whose ONLY
                // durable evidence is ambiguous rows still needs a
                // committed-passthrough entry (seeded after the loop), or
                // its committed offset is never watermark-validated.
                for pk in offsets.keys() {
                    if let Ok(partition) = pk.parse::<i32>() {
                        ambiguous_partitions.insert(partition);
                    }
                }
                continue; // possibly-foreign offset space: neither floor nor advance
            }
    ...
    // Ambiguous-mismatch partitions stay OFF the recreated/low-watermark
    // model (ship3: a floor-0/recreated entry re-floored them at the live
    // `low` on every restart, and with the identity unresolved no coverage
    // could ever advance the floor — unbounded re-consumption; the
    // committed offset governs instead, bounded). But ship3 seeded NO entry
    // at all, and `apply_resume_seek` no-ops on a partition without one —
    // the committed offset was resumed from WITHOUT the `[low, high]`
    // watermark validation every other durable-managed partition gets
    // (ship4 High fix): an OUT-OF-RANGE committed offset (exactly what a
    // genuine-but-unconfirmable recreation/truncation leaves behind) hit
    // broker-side OFFSET_OUT_OF_RANGE and librdkafka's `auto.offset.reset`
    // — under `latest` (useEarliestOffset=false) every retained
    // `[low, high)` record was skipped forever, the exact skip the R5-H1
    // clamp exists to prevent (and which the ship3 warn wrongly claimed
    // still applied). Seed a COMMITTED-PASSTHROUGH entry instead
    // ([`COMMITTED_PASSTHROUGH_MIN_START`]: a floor that never pulls the
    // committed offset down, NO coverage, NOT recreation-flagged): the
    // authoritative seek resolves to the committed offset and the watermark
    // clamp bounds it — in-range resumes exactly from it (bounded, the
    // ship3/R2 guarantee), out-of-range or absent floors at the live `low`
    // (bounded at-least-once re-consume; once those records commit, the
    // next restart is in-range again — never the ship3 per-restart loop).
    let mut ambiguous_passthrough = 0usize;
    for partition in &ambiguous_partitions {
        if !min_start.contains_key(partition) {
            min_start.insert(*partition, COMMITTED_PASSTHROUGH_MIN_START);
            ambiguous_passthrough += 1;
        }
    }
    ...
    if ambiguous_topic_mismatch > 0 {
        tracing::warn!(
            data_source,
            topic,
            ambiguous_topic_mismatch,
            ambiguous_passthrough,
            current_topic_id = topic_id.unwrap_or(""),
            "resume frontier: durable rows carry a topicId in MISMATCH with the \
             current topic's id, but their CLUSTER identity could not be \
             positively confirmed (current cluster id unresolved, or the row's \
             clusterId stamp absent/malformed) — ship2/ship3/ship4, the R24 H1 \
             / R26 H1 gate applied per-row. A same-cluster recreation and a \
             bootstrap cluster repoint both present as a topicId mismatch and \
             are told apart only by the cluster identity — but EITHER way the \
             rows' spans name offsets of a FOREIGN (repoint) or DEAD \
             (recreation) offset space, so they contribute NEITHER role: \
             flooring the resume at such a start would make it the seek target \
             when no committed offset exists, skipping every live record below \
             it (silent loss — the ship2 High fix). The ambiguity is otherwise \
             NON-ACTIONABLE (ship3 High fix): the named partitions resume from \
             their COMMITTED consumer offsets — the BOUNDED default \
             (persistently low-flooring them re-consumed and re-published the \
             retained log on EVERY restart while the identity stayed \
             unresolved, since identity-less publishes carry no kafkaOffsets \
             to advance the floor — unbounded duplication). The committed \
             offset is ALWAYS watermark-validated (ship4 High fix): a \
             partition whose only evidence is ambiguous rows gets a \
             committed-passthrough frontier entry, so an out-of-range or \
             absent committed offset FLOORS at the live low watermark \
             (re-consume, bounded duplication) instead of falling to \
             librdkafka's auto.offset.reset — under latest \
             (useEarliestOffset=false) that silently skipped every retained \
             record of the live window. DOCUMENTED RESIDUAL of this \
             skip-vs-dup tradeoff: if the mismatch WAS a genuine recreation \
             and the committed offset lands in-range in the new numbering, \
             the resume may MIS-ALIGN (records below the committed offset in \
             the new numbering are not re-consumed). This is a fundamental \
             limitation of unresolvable Kafka cluster identity — resolvable \
             (KIP-78, Kafka 2.8+/PLAINTEXT) brokers never hit it. Resolve the \
             broker's cluster id to restore full disambiguation (a confirmed \
             recreation floors at the live low watermark), or delete the rows \
             explicitly if their origin is known foreign",
        );
    }
    ...
    min_start
        ...
        .map(|(partition, ms)| {
            ...
            let resume = if dead_gen_partitions.contains(&partition) {
                ...
                // restore) regardless of live evidence. Reserved for the
                // cluster-CONFIRMED recreation path ONLY (ship3): an
                // ambiguous mismatch must never set this persistent flag —
                // while the identity stays unresolved no coverage can ever
                // advance the low floor (identity-less publishes carry no
                // kafkaOffsets, R4 H4), so the flag re-consumed the retained
                // log on EVERY restart (unbounded duplication).
...
mod tests {
    ...
    /// ship3 High (unbounded-duplication regression on the ship2 fix): an
    /// UNCONFIRMED / ambiguous topicId mismatch (current cluster id
    /// unresolved, or the row's clusterId stamp absent/malformed) must be
    /// NON-ACTIONABLE for resume — the row contributes NEITHER floor NOR
    /// advance (the ship2 skip fix: a foreign `start` must never become the
    /// seek target), and the partition is NOT put on the persistent
    /// recreated/low-watermark model. It resumes from its COMMITTED consumer
    /// offset like a normal (no-mismatch) partition instead.
    ///
    /// The regression this pins: ship2 marked the ambiguous partition
    /// `recreated` PERSISTENTLY, so every restart floored it at the LIVE low
    /// watermark. But the ambiguity (cluster id unresolved) is exactly the
    /// mode in which published segments OMIT `kafkaOffsets` (the R4 H4 stamp
    /// invariant), so no `loaded` coverage ever advances the floor — the
    /// same records were re-consumed and RE-PUBLISHED on EVERY restart
    /// indefinitely (unbounded duplication). Committed-offset resume is
    /// bounded: the committed offset advances as records are consumed, so a
    /// second restart never re-consumes the first restart's records.
    #[test]
    fn ambiguous_topic_mismatch_resumes_from_committed_offset_bounded() {
        use ferrodruid_ingest_kafka::streaming::{
            PartitionResume, recreated_resume_target, resolve_seek_target, resume_offset,
        };
        use std::collections::BTreeMap;

        /// Mirror the consumer's per-partition resume decision: a frontier
        /// entry drives an authoritative (clamped) seek; an ABSENT partition
        /// is never seeked — librdkafka resumes it from the COMMITTED
        /// consumer-group offset (`auto.offset.reset` only without one).
        fn simulated_resume(
            ...
            match frontier.get(&partition) {
                Some(r) => {
                    let placeholder = resume_offset(Some(committed), r);
                    resolve_seek_target(r.recreated.then_some(r), false, placeholder, low, high).0
                }
                None => committed,
            }
        }

        // One durable row stamped with a FOREIGN topicId on p0; the current
        // cluster id is UNRESOLVED — the ambiguous (repoint-vs-recreation)
        // signature. While it stays unresolved, publishes omit kafkaOffsets,
        // so this row set never grows new durable evidence across restarts.
        let rows = vec![streaming_row_gen(
            "s0",
            "events",
            "t",
        ...
        // RESTART 1: the ambiguous row is non-actionable — p0 carries only a
        // COMMITTED-PASSTHROUGH entry (ship4), so neither the foreign start
        // 5 (the ship2 skip bug, prevented by exclusion) nor a per-restart
        // low-watermark floor (the ship3 unbounded-dup bug) can become the
        // seek target. The committed offset governs, watermark-validated.
        let f1 = compute_resume_frontier(&rows, "events", "t", None, Some("tid-B"), |_| true);
        let e0 = f1.get(&0).expect(
            "ship4: the ambiguous partition carries a committed-passthrough \
             entry (its committed offset must be watermark-validated)",
        );
        assert!(
            !e0.recreated && e0.loaded.is_empty(),
            "an UNCONFIRMED topicId mismatch must stay non-actionable: never \
             a foreign-start seed (ship2), never a persistent recreated flag \
             (ship3 unbounded dup), never advancing coverage"
        );
        assert_eq!(
            e0.min_start,
            i64::MAX,
            "the passthrough floor must never pull a real committed offset \
             down (the foreign start 5 must not become a floor)"
        );
        // Committed = 100 in live numbering; live window [0, 500).
        assert_eq!(
            simulated_resume(&f1, 0, 100, 0, 500),
            100,
            "restart 1 resumes from the committed offset, not the live low \
             watermark"
        );
        // RESTART 2: records 100..150 were consumed and committed in the
        // meantime (identical durable rows — unresolved identity means no
        // new kafkaOffsets were stamped). The resume must be the ADVANCED
        // committed offset: [0, 150) is NOT re-consumed. Pre-fix, p0 was
        // marked recreated persistently and floored at low=0 again — the
        // same records re-consumed and re-published on every restart.
        let f2 = compute_resume_frontier(&rows, "events", "t", None, Some("tid-B"), |_| true);
        assert_eq!(
            simulated_resume(&f2, 0, 150, 0, 500),
            150,
            "restart 2 must NOT re-consume the first restart's records — \
             committed-offset resume is bounded (no per-restart low-floor)"
        );
        // A MALFORMED clusterId row with the same mismatch is equally
        // ambiguous — equally non-actionable (R16 H2 consistency).
        ...
        malformed.payload["clusterId"] = json!(null);
        let fm = compute_resume_frontier(
            &[malformed],
            "events",
            "t",
            Some("kc-B"),
        ...
        let e1 = fm.get(&1).expect(
            "a malformed-clusterId mismatch row is equally ambiguous — its \
             partition gets the same committed-passthrough entry",
        );
        assert!(
            !e1.recreated && e1.loaded.is_empty() && e1.min_start == i64::MAX,
            "a malformed-clusterId mismatch row is equally ambiguous and \
             equally non-actionable (committed-passthrough only)"
        );
        // Mixed evidence: the SAME partition also carries a normal
        // live-generation row — the live row governs alone (normal frontier
        // entry, no recreated flag), the ambiguous row still contributes
        // neither role.
        let mixed = vec![
            streaming_row_gen(
                "ambig",
            ...
            streaming_row_gen(
                ...
                "events",
                "t",
                None, // unstamped (identity was unresolved) → floor-only
                None,
                "v2:fp",
                Some(&[(0, 90, 120)]),
            ),
        ];
        let fx = compute_resume_frontier(&mixed, "events", "t", None, Some("tid-B"), |_| true);
        let p0 = fx.get(&0).expect("the live row must gate p0");
        assert!(
            !p0.recreated,
            "an ambiguous mismatch must not put the partition on the \
             recreated model"
        );
        assert_eq!(
            p0.min_start, 90,
            "the live row's start floors the resume — never the foreign \
             start 5"
        );
        // Last-resort safety (R5 H1 clamp, unchanged): a committed target
        // from a genuine-but-unconfirmed recreation that lies OUTSIDE the
        // live window can never seek past the live high — it re-clamps to
        // `low` (bounded re-consume, never an unbounded-future skip).
        let placeholder = resume_offset(Some(900), p0);
        let (target, shrank) = resolve_seek_target(None, false, placeholder, 0, 800);
        assert_eq!(target, 90, "in-range floor passes the clamp untouched");
        assert!(!shrank);
        let (clamped, shrank) = resolve_seek_target(None, false, 900, 0, 800);
        assert_eq!(
            clamped, 0,
            "an out-of-window target clamps to low, never past the live high"
        );
        assert!(shrank, "the clamp reports the shrunken offset space");
        // The cluster-CONFIRMED mismatch still detects the recreation and
        // floors at the LIVE low watermark (the R7/R8/R9 path is untouched).
        let f3 =
            compute_resume_frontier(&rows, "events", "t", Some("kc-A"), Some("tid-B"), |_| true);
        let confirmed = f3.get(&0).expect("dead-gen partition floors at 0");
        assert!(
            confirmed.recreated,
            "a cluster-CONFIRMED topicId mismatch must still mark the \
             recreation"
        );
        assert_eq!(
            recreated_resume_target(3, 10, confirmed),
            3,
            "confirmed recreation floors at the LIVE low watermark"
        );
    }

    #[test]
    fn ambiguous_mismatch_out_of_range_committed_floors_at_low_not_auto_reset() {
        // ship4 High (the last horn of the ambiguous-recreation problem):
        // ship3 made an UNCONFIRMED topicId mismatch non-actionable by giving
        // the partition NO frontier entry — but `apply_resume_seek`
        // early-returns on an empty/non-recreated frontier, so the committed
        // offset was never validated against the live [low, high] watermark
        // window. An OUT-OF-RANGE committed offset — exactly what a
        // genuine-but-unconfirmable recreation/truncation leaves behind —
        // then fell through to librdkafka's `auto.offset.reset`: LATEST
        // under useEarliestOffset=false, permanently skipping the retained
        // [low, high) records. The committed offset must ALWAYS pass through
        // the watermark clamp: the derivation seeds a COMMITTED-PASSTHROUGH
        // frontier entry so the authoritative seek path validates it.
        use ferrodruid_ingest_kafka::streaming::{resolve_seek_target, resume_offset};
        ...
        let f = compute_resume_frontier(&rows, "events", "t", None, Some("tid-B"), |_| true);
        // RED core: absent entry == no seek == no clamp == auto.offset.reset.
        let p0 = f.get(&0).expect(
            "ship4: an ambiguous-mismatch partition must carry a \
             committed-passthrough frontier entry — with NO entry the \
             consumer performs no seek, so an out-of-range committed offset \
             falls to auto.offset.reset (latest = permanent skip of the \
             retained window)",
        );
        assert!(
            !p0.recreated,
            "the passthrough entry must NOT revive the persistent recreated \
             flag (the ship3 unbounded-dup regression)"
        );
        assert!(
            p0.loaded.is_empty(),
            "an ambiguous row's span may never advance the resume"
        );
        // (a) OUT-OF-RANGE committed offset (500 vs live window [0, 100)):
        // the passthrough target IS the committed offset, and the seek-time
        // watermark clamp FLOORS it at low = 0 — the retained [0, 100)
        // records are re-consumed, never latest-skipped.
        let placeholder = resume_offset(Some(500), p0);
        let (target, shrank) =
            resolve_seek_target(p0.recreated.then_some(p0), false, placeholder, 0, 100);
        assert_eq!(
            target, 0,
            "committed=500 outside the live window [0,100) must floor at low \
             (re-consume the retained records), never fall through to \
             auto.offset.reset"
        );
        assert!(
            shrank,
            "the shrunken offset space is reported so the session remembers \
             the reset (R27 H1)"
        );
        // (b) IN-RANGE committed offsets still resume from the committed
        // position across restarts (the ship3/R2 boundedness): 100 → 100,
        // then after more records were consumed+committed, 150 → 150 — no
        // per-restart low-floor, no unbounded re-consume.
        for committed in [100, 150] {
            let f = compute_resume_frontier(&rows, "events", "t", None, Some("tid-B"), |_| true);
            let p = f.get(&0).expect("passthrough entry");
            let placeholder = resume_offset(Some(committed), p);
            let (target, shrank) =
                resolve_seek_target(p.recreated.then_some(p), false, placeholder, 0, 500);
            assert_eq!(
                target, committed,
                "an in-range committed offset resumes exactly from the \
                 committed position (bounded — no re-consume)"
            );
            assert!(!shrank);
        }
        // (c) NO committed offset (never landed / group expired): a
        // durable-managed partition must not fall to auto.offset.reset
        // either (the C1 discipline) — the passthrough floors at low.
        let placeholder = resume_offset(None, p0);
        let (target, _) =
            resolve_seek_target(p0.recreated.then_some(p0), false, placeholder, 30, 200);
        assert_eq!(
            target, 30,
            "with no committed offset the passthrough floors at the live low \
             watermark instead of falling to auto.offset.reset"
        );
        // (d) The CONFIRMED-recreation path is untouched: a cluster-confirmed
        // mismatch still floors at the live low via the recreated model.
        let fc =
            compute_resume_frontier(&rows, "events", "t", Some("kc-A"), Some("tid-B"), |_| true);
        let confirmed = fc.get(&0).expect("dead-gen partition entry");
        assert!(confirmed.recreated);
        let placeholder = resume_offset(Some(500), confirmed);
        let (target, _) = resolve_seek_target(
            confirmed.recreated.then_some(confirmed),
            false,
            placeholder,
            20,
            100,
        ...
        assert_eq!(
            target, 20,
            "confirmed recreation still floors at the live low watermark"
        );
        // (e) A partition with REAL frontier evidence alongside the
        // ambiguous row keeps its normal entry — the passthrough only seeds
        // partitions that would otherwise be absent.
        let mixed = vec![
            streaming_row_gen(
                "ambig",
                "events",
        ...
        let fx = compute_resume_frontier(&mixed, "events", "t", None, Some("tid-B"), |_| true);
        let p0 = fx.get(&0).expect("live row gates p0");
        assert_eq!(
            p0.min_start, 90,
            "real durable evidence governs — the passthrough sentinel never \
             overwrites a live floor"
        );
    }
