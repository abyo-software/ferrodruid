...
/// ONLY when a RESUME position (`At`/`AfterSequenceNumber`) is
/// POSITIVELY rejected as invalid by the service
/// ([`KinesisSourceError::InvalidArgument`] — on real Kinesis a
/// retention-trimmed / out-of-range sequence raises
/// `InvalidArgumentException`): re-consuming the retained log is
/// bounded at-least-once duplication, never loss, whereas refusing to
/// seek would stall the shard forever. EVERY other error — transport,
/// throttle, and crucially a generic/transient `Api` failure (e.g. an
/// InternalFailureException 5xx surfacing after SDK retries) — is NOT
/// downgraded: the caller retries the SAME position later (ship2
/// Medium: downgrading a transient Api error silently re-consumed the
/// shard's whole retention window into append-only segments —
/// permanent double-counting where a plain retry was correct).
async fn get_iterator_with_trim_fallback(
    ...
    match source.get_shard_iterator(stream, shard, position).await {
        Ok(it) => Ok(it),
        Err(KinesisSourceError::InvalidArgument(detail))
            if matches!(
                position,
            ...
            if cursors[i].iterator.is_none() {
                ...
                .await
                {
                    ...
                    Err(KinesisSourceError::StreamNotFound(detail)) => {
                        // FATAL, mirroring the GetRecords arm (ship2 Low):
                        // retrying "transiently" would let the loop resume
                        // against a RECREATED stream while the sink still
                        // stamps the identity resolved at startup — the next
                        // restart would mark those rows dead-generation and
                        // re-consume the whole retention window (avoidable
                        // double-ingestion). Stop; a restart re-resolves the
                        // identity cleanly.
                        tracing::error!(
                            stream, error = %detail,
                            "kinesis consumer: the stream disappeared while fetching a \
                             shard iterator (deleted or recreated) — stopping; a restart \
                             re-resolves the stream identity and, on a detected \
                             recreate, re-consumes from TRIM_HORIZON",
                        );
                        stats.fatal_consume_error = true;
                        break 'main;
                    }
            ...
            match out_result {
                Ok(out) => {
                    ...
                    for record in &out.records {
                        ...
                        // Multi-object records — a producer batching
                        // several newline-separated / concatenated JSON
                        // objects into one record, the Druid `json`
                        // inputFormat shape — decode to ALL their rows
                        // (ship2 H7); only records the shared buffer's
                        // own parse would refuse are dead-lettered here.
                        let decoded =
                            decode_batch(cursors[i].shard.as_str(), std::slice::from_ref(record));
                        let mut should_roll = false;
                        if decoded.rows.is_empty() {
                            // Deliberately dead-lettered (already
                            // warn-logged by decode_batch); the frontier
                            // still advances past it below — Druid's
                            // commit-past-unparseable behavior.
                        } else {
                            // Single source of ingestion validation: the
                            // shared buffer re-parses (every object of a
                            // multi-object record) and enforces timestamp
                            // presence, depth/size bounds, and the
                            // exact-long storage guard, exactly as the
                            // kafka path.
                            match buffer.push_payload(&record.data) {
                                Ok(roll) => should_roll = roll,
                                Err(e) => tracing::warn!(
                                    stream,
                        ...
                        // Sequence bookkeeping AFTER the push/dead-letter
                        // decision (ship2 H7): the durable frontier may
                        // only advance past a record once its payload(s)
                        // are actually in the buffer OR it was
                        // DELIBERATELY dead-lettered (Druid commits past
                        // unparseable records too). Noting it BEFORE the
                        // decision turned any gate-vs-buffer mismatch
                        // into a published checkpoint covering rows that
                        // were never ingested — permanent silent loss on
                        // the next restart.
...
mod tests {
    ...
    /// Ship2 H7: a single Kinesis record carrying MULTIPLE newline-
    /// separated JSON objects — a producer batching pattern Druid's
    /// JsonReader ingests and FerroDruid's own Kafka `push_payload`
    /// ingests — must round-trip EVERY object. Pre-fix, the single-object
    /// decode pre-filter dead-lettered the whole record BEFORE
    /// `push_payload` while `note_consumed` had already advanced the
    /// frontier past it, so both rows were PERMANENTLY lost (the durable
    /// checkpoint covered a record never ingested; no restart would ever
    /// re-consume it). Phase 2 restarts over the same durable state to
    /// prove the checkpoint stays honest: no loss AND no duplication.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn multi_object_record_round_trips_every_object() {
        let (metadata, hist1, _dir1) = setup().await;
        let ds_dir = tempfile::tempdir().expect("deep-storage dir");
        let deep: Arc<dyn DeepStorage> =
            Arc::new(LocalDeepStorage::new(ds_dir.path().to_path_buf()));
        ...
        mock.push_record(
            ...
            "pk",
            "100",
            row(0, "a").to_string().into_bytes(),
            None,
        );
        // ONE record, TWO newline-separated JSON objects (producer batch).
        let two_objects = format!("{}\n{}", row(1, "b"), row(2, "c"));
        mock.push_record("s1", "pk", "200", two_objects.into_bytes(), None);

        // Phase 1: all THREE rows must become query-visible.
        let mut handle = start_kinesis_consumer_with_params(
            Box::new(mock.clone()),
            loop_params("kin-multi", "st", "kin_multi"),
            Arc::clone(&metadata),
            Arc::clone(&hist1),
            Some(Arc::clone(&deep)),
        );
        wait_for_count(&hist1, "kin_multi", 3).await;
        handle.shutdown().await.expect("clean phase-1 stop");
        assert!(!handle.replay_required());

        // Phase 2 (restart): the durable frontier must cover the
        // multi-object record exactly once — the retained log is NOT
        // re-consumed (no duplication) and a new record still arrives.
        let cache2 = tempfile::tempdir().expect("cache2");
        let hist2 = Arc::new(Historical::new(cache2.path().to_path_buf(), 10_000_000));
        let ovl2 = crate::Overlord::with_executor(Arc::clone(&metadata), Arc::clone(&hist2))
            .with_deep_storage(Arc::clone(&deep));
        ovl2.bootstrap_reload_segments().await.expect("bootstrap");
        assert_eq!(queried_row_count(&hist2, "kin_multi"), 3);

        mock.push_record(
            "s1",
            "pk",
            "300",
            row(3, "d").to_string().into_bytes(),
            None,
        );
        let mut handle2 = start_kinesis_consumer_with_params(
            Box::new(mock.clone()),
        ...
        wait_for_count(&hist2, "kin_multi", 4).await;
        // Settle several flush intervals: the count must STAY 4 — any
        // re-consumption of the multi-object record would push it to 6.
        tokio::time::sleep(Duration::from_millis(400)).await;
        assert_eq!(
            queried_row_count(&hist2, "kin_multi"),
            4,
            "zero loss (both batched objects + the new record) and no \
             duplication (the multi-object record is checkpointed exactly \
             once)"
        );
        handle2.shutdown().await.expect("clean phase-2 stop");
    }
    ...
    async fn trimmed_resume_position_falls_back_to_trim_horizon() {
        ...
        // A valid shard with a resume position succeeds directly (no
        // fallback involved)...
        ...
        .await;
        assert!(ok.is_ok());
        // ...while a generic Api rejection (the mock's unknown-shard
        // error) PROPAGATES — only the specific invalid-position
        // rejection may reset to TRIM_HORIZON (ship2 Medium).
        ...
        .await;
        assert!(err.is_err(), "an unknown shard's Api error still fails");
    }

    /// ship2 Medium: a TRANSIENT service failure (e.g. an
    /// InternalFailureException 5xx surfacing as a generic `Api` error
    /// after SDK retries) on a RESUME-position `GetShardIterator` must
    /// PROPAGATE so the caller retries the SAME position after backoff.
    /// Silently downgrading it to TRIM_HORIZON re-consumes the shard's
    /// whole retention window into append-only segments — permanent
    /// double-counting triggered by a transient error.
    #[tokio::test]
    async fn transient_api_error_on_resume_position_propagates_not_trim_horizon() {
        let mock = MockKinesisSource::new("st");
        mock.push_record("s1", "pk", "100", br#"{}"#.to_vec(), None);
        mock.push_iterator_error(KinesisSourceError::Api(
            "InternalFailureException: transient 5xx after SDK retries".to_owned(),
        ));
        let res = get_iterator_with_trim_fallback(
            &mock,
            "st",
            &ShardId::new("s1"),
            &StartPosition::AfterSequenceNumber(seq("50")),
        ...
        .await;
        assert!(
            matches!(res, Err(KinesisSourceError::Api(_))),
            "a transient Api error on a resume seek must propagate (retry the \
             same position), NEVER silently reset to TRIM_HORIZON: {res:?}"
        );
    }

    /// The genuine invalid-position rejection (real Kinesis raises
    /// `InvalidArgumentException` for a retention-trimmed / out-of-range
    /// StartingSequenceNumber) DOES fall back to TRIM_HORIZON — bounded
    /// at-least-once re-consumption, never a stalled shard.
    #[tokio::test]
    async fn invalid_position_rejection_falls_back_to_trim_horizon() {
        let mock = MockKinesisSource::new("st");
        mock.push_record("s1", "pk", "100", br#"{}"#.to_vec(), None);
        mock.push_iterator_error(KinesisSourceError::InvalidArgument(
            "StartingSequenceNumber is out of the shard's retention window".to_owned(),
        ));
        let it = get_iterator_with_trim_fallback(
            &mock,
            "st",
            &ShardId::new("s1"),
            &StartPosition::AfterSequenceNumber(seq("500")),
        )
        .await
        .expect("invalid-position rejection must fall back to TRIM_HORIZON");
        let out = mock.get_records(&it).await.expect("records");
        assert_eq!(
            out.records.len(),
            1,
            "the fallback iterator must re-read the retained log from TRIM_HORIZON"
        );
        assert_eq!(out.records[0].sequence_number, "100");
    }

    /// ship2 Low: `StreamNotFound` while (re-)fetching a shard iterator
    /// is FATAL, exactly like the `GetRecords` arm — retrying it
    /// non-fatally would let the loop resume against a RECREATED stream
    /// while still stamping the OLD generation's identity (the durable
    /// rows would be marked dead-generation on the next restart and the
    /// whole retention window re-consumed: avoidable double-ingestion).
    #[tokio::test]
    async fn stream_not_found_on_iterator_fetch_is_fatal() {
        let (metadata, historical, _dir) = setup().await;
        let mock = MockKinesisSource::new("st");
        mock.push_record("s1", "pk", "100", row(0, "a").to_string(), None);
        mock.push_iterator_error(KinesisSourceError::StreamNotFound(
            "stream st was deleted".to_owned(),
        ));
        let (_tx, rx) = mpsc::channel::<()>(1);
        let stats = tokio::time::timeout(
            Duration::from_secs(5),
            run_kinesis_streaming_loop(
                Box::new(mock),
                loop_params("sup-nf", "st", "kin_nf"),
                metadata,
                historical,
                None,
                rx,
        ...
        .await
        .expect("the loop must stop ON ITS OWN when the stream disappears");
        assert!(
            stats.fatal_consume_error,
            "StreamNotFound on an iterator fetch must be a fatal stop"
        );
    }
}
