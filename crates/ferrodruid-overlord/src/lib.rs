...
impl Overlord {
    ...
    pub async fn create_supervisor(&self, spec: serde_json::Value) -> Result<String> {
        ...
        // Non-runnable paths (non-Kafka / no Historical / any spec in the
        // default build): nothing destructive happens and no consumer can be
        // running, so the persist stays in the caller's context.
    ...
    pub async fn shutdown_supervisor(&self, id: &str) -> Result<()> {
        ...
        #[cfg(feature = "kafka-io")]
        {
            ...
            return kafka::run_lifecycle_op(
                &self.kafka_lifecycle_ops,
                "the supervisor-shutdown lifecycle op",
                async move {
                    {
                        let mut sups = supervisors.lock().await;
...
mod tests {
    ...
    async fn create_get_shutdown_supervisor() {
        let (_store, overlord) = setup().await;
        // Generic supervisor CRUD test — uses a non-kafka type so the
        // kafka-io feature's spec validation / consumer spawn does not apply
        // (real Kafka supervisor lifecycle is covered by the `kafka::` tests).
        let spec = json!({
            "id": "wiki-kafka",
            "type": "kinesis",
    ...
    async fn supervisor_auto_id() {
        let (_store, overlord) = setup().await;
        // Non-kafka type so the synthetic-id fallback is exercised under
        // both feature sets (a kafka spec without id derives its id from
        // dataSource — covered by `kafka::id_less_kafka_spec_derives_...`).
        let spec = json!({
            "type": "kinesis",
