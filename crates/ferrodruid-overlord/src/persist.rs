...
///
/// This is the content-identity witness recorded at persist time
...
/// committed.
///
/// Determinism (an identical value on both the persist and the reload side):
...
pub(crate) fn blob_content_hash(dir: &Path) -> Result<String> {
    let mut files: Vec<(Vec<u8>, std::path::PathBuf)> = Vec::new();
    let read_dir = std::fs::read_dir(dir)
        .map_err(|e| DruidError::Segment(format!("hash: read blob dir {}: {e}", dir.display())))?;
    for entry in read_dir {
        let entry = entry.map_err(|e| {
            DruidError::Segment(format!("hash: read entry in {}: {e}", dir.display()))
        })?;
        let file_type = entry
            .file_type()
            .map_err(|e| DruidError::Segment(format!("hash: file type: {e}")))?;
    ...
    for (name, path) in &files {
        let bytes = std::fs::read(path)
            .map_err(|e| DruidError::Segment(format!("hash: read {}: {e}", path.display())))?;
