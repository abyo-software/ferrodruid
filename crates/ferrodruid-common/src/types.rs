...
impl<'de> Deserialize<'de> for DimensionSpec {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        //
        // compat-11 R3: `rename_all_fields = "camelCase"` is REQUIRED here
        // for the same reason as on the outer enum (`tag = "type"`
        // suppresses `rename_all`'s field effect) — without it the wire's
        // canonical `outputName` / `outputType` were silently IGNORED
        // (unknown fields), so `"outputType": "LONG"` silently degraded to
        // STRING, and `extractionFn` / `isWhitelist` failed to parse at
        // all.  The snake_case aliases keep old fixtures loading.
        #[derive(Deserialize)]
        #[serde(
            tag = "type",
            rename_all = "camelCase",
...
mod tests {
    ...
    /// compat-11 R3 regression: the wire's canonical camelCase
    /// `outputName` / `outputType` / `extractionFn` / `isWhitelist` MUST
    /// deserialize with their VALUES honoured.  Pre-fix the internal
    /// `Tagged` helper lacked `rename_all_fields`, so `outputName` /
    /// `outputType` were silently ignored (every Druid-wire
    /// `"outputType": "LONG"` degraded to STRING) and `extractionFn` /
    /// `isWhitelisted` specs failed to parse entirely.
    #[test]
    fn dimension_spec_deserializes_camelcase_field_values() {
        let parsed: DimensionSpec = serde_json::from_str(
            r#"{"type":"default","dimension":"x","outputName":"y","outputType":"LONG"}"#,
        )
        .expect("camelCase default");
        assert_eq!(
            parsed,
            DimensionSpec::Default {
                dimension: "x".into(),
            ...
            "camelCase outputName/outputType values must be honoured"
        );

        let parsed: DimensionSpec = serde_json::from_str(
            r#"{"type":"extraction","dimension":"x","outputName":"len",
                "extractionFn":{"type":"strlen"}}"#,
        )
        .expect("camelCase extraction");
        assert_eq!(
            parsed,
            DimensionSpec::Extraction {
                dimension: "x".into(),
        ...
        let parsed: DimensionSpec = serde_json::from_str(
            r#"{"type":"listFiltered",
                "delegate":{"type":"default","dimension":"x"},
                "values":["a"],"isWhitelist":false}"#,
        )
        .expect("camelCase listFiltered");
        match parsed {
            DimensionSpec::ListFiltered { is_whitelist, .. } => {
                assert!(!is_whitelist, "isWhitelist value must be honoured");
            }
