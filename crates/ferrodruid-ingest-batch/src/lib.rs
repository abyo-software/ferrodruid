...
/// Largest magnitude exactly representable in `f64` (2^53). A `long` dimension
/// column that mixes NULLs (stored via the NaN-marker DOUBLE fallback) with a
/// value beyond this range cannot be stored exactly — see [`build_long_column`]
/// and [`long_dim_value_class`].
...
/// `Some(false)` when in range. A segment whose long column mixes a NULL with
/// an out-of-range value cannot be stored exactly ([`build_long_column`] fails
/// closed), so streaming uses this to dead-letter the CONFLICTING record rather
/// than losing the whole segment (Codex R14).
#[must_use]
