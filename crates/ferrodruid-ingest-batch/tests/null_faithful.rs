...
/// null-bearing batch stores `LongNullable` — exact `i64` values plus an
/// explicit null-row bitmap (`i64` has no in-band NULL marker; pre-2026-07
/// this degraded to a NaN-null DOUBLE).
#[test]
fn typed_long_dimension_null_handling() {
    ...
    // Null-bearing: LongNullable with an exact value vector (0 at the NULL
    // row) and the null row recorded in the bitmap.
    let ingester = BatchIngester::with_schemas("gt".into(), "__time".into(), schemas, vec![]);
    let rows = vec![
        serde_json::json!({"__time": 1000, "code": 7}),
    ...
    match seg.column("code") {
        ...
        other => panic!("null-bearing long dim must be LongNullable, got {other:?}"),
    }
}

/// compat-10 core claim: a batch mixing a NULL with a value beyond ±2^53
/// ingests to `LongNullable` with EXACT i64 values — no Double, no
/// precision loss, no refusal.
#[test]
fn nullable_long_ingest_is_i64_exact() {
    let schemas = vec![DimensionSchema::new("code", DimensionType::Long)];
    let ingester = BatchIngester::with_schemas("gt".into(), "__time".into(), schemas, vec![]);
    let rows = vec![
        serde_json::json!({"__time": 1000, "code": 7}),
        serde_json::json!({"__time": 2000, "code": null}),
        serde_json::json!({"__time": 3000, "code": 9_007_199_254_740_993_i64}), // 2^53 + 1
    ];
    let seg = ingester.ingest(rows).expect("ingest").segment_data;
    match seg.column("code") {
        Some(ColumnData::LongNullable(v, nulls)) => {
...
/// The real Scan read path over a `LongNullable` column must render NULL
/// rows as JSON null and non-null rows as EXACT i64 JSON numbers — the
/// 2^53+1 value must arrive as `9007199254740993`, not a rounded f64 —
/// with the query crate's generic row renderer (no fast path).
#[test]
fn scan_renders_nullable_long_null_and_exact() {
    let schemas = vec![DimensionSchema::new("code", DimensionType::Long)];
    let ingester = BatchIngester::with_schemas("gt".into(), "__time".into(), schemas, vec![]);
    let rows = vec![
        serde_json::json!({"__time": 1000, "code": 7}),
    ...
    assert_eq!(events[0]["code"], serde_json::json!(7));
    assert_eq!(
        events[1]["code"],
        serde_json::Value::Null,
        "NULL long row must scan as JSON null"
    );
    assert_eq!(
        events[2]["code"],
        serde_json::json!(9_007_199_254_740_993_i64),
        "beyond-2^53 long must scan as the EXACT i64 (not a rounded double)"
    );
    // The exactness assertion above is meaningful only if the JSON number is
    // integral: pin it.
    assert_eq!(events[2]["code"].as_i64(), Some(9_007_199_254_740_993));
}
...
/// as NULL (recorded in the `LongNullable` null bitmap), never a corrupted
...
fn out_of_range_long_input_stores_null_not_wrapped() {
    ...
    match seg.column("code") {
        Some(ColumnData::LongNullable(v, nulls)) => {
            ...
            assert!(
                nulls.contains(1),
                "u64 above i64::MAX must store NULL (wrap corruption otherwise)"
            );
            assert!(nulls.contains(2), "float outside i64 range must store NULL");
...
/// A batch that combines NULLs with values OUTSIDE ±2^53 now STORES EXACTLY
/// as `LongNullable` (exact i64 values + null bitmap) — no precision loss,
/// no refusal.  (History: codex-review r6, 2026-07-11 — while the null
/// fallback was a NaN-Double, this mix had to FAIL CLOSED because f64
/// rounding would corrupt adjacent IDs, e.g. 9007199254740993 → ...992.
/// The `LongNullable` layout removes the degrade, so the refusal is gone.)
/// Null-free big-value batches keep the plain LONG column.
#[test]
fn long_nulls_with_beyond_2p53_values_store_exactly() {
    let mk = || {
    ...
    // NULL + beyond-2^53 value: stored EXACTLY, null row in the bitmap.
    let rows = vec![
        serde_json::json!({"__time": 1000, "id": 9_007_199_254_740_993_i64}),
        serde_json::json!({"__time": 2000, "id": null}),
    ];
    let seg = mk().ingest(rows).expect("must store exactly").segment_data;
    match seg.column("id") {
        Some(ColumnData::LongNullable(v, nulls)) => {
            assert_eq!(
                v,
                &[9_007_199_254_740_993, 0],
                "2^53+1 must be stored exactly (NOT rounded to ...992)"
            );
            assert_eq!(nulls.len(), 1);
            assert!(nulls.contains(1), "row 1 must be NULL");
        }
        other => panic!("null + big-value long must be LongNullable, got {other:?}"),
    }
    ...
    // NULL + in-range values: LongNullable too (uniform layout).
    let rows = vec![
    ...
    match seg.column("id") {
        ...
        other => panic!("in-range null-bearing long must be LongNullable, got {other:?}"),
    }
}
