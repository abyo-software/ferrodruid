...
impl Aggregator for LongSumAggregator {
    ...
    fn get(&self) -> serde_json::Value {
        if !self.seen {
            // W-B legacy null mode: an empty SUM is the 0 identity, not
            // SQL null (oracle empty_sum_y.json: Druid 27 legacy answers 0).
...
impl Aggregator for DoubleSumAggregator {
    ...
    fn get(&self) -> serde_json::Value {
        if !self.seen {
            // W-B legacy null mode: empty double SUM = 0.0 (oracle
            // ext_empty_sum_d.json).
            if ferrodruid_common::legacy_null_mode() {
                return serde_json::to_value(0.0).unwrap_or(serde_json::Value::Null);
            }
            return serde_json::Value::Null;
        }
...
impl Aggregator for FloatSumAggregator {
    ...
    fn get(&self) -> serde_json::Value {
        if !self.seen {
            // W-B legacy null mode: empty float SUM = 0.0 (same oracle
            // family as the double form).
            if ferrodruid_common::legacy_null_mode() {
                return serde_json::to_value(0.0).unwrap_or(serde_json::Value::Null);
            }
            return serde_json::Value::Null;
