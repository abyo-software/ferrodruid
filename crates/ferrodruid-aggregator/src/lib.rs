...
pub use sketch::{
    DRUID_HYPER_UNIQUE_TAG, HLL_SKETCH_TAG, HllSketchAggregator, HyperUniqueAggregator,
    QUANTILES_SKETCH_TAG, QuantilesSketchAggregator, SKETCH_MIX_ERROR_TAG, THETA_SKETCH_TAG,
    ThetaSketchAggregator, druid_hyper_unique_envelope, finalize_sketch_json, merge_sketch_json,
...
/// A numeric min/max merge operand, classified so LONG partials keep
/// their exact integer identity across the broker merge (H2) — used
/// **only under the W-B legacy latch**: with the flag off the merge
/// keeps the historical `as_f64` comparison byte-identically (ANSI
/// byte-identity is W-B's absolute invariant, and exact comparison
/// CHANGES the answer for integer neighbours beyond 2^53).
///
/// Comparing through `as_f64` collapses i64 neighbours beyond 2^53, so
/// an EMPTY legacy `longMin` partial (the `i64::MAX` sentinel identity)
/// would tie with a real `i64::MAX - 1` and win by SHARD ORDER (and the
/// `longMax` analogue near `i64::MIN`).  Integers therefore compare as
/// integers; only float operands fall back to the f64 comparison, which
/// is exact for double/float partials.
#[derive(Debug, Clone, Copy)]
enum MinMaxOperand {
    /// An i64-representable integer.
    Int(i64),
    /// An integer beyond `i64::MAX` (u64-only wire shape).
    UInt(u64),
    /// A floating-point number (`serde_json` numbers are never NaN).
    Float(f64),
}

impl MinMaxOperand {
    /// Classify a JSON partial; `None` for every non-numeric shape
    /// (JSON null = empty ANSI partial, the legacy `"±Infinity"` double
    /// sentinels, malformed input) — callers treat those exactly like
    /// the pre-H2 non-numeric arms.
    fn classify(v: &serde_json::Value) -> Option<Self> {
        if let Some(i) = v.as_i64() {
            return Some(Self::Int(i));
        }
        if let Some(u) = v.as_u64() {
            return Some(Self::UInt(u));
        }
        v.as_f64().map(Self::Float)
    }

    /// Total order over operands.  Integer pairs compare exactly; a
    /// u64-only value is strictly greater than any i64; mixed
    /// integer/float pairs keep the pre-H2 f64 comparison.
    fn cmp_exact(self, other: Self) -> std::cmp::Ordering {
        match (self, other) {
            (Self::Int(a), Self::Int(b)) => a.cmp(&b),
            (Self::UInt(a), Self::UInt(b)) => a.cmp(&b),
            (Self::Int(_), Self::UInt(_)) => std::cmp::Ordering::Less,
            (Self::UInt(_), Self::Int(_)) => std::cmp::Ordering::Greater,
            (a, b) => a
                .to_f64()
    ...
    /// f64 view for the mixed integer/float fallback comparison.
...
/// Broker JSON `min` merge.
///
/// * **ANSI (legacy latch off)** — byte-identical to the historical
///   pre-W-B implementation: both sides compare through `as_f64`, so
///   integer neighbours beyond 2^53 TIE and `dst` wins by shard order.
///   W-B's absolute invariant is that flag-off behavior never changes,
///   even where the f64 comparison is imprecise, so the exact
///   comparator must not leak here (it would return a DIFFERENT value
///   for `>2^53` adjacents).
/// * **Legacy latch on** — exact-integer comparison via
///   [`MinMaxOperand`], so the empty-partial `i64::MAX` sentinel can
///   never f64-tie with (and beat, by shard order) a real
///   near-boundary value.
fn merge_min(dst: &serde_json::Value, src: &serde_json::Value) -> serde_json::Value {
    if ferrodruid_common::legacy_null_mode() {
        match (MinMaxOperand::classify(dst), MinMaxOperand::classify(src)) {
            (Some(d), Some(s)) => {
                if s.cmp_exact(d) == std::cmp::Ordering::Less {
                    src.clone()
                } else {
                    dst.clone()
                }
...
/// Broker JSON `max` merge — same latch split as [`merge_min`]: the
/// historical `as_f64` comparison with the flag off (ANSI
/// byte-identity), the exact [`MinMaxOperand`] comparison under the
/// legacy latch (sentinel identity near `i64::MIN`).
fn merge_max(dst: &serde_json::Value, src: &serde_json::Value) -> serde_json::Value {
    if ferrodruid_common::legacy_null_mode() {
        match (MinMaxOperand::classify(dst), MinMaxOperand::classify(src)) {
            (Some(d), Some(s)) => {
                if s.cmp_exact(d) == std::cmp::Ordering::Greater {
                    src.clone()
                } else {
                    dst.clone()
                }
...
pub enum AggregatorSpec {
    ...
    /// `hyperUnique` aggregator (W-A, v1.5.0) — fold DECODED Druid
    /// HyperLogLog register states read from a migrated
    /// `COMPLEX<hyperUnique>` column.  Strictly MERGE-mode: FerroDruid
    /// cannot hash raw values into Druid's register space (clean-room —
    /// the hash function is unknown), so a `hyperUnique` aggregation over
    /// a non-hyperUnique column poisons loudly (a
    /// [`sketch::SKETCH_MIX_ERROR_TAG`] envelope, never a silent wrong
    /// estimate).
    HyperUnique {
        /// Output name for this aggregation.
        name: String,
        /// Column carrying decoded Druid hyperUnique sketches.
        #[serde(rename = "fieldName")]
        field_name: String,
        /// Druid's `isInputHyperUnique` flag — accepted on the wire for
        /// spec compatibility; FerroDruid's implementation is merge-only
        /// either way, so the flag changes nothing here.
        #[serde(
            default,
            rename = "isInputHyperUnique",
            skip_serializing_if = "Option::is_none"
        )]
        is_input_hyper_unique: Option<bool>,
        /// Druid's `round` flag: `true` finalizes to the nearest long
        /// (Java `Math.round` semantics) instead of the raw estimator
        /// double.
        #[serde(default, skip_serializing_if = "Option::is_none")]
        round: Option<bool>,
        /// Per-aggregator finalize opt-out (Druid `shouldFinalize`): `false`
        /// keeps the intermediate sketch even under context finalize;
        /// absent / `true` follows the context default.
        #[serde(
            default,
            rename = "shouldFinalize",
            skip_serializing_if = "Option::is_none"
...
impl AggregatorSpec {
    ...
    pub fn create(&self) -> Box<dyn Aggregator> {
        match self {
            ...
            Self::HyperUnique { .. } => Box::new(sketch::HyperUniqueAggregator::new()),
...
impl PostAggregatorSpec {
    ...
    /// `HyperUniqueCardinality` is evaluatable since W-A (v1.5.0) — it
    /// computes the Druid-parity estimate of a decoded `hyperUnique`
    /// aggregation.
    ///
    /// # Errors
    ///
    /// Returns [`ferrodruid_common::DruidError::Query`] when a sub-tree
    /// uses an `Expression` whose expression string does not parse under
    /// the supported grammar subset.
    pub fn validate_supported(&self) -> ferrodruid_common::Result<()> {
        match self {
            ...
            // W-A (v1.5.0): hyperUniqueCardinality evaluates against a
            // decoded Druid hyperUnique partial (see `evaluate`).
            Self::HyperUniqueCardinality { .. } => Ok(()),
        }
    }
    ...
    pub fn evaluate(&self, agg_results: &HashMap<String, serde_json::Value>) -> Option<f64> {
        match self {
            Self::FieldAccess { field_name, .. } => {
                ...
                // A fieldAccess on a SKETCH aggregation inside an
                // arithmetic post-agg uses the sketch's estimate, matching
                // Druid (oracle W-A: `events_per_uu = events / uu` divides
                // by the finalized hyperUnique estimate).  The envelope's
                // `estimate` convenience field is recomputed by every
                // envelope constructor and JSON merge, so it always equals
                // the finalized value.  A mix-error envelope has no
                // estimate and stays None — the loud error is never
                // numerified.
                let obj = val.as_object()?;
                if obj.get("@sketch").and_then(serde_json::Value::as_str)
                    == Some(sketch::SKETCH_MIX_ERROR_TAG)
                {
                    return None;
                }
                obj.get("@sketch")?;
            ...
            Self::HllSketchEstimate { field, round, .. } => {
                let v = field.resolve_value(agg_results)?;
                // W-A (v1.5.0): SQL `APPROX_COUNT_DISTINCT` lowers to
                // HLLSketchBuild + this post-agg, and over a migrated
                // `COMPLEX<hyperUnique>` column the accumulator ADOPTS the
                // decoded-Druid feed (see `HllSketchAggregator`) — so the
                // partial here may be a decoded-hyperUnique envelope.  Its
                // finalized value MUST come from the Druid-parity
                // estimator (never re-estimated through the native FNV
                // HLL), with Java `Math.round` semantics under `round`.
                if let Some(bytes) = sketch_field_bytes(&v, sketch::DRUID_HYPER_UNIQUE_TAG) {
                    let sketch = ferrodruid_sketches::DruidHyperUnique::deserialize(&bytes).ok()?;
                    return Some(if *round == Some(true) {
                        #[allow(clippy::cast_precision_loss)]
                        let rounded = sketch.estimate_rounded() as f64;
                        rounded
                    } else {
                        sketch.estimate()
                    });
            ...
            Self::HyperUniqueCardinality { field_name, .. } => {
                // W-A (v1.5.0): the estimate of a decoded Druid
                // hyperUnique aggregation — identical to the finalized
                // aggregation value (oracle: `uu_card == uu` in every
                // captured query).  Accepts either the partial-state
                // envelope (pre-finalize result map) or an
                // already-finalized number (post-finalize map); a
                // mix-error envelope yields None (the loud error object
                // stays visible, never a fabricated number).
                let v = agg_results.get(field_name)?;
                if let Some(n) = v.as_f64() {
                    return Some(n);
                }
                let bytes = sketch_field_bytes(v, sketch::DRUID_HYPER_UNIQUE_TAG)?;
                ferrodruid_sketches::DruidHyperUnique::deserialize(&bytes)
                    .ok()
                    .map(|s| s.estimate())
            }
        }
...
mod tests {
    ...
    // --- W-B ANSI byte-identity: broker min/max merge (latch OFF) ---

    /// The lib-test binary never latches legacy mode, so this pins the
    /// ANSI (flag-off) behavior: the broker min/max merge must stay
    /// byte-identical to the historical pre-W-B `as_f64` comparison.
    /// Two adjacent integers above 2^53 collapse to the SAME f64, so
    /// the comparison ties and `dst` wins by shard order — the
    /// exact-integer comparator (which would pick the numerically
    /// smaller/larger side) is gated to the legacy latch.
    #[test]
    fn ansi_broker_minmax_merge_matches_historical_f64_comparison() {
        assert!(
            !ferrodruid_common::legacy_null_mode(),
            "the lib-test binary must stay in ANSI mode"
        );
        ...
        let lo = json!(P53);
        let hi = json!(P53 + 1);
        // min, dst=hi src=lo: both read 2^53.0 → `s < d` is FALSE → dst
        // (2^53+1) wins — the pre-W-B answer, NOT the exact minimum.
        assert_eq!(
            merge_json_by_spec(&min_spec, &hi, &lo),
            hi,
            "ANSI min merge must keep dst on an f64 tie (historical as_f64 behavior)"
        );
        assert_eq!(merge_json_by_spec(&min_spec, &lo, &hi), lo);
        // max, dst=lo src=hi: tie again → dst (2^53) wins.
        assert_eq!(
            merge_json_by_spec(&max_spec, &lo, &hi),
            lo,
            "ANSI max merge must keep dst on an f64 tie (historical as_f64 behavior)"
        );
        assert_eq!(merge_json_by_spec(&max_spec, &hi, &lo), hi);
    }
    ...
    /// W-A (v1.5.0): `hyperUniqueCardinality` is now implemented — it
    /// validates, and evaluates a decoded Druid hyperUnique partial to
    /// the SAME value the finalized aggregation reports (the oracle pins
    /// `uu_card == uu` in every captured query).  A mix-error envelope
    /// yields `None`, never a fabricated number.
    #[test]
    fn post_agg_hyper_unique_cardinality_validates_and_evaluates() {
        let spec = PostAggregatorSpec::HyperUniqueCardinality {
            name: "h".into(),
            field_name: "u".into(),
        };
        spec.validate_supported()
            .expect("hyperUniqueCardinality must validate since W-A");

        // Envelope feed: identical to the sketch's own estimator value.
        let sketch = ferrodruid_sketches::DruidHyperUnique::empty();
        let mut results = HashMap::new();
        results.insert(
            "u".to_string(),
            sketch::druid_hyper_unique_envelope(&sketch),
        );
        assert_eq!(spec.evaluate(&results), Some(sketch.estimate()));

        // Already-finalized number: passes through.
        results.insert("u".to_string(), serde_json::json!(12.035_294_185_441_22));
        assert_eq!(spec.evaluate(&results), Some(12.035_294_185_441_22));

        // Mix-error envelope: no fabricated number.
        results.insert(
            "u".to_string(),
            serde_json::json!({"@sketch": sketch::SKETCH_MIX_ERROR_TAG, "message": "boom"}),
        );
        assert_eq!(spec.evaluate(&results), None);
    }
    ...
    /// The genuinely mode-independent min/max merge behaviors.  The
    /// LONG comparison rule is LATCH-SPLIT and pinned elsewhere: exact
    /// i64 under the legacy latch
    /// (`tests/legacy_null_empty_aggs.rs::legacy_broker_minmax_merge_is_exact_beyond_2_pow_53`),
    /// historical `as_f64` with the flag off
    /// ([`ansi_broker_minmax_merge_matches_historical_f64_comparison`] —
    /// the prior "mode-independent" exact comparison changed ANSI
    /// answers for `>2^53` adjacents and broke W-B's byte-identity
    /// invariant).
    #[test]
    fn merge_json_minmax_doubles_and_null_partials_unchanged() {
        // Doubles keep the f64 comparison; a null (empty ANSI partial)
        // still adopts / is ignored exactly as before.
        let dmin = AggregatorSpec::DoubleMin {
            name: "d".into(),
            field_name: "v".into(),
        };
        assert_eq!(
            merge_json_by_spec(&dmin, &json!(2.5), &json!(1.5)),
            json!(1.5)
        );
        assert_eq!(
            merge_json_by_spec(&dmin, &serde_json::Value::Null, &json!(1.5)),
            json!(1.5)
        );
        assert_eq!(
            merge_json_by_spec(&dmin, &json!(1.5), &serde_json::Value::Null),
            json!(1.5)
        );
    }
