    ...
    match node {
        ...
        Node::Binary(op, lhs, rhs) => {
            ...
            Some(match op {
                ...
                BinOp::Sub => a - b,
                BinOp::Mul => a * b,
                // Ordinary IEEE division in EVERY null mode: Druid's
                // expression division has no divide-by-zero exception
                // (0/0 → NaN → None → null).  The legacy empty-set
                // AVG = 0 (oracle empty_avg_y.json) comes from AVG's OWN
                // lowering — an arithmetic `/` post-agg whose documented
                // divide-by-zero → 0 rule lives in `PostAggregatorSpec`
                // — never from a global exception here, which would
                // corrupt every legacy expression containing 0/0
                // (e.g. `ROUND(SUM(x)/COUNT(*), 2)` over an empty match
                // must stay null).
                BinOp::Div => a / b,
            })
        }
