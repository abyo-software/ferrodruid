...
macro_rules! define_minmax_aggregator {
    (
        ...
        $to_json:expr, $legacy_empty:expr
    ) => {
        #[doc = $doc]
        #[doc = ""]
        ...
        impl Aggregator for $name {
            ...
            fn get(&self) -> serde_json::Value {
                if !self.seen {
                    // W-B legacy null mode: an empty MIN/MAX leaks the fold
                    // sentinel, exactly as legacy Druid answers (oracle
                    // empty_min_y.json = i64::MAX / empty_max_y.json =
                    // i64::MIN; ext_empty_min_d.json = the JSON STRING
                    // "Infinity" — Druid's Double sentinel rendering).
                    // Either shape stays a merge IDENTITY: a numeric i64
                    // sentinel folds away by comparison and the Infinity
                    // strings extract no number (merge no-op).
