...
impl PendingBatch {
    ...
    /// `offset`. The span `next` advances to `offset + 1` (saturating:
    /// a protocol-valid `i64::MAX` offset must not wrap into a poisoned
    /// `[MAX, MIN)` sidecar span) so a subsequent crash + resume starts
    /// at the right place.
    ///
    /// **Discontinuity guard (offset-space reset — the sidecar-writer
    /// side of `StreamingBuffer::record_offset`'s R27 H1 guard):** Kafka
    /// delivers per-partition offsets in increasing order between seeks,
    /// so a delivered offset BELOW the span's `next` means the consume
    /// position jumped backward into a RE-NUMBERED offset space (a
    /// truncation's OFFSET_OUT_OF_RANGE → `auto.offset.reset` redelivery,
    /// or a delete+recreate). Min/max-merging across that jump would
    /// durably stamp fabricated `[low, old_next)` coverage into
    /// `offsets.json` — coverage over records never consumed in the new
    /// numbering, which a restart's [`EosSegmentWriter::resume_offsets`]
    /// (max `next`) would then skip forever (permanent silent loss). The
    /// span is RESET to the new delivery instead: the row itself is kept
    /// (real consumed data), only the offset CLAIM is withdrawn, which
    /// floors the resume (bounded re-consume, never a skip). A FORWARD
    /// gap (`offset >= next`) still merges: aborted-transaction records,
    /// control batches, and compacted offsets are legitimately never
    /// delivered and must be committed over.
    pub fn record(&mut self, partition: TopicPartition, offset: i64, row: serde_json::Value) {
        self.rows.push(row);
        let next = offset.saturating_add(1);
        if let Some(entry) = self.spans.get_mut(&partition) {
            if offset < entry.next {
                tracing::warn!(
                    ...
                    span_next = entry.next,
                    "backward offset delivery into the pending sidecar span — the \
                     offset space was re-numbered (truncation/recreation \
                     redelivery); RESETTING the span instead of merging, so the \
                     durable offsets.json sidecar cannot claim fabricated \
                     contiguous coverage across the discontinuity (a restart \
                     re-consumes the regrown records, never skips them)",
                );
                *entry = OffsetSpan::new(offset, next);
            } else {
                entry.next = next;
            }
        } else {
            self.spans.insert(partition, OffsetSpan::new(offset, next));
        }
    }
}
...
mod tests {
    ...
    /// ship2 follow-on (the H11 `record_offset` discontinuity guard,
    /// mirrored on the sidecar writer): after consuming offsets 10..=11,
    /// a truncation / delete+recreate redelivery jumps BACKWARD to 3 —
    /// the offset space was RE-NUMBERED. Min/max-merging would durably
    /// stamp `[3, 12)` into `offsets.json`, fabricating coverage over
    /// 4..10 that was never consumed in the new numbering; a restart's
    /// `resume_offsets()` (max `next`) would then skip those records
    /// forever. The span must RESET to the new delivery (floor, never a
    /// merged claim).
    #[test]
    fn backward_offset_jump_resets_span_never_fabricates_coverage() {
        let mut batch = PendingBatch::new();
        batch.record(tp(0), 10, make_row(1_700_000_000_000, "a"));
        batch.record(tp(0), 11, make_row(1_700_000_000_001, "b"));
        batch.record(tp(0), 3, make_row(1_700_000_000_002, "c"));
        // Every row is kept (real consumed data — only the offset CLAIM
        // is withdrawn)...
        assert_eq!(batch.len(), 3);
        // ...but the span restarts at the discontinuity.
        assert_eq!(
            batch.spans.get(&tp(0)),
            Some(&OffsetSpan::new(3, 4)),
            "a backward jump must RESET the span, not min/max-merge \
             fabricated coverage across the re-numbered space"
        );
        // A FORWARD gap still merges: aborted-transaction records /
        // control batches (and compaction) legitimately never deliver
        // their offsets, which must be committed over.
        batch.record(tp(0), 7, make_row(1_700_000_000_003, "d"));
        assert_eq!(batch.spans.get(&tp(0)), Some(&OffsetSpan::new(3, 8)));
    }

    /// A protocol-valid `i64::MAX` offset must not overflow `next`
    /// (debug panic / release wrap to `i64::MIN` would persist a
    /// poisoned `[MAX, MIN)` sidecar span).
    #[test]
    fn i64_max_offset_saturates_next() {
        let mut batch = PendingBatch::new();
        batch.record(tp(0), i64::MAX, make_row(1_700_000_000_000, "x"));
        assert_eq!(
            batch.spans.get(&tp(0)),
