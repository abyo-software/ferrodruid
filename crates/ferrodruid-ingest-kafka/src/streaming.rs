...
impl StreamingBuffer {
    ...
    /// stage 2). The span `next` advances to `offset + 1` (the value
    /// manual-committed to Kafka on a successful publish). Called once per
    /// Kafka MESSAGE — a multi-object record shares one offset — and even for
    /// a dead-lettered record, so the resume frontier advances past
    /// consumed-but-unstored messages exactly as Druid commits past
    /// unparseable ones.
    ///
    /// **Discontinuity guard (offset-space reset, buffered-span side of R27
    /// H1):** a delivered offset BELOW the span's `next` is never in-order
    /// same-numbering delivery — the seek gate drops records until an
    /// authoritative seek lands, and Kafka delivers per-partition offsets in
    /// increasing order between seeks — so it means the consume position
    /// jumped backward into a RE-NUMBERED offset space (a truncation's
    /// OFFSET_OUT_OF_RANGE → `auto.offset.reset=earliest` redelivery, or a
    /// watermark-clamped restore seek). Min/max-merging across that jump
    /// would fabricate `[low, old_next)` coverage over regrown records that
    /// were never consumed; once durably stamped + committed, a restart
    /// resumes PAST them (permanent silent loss). The span is instead RESET
    /// to the new delivery — the durable stamp then claims only offsets
    /// actually consumed contiguously in the live numbering, and any
    /// withdrawn old-numbering claim floors the resume (bounded re-consume,
    /// never a skip). A FORWARD gap (`offset > next`) still merges: under
    /// `read_committed`, aborted-transaction records and control batches are
    /// legitimately never delivered (likewise compaction), and their offsets
    /// must be committed over exactly as Druid commits past them.
    pub fn record_offset(&mut self, partition: i32, offset: i64) {
        ...
        if let Some(span) = self.offset_spans.get_mut(&partition) {
            if offset < span.next {
                tracing::warn!(
                    partition,
                    offset,
                    ...
                    span_next = span.next,
                    "backward offset delivery into the buffered span — the \
                     offset space was re-numbered (truncation/recreation \
                     redelivery); RESETTING the span instead of merging, so \
                     the durable kafkaOffsets stamp cannot fabricate \
                     contiguous coverage across the discontinuity (a restart \
                     re-consumes the regrown records, never skips them)",
                );
                *span = OffsetSpan::new(offset, next);
            } else {
                span.next = next;
            }
        } else {
            self.offset_spans
                .insert(partition, OffsetSpan::new(offset, next));
        }
    }

    /// Purge `partition`'s buffered offset span because an authoritative seek
    /// was CLAMPED into the retained watermark window and RE-BASED the
    /// contiguous-commit tracker ([`ContiguousFrontier::reset_base`], via
    /// [`rebase_after_clamped_seek`]) — the buffered span predates the
    /// discontinuity, so its offsets belong to a numbering the consume
    /// position can never reconnect to. Returns whether a span was present.
    ///
    /// Without the purge there is a stamp window: between the re-base and the
    /// first post-seek delivery (which would reset the span via
    /// [`record_offset`](Self::record_offset)'s discontinuity guard), a
    /// flush-timer roll would durably stamp the DEAD numbering's span into
    /// `payload.kafkaOffsets` — and once the log regrows past it, a restart's
    /// resume walk trusts that in-range coverage and seeks past regrown
    /// records that were never consumed (permanent silent loss; the
    /// watermark clamp cannot catch a regrown in-range target).
    ///
    /// The buffered ROWS are kept: they are real consumed data whose records
    /// no longer exist in Kafka (truncated/retention-deleted), so publishing
    /// them preserves data — only the offset CLAIM is withdrawn, which floors
    /// the resume (bounded re-consume of obtainable records, never a skip).
    pub fn purge_partition_span(&mut self, partition: i32) -> bool {
        self.offset_spans.remove(&partition).is_some()
    }
...
/// Re-base `partition`'s contiguous-commit tracker to a CLAMPED/reset-floor
/// seek position AND purge its stale buffered offset span — the two halves of
/// one invariant (offset-space reset, buffered-span side of R27 H1): a
/// re-base means the consume position was moved somewhere the pre-clamp
/// numbering can never reconnect to ([`ContiguousFrontier::reset_base`],
/// Codex R6 H2), so a buffered span recorded under that numbering must not
/// survive to be durably stamped as `kafkaOffsets` coverage. Left in place,
/// a flush-timer roll landing BEFORE the first post-seek delivery would stamp
/// the dead span; after the log regrows past it, a restart's resume walk
/// trusts the in-range coverage and seeks past regrown records that were
/// never consumed (permanent silent loss — exactly the skip the R27/R9
/// low-watermark flooring exists to prevent). Purging withdraws only the
/// CLAIM (the rows are still published; see
/// [`StreamingBuffer::purge_partition_span`]) so the worst case is a bounded
/// re-consume, never a skip.
///
/// Pure (no Kafka I/O — the actual seek is `try_activate_pending`) so the
/// rebase-purges-span coupling is unit-testable without a broker.
pub fn rebase_after_clamped_seek(
    ...
    if buffer.purge_partition_span(partition) {
        tracing::warn!(
            partition,
            target,
            "clamped seek re-based the commit tracker: PURGED the partition's \
             buffered offset span (recorded under the pre-clamp numbering) so \
             the next roll cannot durably stamp dead-numbering kafkaOffsets \
             coverage — the buffered rows are still published, only the \
             offset claim is withdrawn (bounded re-consume, never a skip)",
        );
    }
}
...
fn fetch_committed_offset(
    ...
    match consumer.committed_offsets(tpl, RESUME_SEEK_TIMEOUT) {
        Ok(c) => match c.find_partition(topic, partition) {
            Some(elem) => {
                // ship2: an `Ok` list can still carry a PER-PARTITION
                // OffsetFetch error (coordinator churn, a topic
                // delete/recreate propagation window) — the element then
                // holds `Offset::Invalid` WITH its error set. That is an
                // UNKNOWN committed offset, not a known-absent one:
                // conflating them resolves the target from `min_start`
                // alone, and a real commit pinned below `min_start` (the
                // C1 never-durable-gap case) would be skipped forever.
                // Keep the partition gated instead (retried).
                if let Err(e) = elem.error() {
                    tracing::warn!(
                        topic, partition, error = %e,
                        "resume seek: committed-offset fetch returned a PER-PARTITION \
                         error — the committed offset is UNKNOWN (not known-absent), \
                         so the partition stays gated until a clean fetch (retried)",
                    );
                    return None;
                }
                match elem.offset() {
                    Offset::Offset(v) => Some(Some(v)),
                    // No per-partition error + no concrete offset: the
                    // group genuinely has nothing committed.
                    _ => Some(None),
                }
            }
            None => {
                // Defensive: the response omitted the requested partition
                // entirely — treat as fetch-failed (unknown), never as
                // known-absent.
                tracing::warn!(
                    topic,
                    partition,
                    "resume seek: committed-offset response omitted the requested \
                     partition — treated as an UNKNOWN committed offset, the \
                     partition stays gated (retried)",
                );
                None
            }
        },
...
///
/// `buffer` is the loop's shared [`StreamingBuffer`]: the same re-base also
/// PURGES the partition's buffered offset span
/// ([`rebase_after_clamped_seek`]) — recorded under the pre-clamp numbering,
/// it would otherwise be durably stamped as fabricated `kafkaOffsets`
/// coverage by a flush-timer roll landing before the first post-seek
/// delivery (restart skip of regrown records = permanent silent loss).
///
...
///
/// **Blocking discipline (ship2):** the pass issues bounded-blocking
/// librdkafka calls (committed-offset fetch, watermark fetch, seek,
/// pause, resume — each capped at [`RESUME_SEEK_TIMEOUT`]) per pending
/// partition, so with an unreachable broker one pass can block for
/// multiples of the timeout. The pass therefore runs inside
/// [`run_blocking_section`]: on a multi-thread runtime the worker is
/// handed to the blocking pool (`block_in_place`), so OTHER tasks on the
/// runtime — sibling supervisors, servers — are never starved by it.
/// Honest residual: the streaming loop's OWN select arms (shutdown /
/// flush timer / drift tripwire) still wait for the pass to return —
/// they live on the same task, whose `&mut` loop state the pass borrows;
/// making them concurrent would require restructuring the loop's
...
fn try_activate_pending(
    ...
    run_blocking_section(|| {
        try_activate_pending_blocking(consumer, topic, gate, frontier, contiguous, buffer);
    });
}

/// Run a bounded-blocking librdkafka section without monopolizing an
/// async worker thread (ship2): on a MULTI-thread tokio runtime the
/// closure runs under [`tokio::task::block_in_place`], which migrates
/// the worker's other tasks before blocking (mirroring the
/// `spawn_blocking` discipline of [`live_identity_drifted`] /
/// `resolve_cluster_id`, but borrow-compatible with the loop's `&mut`
/// state). On a current-thread runtime — the unit-test flavor, where
/// `block_in_place` would panic — or outside a runtime, it runs inline.
#[cfg(feature = "kafka-io")]
...
/// The blocking body of [`try_activate_pending`] — see its doc.
#[cfg(feature = "kafka-io")]
fn try_activate_pending_blocking(
    consumer: &KafkaStreamConsumer,
    topic: &str,
    gate: &mut ResumeSeekGate,
    frontier: &BTreeMap<i32, PartitionResume>,
    ...
    for partition in gate.pending() {
        ...
        match consumer.seek(
            ...
            Ok(()) => {
                ...
                {
                    // Re-base + purge the partition's buffered offset span
                    // as ONE invariant: the span was recorded under the
                    // pre-clamp numbering and must not be durably stamped as
                    // fabricated kafkaOffsets coverage (see
                    // `rebase_after_clamped_seek`).
                    rebase_after_clamped_seek(contiguous, buffer, partition, target);
                    gate.set_target(partition, target);
                }
...
fn reseek_active_after_publish_failure(
    ...
    try_activate_pending(consumer, topic, gate, resume_frontier, contiguous, buffer);
}
    ...
    match publish_rolled(sink, topic, buffer).await {
        ...
        Err(_) => {
            *mid_stream_flush_failed = true;
            reseek_active_after_publish_failure(
                consumer,
                topic,
                gate,
                resume_frontier,
...
mod tests {
    ...
    #[test]
    fn backward_jump_into_renumbered_space_does_not_fabricate_coverage() {
        // Offset-space discontinuity (the R27 H1 truncation threat model,
        // buffered-span side): records [500, 600) were consumed in the OLD
        // offset numbering; an unclean-leader-election truncation re-numbered
        // the log (new high=520, low=0) and delivery restarts from 0 — either
        // via a clamped restore seek that re-based the commit tracker (path
        // a), or via the ACTIVE partition's OFFSET_OUT_OF_RANGE →
        // auto.offset.reset=earliest redelivery (path b, no rebase). The
        // buffered span must NOT min/max-merge into [0, 600): that fabricates
        // kafkaOffsets coverage over the gap; once durably stamped +
        // committed, a restart resumes at 600 and permanently SKIPS regrown
        // new-numbering records [100, 600) that were never consumed (silent
        // loss, defeating the R27/R9 low-watermark flooring).
        let mut buf = StreamingBuffer::from_config(&config(1_000));
        let mut frontier = ContiguousFrontier::new();
        // Old numbering: buffered records spanning [500, 600).
        buf.record_offset(0, 500);
        frontier.note_consumed(0, 500);
        assert!(!buf.push_payload(&payload(1, "old-a")).expect("push"));
        buf.record_offset(0, 599);
        assert!(!buf.push_payload(&payload(2, "old-b")).expect("push"));
        // Truncation: the clamped seek re-bases the commit tracker to low=0
        // (R6 H2) and the broker delivers the regrown log from 0.
        frontier.reset_base(0, 0);
        for o in 0..100 {
            buf.record_offset(0, o);
        }
        assert!(!buf.push_payload(&payload(3, "new")).expect("push"));
        // The next roll durably stamps kafkaOffsets from the buffered span.
        let (_seg, stamped) = buf.roll_with_offsets().expect("roll").expect("segment");
        let span = stamped.get(&0).expect("partition 0 span");
        assert!(
            span.next <= 100,
            "durably-stamped kafkaOffsets claim [{}, {}): fabricated contiguous \
             coverage across the truncation discontinuity — a restart would \
             resume past regrown records [100, 600) that were never consumed \
             (permanent silent loss)",
            span.start,
            span.next,
        );
        // And the commit derived from the stamp floors at the actually
        // consumed new-numbering prefix — a restart re-consumes the regrown
        // records (bounded at-least-once dup), never skips them.
        let committable = frontier.record_durable(&stamped);
        assert_eq!(
            committable.get(&0),
            Some(&100),
            "commit must cover exactly the contiguously-consumed new-numbering \
             prefix [0, 100), never the fabricated [0, 600)"
        );
    }

    #[test]
    fn clamped_seek_rebase_purges_stale_buffered_span() {
        // The STAMP WINDOW of the same scenario: a rebalance restore-seek
        // targets 600, the watermark clamp lands at low=0 and re-bases the
        // commit tracker (R6 H2) — but NO post-seek record arrives before the
        // next flush-timer roll. Without the coupled purge, that roll durably
        // stamps the DEAD numbering's [500, 600) into kafkaOffsets; once the
        // log regrows past 600, a restart's resume walk trusts the in-range
        // coverage and seeks to 600, permanently skipping regrown records
        // that were never consumed.
        let mut buf = StreamingBuffer::from_config(&config(1_000));
        let mut frontier = ContiguousFrontier::new();
        // Old numbering on partition 0: buffered rows spanning [500, 600).
        buf.record_offset(0, 500);
        frontier.note_consumed(0, 500);
        assert!(!buf.push_payload(&payload(1, "old-a")).expect("push"));
        buf.record_offset(0, 599);
        assert!(!buf.push_payload(&payload(2, "old-b")).expect("push"));
        // An unaffected partition's span must survive the rebase of 0.
        buf.record_offset(1, 7);
        assert!(!buf.push_payload(&payload(3, "p1")).expect("push"));
        // The clamped restore seek: re-base to low=0 + purge as ONE invariant.
        rebase_after_clamped_seek(&mut frontier, &mut buf, 0, 0);
        assert_eq!(
            frontier.durable_next(0),
            Some(0),
            "the commit base is re-based to the clamped seek position (R6 H2)"
        );
        // A flush-timer roll BEFORE any post-seek delivery: the rows are
        // still published (data preserved), but the segment must NOT claim
        // the dead numbering's offsets for partition 0.
        let (seg, stamped) = buf.roll_with_offsets().expect("roll").expect("segment");
        assert_eq!(seg.num_rows, 3, "the buffered rows are still published");
        assert!(
            !stamped.contains_key(&0),
            "the purged partition stamps NO kafkaOffsets: {:?} — a dead-numbering \
             claim would let a restart skip regrown records once the log regrows \
             past it",
            stamped.get(&0),
        );
        assert_eq!(
            stamped.get(&1),
            Some(&OffsetSpan::new(7, 8)),
            "an unaffected partition's span survives the rebase of another"
        );
        // With nothing stamped for partition 0, nothing commits for it — a
        // restart re-consumes the regrown log from the flooring seek (bounded
        // at-least-once dup, never a skip).
        assert!(
            !frontier.record_durable(&stamped).contains_key(&0),
            "no fabricated commit for the re-based partition"
        );
    }
