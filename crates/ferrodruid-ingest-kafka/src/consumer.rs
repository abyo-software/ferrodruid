...
use ferrodruid_ingest_batch::{
    BatchIngester, DimensionSchema, TsFormat, extract_row_timestamp_millis_fmt,
};
...
pub struct KafkaConsumerConfig {
    ...
    /// Declared `timestampSpec.format` (auto/iso/millis), threaded down to
    /// row extraction — the formats genuinely differ (a declared `iso` reads
    /// `"2023"` as the YEAR 2023, not 2023 ms — Fable audit).
    pub timestamp_format: TsFormat,
    /// Typed dimension schemas to extract. Carrying the full
    /// [`DimensionSchema`] (not just names) preserves declared `long` /
    /// `double` / `float` dimension types through to the segment, instead
    /// of silently materialising every dimension as a string.
    pub dim_schemas: Vec<DimensionSchema>,
    /// Metric aggregator specs (`dataSchema.metricsSpec`), passed through
    /// to the [`BatchIngester`] so `longSum` / `doubleSum` / `count` /…
    /// metrics are materialised on the produced segments rather than
    /// silently dropped.
    pub metrics_specs: Vec<serde_json::Value>,
...
impl KafkaConsumerTask {
    ...
    ///
    /// The record's timestamp is validated here under the DECLARED
    /// `timestampSpec.format` — the same grammar [`flush`](Self::flush)
    /// stores it under (Codex R21: validation and storage must agree, the
    /// discipline the streaming path already follows) — so an invalid
    /// record is dead-lettered individually instead of poisoning the
    /// whole batch's flush.
    pub fn process_record(
        ...
        // Validate the timestamp under the declared format (Codex R21) —
        // the SAME extraction `flush` performs via the ingester, so every
        // buffered row survives the segment build.
        extract_row_timestamp_millis_fmt(
            &row,
    ...
    pub fn flush(&mut self) -> Result<SegmentFlushResult, ConsumerError> {
        ...
        // Use BatchIngester to produce segment (typed dimensions preserved,
        // declared timestamp format threaded — Codex R21: a declared `iso`
        // reads "2023" as the YEAR 2023, not 2023 ms).
        let ingester = BatchIngester::with_schemas(
            self.config.data_source.clone(),
            self.config.timestamp_column.clone(),
            self.config.dim_schemas.clone(),
            self.config.metrics_specs.clone(),
        )
        .with_timestamp_format(self.config.timestamp_format);
...
mod tests {
    ...
    fn make_config(max_rows: usize) -> KafkaConsumerConfig {
        KafkaConsumerConfig {
            ...
            dim_schemas: vec![DimensionSchema::string("dim")],
            metrics_specs: vec![],
    ...
    // -----------------------------------------------------------------
    // Codex R21 finding 1 — the legacy `KafkaConsumerTask` path (used by
    // the real-broker `kafka_e2e` integration test) must honour the
    // declared `timestampSpec.format`, with the SAME grammar governing
    // push-time validation and flush-time storage (the discipline the
    // wired supervisor path — `StreamingBuffer` — already follows).
    // -----------------------------------------------------------------

    /// Codex R21 finding 1 (storage side): with a declared `format:"iso"`,
    /// `"2023"` is the ISO YEAR 2023 in Druid. The un-threaded flush built
    /// the segment under the `auto` grammar and stored 2023 ms — a wrong
    /// instant class. The declared format must govern the segment build.
    #[test]
    fn consumer_task_flush_honors_declared_iso_format() {
        let (_tx, rx) = mpsc::channel(1);
        let mut config = make_config(100);
        config.timestamp_format = TsFormat::Iso;
        let mut task = KafkaConsumerTask::new(config, rx);

        let payload = serde_json::json!({ "__time": "2023", "dim": "a" });
        let record = ConsumedRecord {
            key: None,
            payload: serde_json::to_vec(&payload).expect("serialize"),
            topic: "test-topic".to_string(),
        ...
        let result = task.flush().expect("flush");
        assert_eq!(
            result.interval_start_millis, 1_672_531_200_000,
            "iso '2023' must be stored as the YEAR 2023, not 2023 ms"
        );
    }

    /// Codex R21 finding 1 (validation side): validation and storage must
    /// share one grammar (streaming.rs discipline) — a declared `millis`
    /// format must dead-letter ISO text at `process_record` time instead
    /// of buffering a record the flush can only fail wholesale.
    #[test]
    fn consumer_task_validates_timestamp_under_declared_format() {
        let (_tx, rx) = mpsc::channel(1);
        let mut config = make_config(100);
        config.timestamp_format = TsFormat::Millis;
        let mut task = KafkaConsumerTask::new(config, rx);

        let payload = serde_json::json!({ "__time": "2023-11-14", "dim": "a" });
        let record = ConsumedRecord {
            key: None,
            payload: serde_json::to_vec(&payload).expect("serialize"),
            topic: "test-topic".to_string(),
        ...
        let err = task
            ...
            .expect_err("millis must reject ISO text at push time");
        assert!(matches!(err, ConsumerError::ParseError(_)));
