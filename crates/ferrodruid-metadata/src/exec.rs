...
//! `fetch_optional` / `execute_returning_id` / `begin`).  The 3-arm
...
impl Backend {
    ...
    /// Execute an `INSERT` template and return the database-generated
    /// integer id of the inserted row.
    ///
    /// Per backend: SQLite `last_insert_rowid()`; PostgreSQL appends
    /// ` RETURNING id` and fetches it; MySQL `LAST_INSERT_ID()`.
    pub(crate) async fn execute_returning_id(
        ...
        let (sql, ordered) = prepare(self.dialect(), template, args)?;
        match self {
            Backend::Sqlite(pool) => Ok(build_sqlite(&sql, &ordered)
                .execute(pool)
                .await?
                .last_insert_rowid()),
            Backend::Postgres(pool) => {
                let sql = format!("{sql} RETURNING id");
                let row = build_pg(&sql, &ordered).fetch_one(pool).await?;
