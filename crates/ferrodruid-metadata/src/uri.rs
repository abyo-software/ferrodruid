...
///
/// Any OTHER `scheme://` prefix is rejected loudly instead of being
/// treated as a relative SQLite filename (the historical name-trap:
/// `--metadata-uri postgres://…` used to create a SQLite file literally
/// named `postgres://…`).
...
/// # Errors
///
/// Returns [`DruidError::Metadata`] for an empty input or an
/// unsupported `scheme://` prefix.
pub fn parse_metadata_uri(uri: &str) -> Result<MetadataUri> {
    ...
    if !is_scheme {
        // "://" appeared but the prefix is not a URI scheme (e.g. a very
        // odd filename) — treat the whole string as a path.
        return Ok(MetadataUri::SqlitePath(uri.to_string()));
    }
