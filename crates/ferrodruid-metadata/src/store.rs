...
impl MetadataStore {
    ...
    ///
    /// Use `":memory:"` for an ephemeral in-memory database (useful for
    /// tests): it is routed to [`new_in_memory`](Self::new_in_memory).
    /// Routing matters — to SQLite, every NEW connection to the
    /// `:memory:` filename opens its own PRIVATE empty database, so a
    /// multi-connection pool here would scatter the schema across
    /// per-connection databases and fail nondeterministically with
    /// "no such table" (re-audit 2026-07-19; pre-fix, only
    /// [`connect`](Self::connect) took the safe route).
    pub async fn new_sqlite(path: &str) -> Result<Self> {
        if path == ":memory:" {
            return Self::new_in_memory().await;
        }
        // An empty filename is SQLite's OTHER private-per-connection
        // form (a throwaway temp-file database per connection) — the
        // same scattered-database trap as `:memory:`, and never a real
        // metadata store. Fail loudly.
        if path.is_empty() {
            return Err(DruidError::Metadata(
                "sqlite path is empty (use ':memory:' for an in-memory store or a real \
                 file path)"
                    .into(),
            ));
        }
    ...
    pub async fn new_in_memory() -> Result<Self> {
        ...
        // The ENTIRE database lives inside the pool's single connection:
        // - exactly 1 connection, always held (`min_connections(1)`) —
        //   a second connection to `:memory:` would be its OWN private
        //   empty database;
        // - `idle_timeout(None)` + `max_lifetime(None)`: the sqlx 0.8
        //   DEFAULTS are Some(10 min)/Some(30 min), under which the
        //   pool's background reaper closes the sole idle connection and
        //   the replacement opens a brand-new EMPTY `:memory:` database —
        //   silently wiping the whole metadata store mid-run (re-audit
        //   2026-07-19 Medium; see the
        //   `reaped_memory_connection_wipes_the_database` witness test).
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .min_connections(1)
            .idle_timeout(None)
    ...
    /// as ONE transaction, so both the legacy log view and the lifecycle
    /// query view stay consistent — a crash between two autocommit writes
    /// would durably leave a `druid_tasklogs` row with no
    /// `druid_task_status` row: visible in the legacy log view, absent
    /// from `get_task`/`get_active_tasks` (re-audit 2026-07-19).
    pub async fn insert_task(&self, task: &TaskRow) -> Result<()> {
        ...
        let mut txn = self
            ...
            .begin()
            .await
            .map_err(|e| DruidError::Metadata(format!("begin insert-task txn: {e}")))?;
        ...
        txn.commit()
            .await
            .map_err(|e| DruidError::Metadata(format!("commit insert-task txn: {e}")))?;
        Ok(())
    }
    ...
    ///
    /// Both table updates (`druid_task_status` + the `druid_tasklogs`
    /// payload mirror) commit as ONE transaction — a crash between two
    /// autocommit writes would durably persist the new status while the
    /// legacy log view keeps the stale payload (re-audit 2026-07-19).
    pub async fn update_task_status(&self, task: &TaskRow) -> Result<()> {
        ...
        let mut txn = self
            ...
            .begin()
            .await
            .map_err(|e| DruidError::Metadata(format!("begin update-task txn: {e}")))?;
        ...
        txn.commit()
            .await
            .map_err(|e| DruidError::Metadata(format!("commit update-task txn: {e}")))?;
        Ok(())
    }
    ...
    /// `LAST_INSERT_ID()` on MySQL) as ONE transaction.  A crash between
    /// two autocommit writes would durably leave a canonical row with no
    /// detail row — invisible to every JOIN-based reader and undeletable
    /// via [`delete_lock`](Self::delete_lock), since its id was never
    /// returned: a permanent orphan (re-audit 2026-07-19).
    pub async fn insert_task_lock(&self, lock: &TaskLockRow) -> Result<String> {
        ...
        let mut txn = self
            ...
            .begin()
            .await
            .map_err(|e| DruidError::Metadata(format!("begin insert-lock txn: {e}")))?;
        ...
        txn.commit()
            .await
            .map_err(|e| DruidError::Metadata(format!("commit insert-lock txn: {e}")))?;
        Ok(rowid.to_string())
    }
    ...
    /// Permanently delete a lock (both canonical and detail rows), as
    /// ONE transaction — a crash between two autocommit deletes would
    /// leave the canonical row detail-less: invisible to the JOIN-based
    /// readers yet still present, the same orphan species as a torn
    /// [`insert_task_lock`](Self::insert_task_lock) (re-audit
    /// 2026-07-19).
    pub async fn delete_lock(&self, id: &str) -> Result<()> {
        ...
        let mut txn = self
            ...
            .begin()
            .await
            .map_err(|e| DruidError::Metadata(format!("begin delete-lock txn: {e}")))?;
        txn.execute(T_DELETE_LOCK_DETAIL, &[Arg::I64(rowid)])
            .await
            .map_err(|e| DruidError::Metadata(format!("delete lock detail: {e}")))?;
        txn.execute(T_DELETE_LOCK, &[Arg::I64(rowid)])
            .await
            .map_err(|e| DruidError::Metadata(format!("delete lock: {e}")))?;
        txn.commit()
            .await
            .map_err(|e| DruidError::Metadata(format!("commit delete-lock txn: {e}")))?;
        Ok(())
    }
...
mod tests {
    ...
    /// Re-audit 2026-07-19: the two `insert_task_lock` writes (canonical
    /// `druid_tasklocks` row + `druid_task_lock_detail` row) are ONE
    /// database transaction.  Pre-fix they were separate autocommit
    /// statements; a failure after the canonical insert left a durable
    /// lock row with NO detail row — invisible to every JOIN-based reader
    /// (`get_locks_for_datasource` / `get_locks_for_task`) and
    /// undeletable via `delete_lock` (its id was never returned): a
    /// permanent orphan.  The detail insert here fails naturally (a
    /// pre-seeded primary-key collision on the id the canonical insert
    /// allocates), and the canonical row must roll back with it.
    async fn case_lock_insert_failure_leaves_no_orphan_row(store: &MetadataStore) {
        let first_id: i64 = store
            .insert_task_lock(&make_lock("task_first", "wiki", "EXCLUSIVE", 50))
            .await
            .expect("first lock insert")
            .parse()
            .expect("numeric lock id");

        // Occupy the detail-table primary key that the NEXT canonical
        // insert will allocate (auto-generated ids are consecutive in a
        // serial workload on all three backends), so the SECOND statement
        // of the pair fails AFTER the first has executed.
        store
            .backend
            .execute(
                T_INSERT_LOCK_DETAIL,
                &[
                    Arg::I64(first_id + 1),
                    Arg::Str("wiki".to_string()),
                    Arg::Str("2024-01-01T00:00:00Z".to_string()),
                    Arg::Str("2024-02-01T00:00:00Z".to_string()),
                    Arg::Str("EXCLUSIVE".to_string()),
                    Arg::I64(50),
                    Arg::Bool(false),
                ],
            )
            .await
            .expect("seed colliding detail row");

        let err = store
            .insert_task_lock(&make_lock("task_orphan", "wiki", "EXCLUSIVE", 50))
            .await
            .expect_err("colliding detail insert must fail the whole lock insert");
        assert!(
            format!("{err}").contains("lock"),
            "error should carry lock-insert context: {err}"
        );

        // Atomic: the canonical row written before the failing detail
        // insert must NOT survive as an invisible orphan.
        let rows = store
            .backend
            .fetch_all(
                "SELECT id FROM druid_tasklocks WHERE task_id = {1}",
                &[Arg::Str("task_orphan".to_string())],
            )
            .await
            .expect("scan tasklocks");
        assert!(
            rows.is_empty(),
            "aborted lock insert must leave no canonical druid_tasklocks row behind"
        );

        // The store remains fully usable: retrying after the collision is
        // cleared succeeds end-to-end.
        store
            .backend
            .execute(T_DELETE_LOCK_DETAIL, &[Arg::I64(first_id + 1)])
            .await
            .expect("clear seeded collision");
        let retry_id = store
            .insert_task_lock(&make_lock("task_retry", "wiki", "SHARED", 5))
            .await
            .expect("retry lock insert");
        let by_task = store.get_locks_for_task("task_retry").await.expect("read");
        assert_eq!(by_task.len(), 1);
        assert_eq!(by_task[0].id, retry_id);
    }
    ...
    #[tokio::test]
    async fn lock_insert_failure_leaves_no_orphan_row() {
        case_lock_insert_failure_leaves_no_orphan_row(&setup().await).await;
    }
    ...
    async fn run_full_suite(uri: &str) {
        ...
        case_lock_insert_failure_leaves_no_orphan_row(&fresh_env_store(uri).await).await;
    ...
    /// Re-audit 2026-07-19 (Medium): an in-memory SQLite database lives
    /// INSIDE its single pooled connection, so the pool must never reap
    /// that connection.  sqlx 0.8 `PoolOptions` DEFAULTS keep
    /// `idle_timeout = Some(10 min)` and `max_lifetime = Some(30 min)` —
    /// with the defaults, the pool's background reaper closes the sole
    /// idle connection and the replacement connection to `:memory:`
    /// opens a brand-new EMPTY database, silently destroying every
    /// published segment/rule/supervisor/task row mid-run.  Both
    /// in-memory constructor paths must pin one never-reaped connection.
    /// (`reaped_memory_connection_wipes_the_database` below is the
    /// time-compressed witness of the loss mechanism.)
    #[tokio::test]
    async fn in_memory_pool_never_reaps_its_connection() {
        let direct = MetadataStore::new_in_memory()
            .await
            .expect("in-memory store");
        let path_form = MetadataStore::new_sqlite(":memory:")
            .await
            .expect("path-form in-memory store");
        for (name, store) in [
            ("new_in_memory", direct),
            ("new_sqlite(\":memory:\")", path_form),
        ] {
            let Backend::Sqlite(pool) = &store.backend else {
                panic!("{name}: in-memory store must be SQLite-backed");
            };
            let opts = pool.options();
            assert_eq!(
                opts.get_max_connections(),
                1,
                "{name}: >1 connection would give each its OWN private empty :memory: database"
            );
            assert_eq!(
                opts.get_min_connections(),
                1,
                "{name}: the pool must always hold its one connection"
            );
            assert!(
                opts.get_idle_timeout().is_none(),
                "{name}: idle_timeout must be disabled — reaping the sole :memory: \
                 connection destroys the database (sqlx default is Some(10 min))"
            );
            assert!(
                opts.get_max_lifetime().is_none(),
                "{name}: max_lifetime must be disabled — recycling the sole :memory: \
                 connection destroys the database (sqlx default is Some(30 min))"
            );
        }
    }

    /// Time-compressed witness of the data-loss mechanism guarded by
    /// `in_memory_pool_never_reaps_its_connection`: a single-connection
    /// `:memory:` pool whose idle connection CAN be reaped (any `Some`
    /// idle_timeout — compressed here from the sqlx default 10 minutes
    /// to 100ms so it is observable in test time) loses the ENTIRE
    /// database once the reaper closes that connection — the next
    /// acquire opens a fresh, empty `:memory:` database.
    #[tokio::test]
    async fn reaped_memory_connection_wipes_the_database() {
        let opts = SqliteConnectOptions::new()
            .filename(":memory:")
            .create_if_missing(true);
        let pool = SqlitePoolOptions::new()
        ...
        // Poll SLOWER than the idle_timeout so the connection is idle
        // long enough for the reaper to close it between probes.
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(20);
        loop {
            tokio::time::sleep(std::time::Duration::from_millis(300)).await;
            match sqlx::query("SELECT count(*) FROM t").fetch_one(&pool).await {
                Err(e) => {
                    // The database was wiped with the reaped connection —
                    // this is the loss `new_in_memory` must never allow.
                    assert!(
                        format!("{e}").contains("no such table"),
                        "unexpected error observing the wipe: {e}"
                    );
                    break;
                }
                Ok(_) => {
                    assert!(
                        ...
                        "reaper never closed the idle :memory: connection — sqlx pool \
                         semantics changed; re-evaluate new_in_memory's reap guards"
                    );
                }
            }
        }
