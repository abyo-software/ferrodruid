...
//!
//! plus one DECODED-foreign sketch type (W-A, v1.5.0):
//!
//! - [`DruidHyperUnique`] — a merge-only decoded Apache Druid `hyperUnique`
//!   HyperLogLog register state (NOT interchangeable with [`HllSketch`],
//!   whose FNV hash space is unrelated — see the `druid_hll` module docs)
...
pub use druid_hll::DruidHyperUnique;
...
pub enum SketchError {
    ...
    /// Cross-hash-space Theta operation.  A sketch decoded from an Apache
    /// DataSketches (Druid) compact image carries MurmurHash3-space hashes
    /// and is **union-only** with other Druid-origin sketches; mixing it
    /// with FerroDruid's native FNV-space hashes would silently corrupt
    /// the estimate (see [`crate::ThetaSketch::from_druid_compact`]).
    /// Also raised when two non-empty Druid-origin sketches carry
    /// DIFFERENT DataSketches seed hashes — sketches built with different
    /// update seeds are themselves different hash spaces, and combining
    /// them would double-count (see
    /// [`crate::ThetaSketch::druid_seed_hash`]).
    #[error("theta hash-space mismatch: {0}")]
    HashSpaceMismatch(String),
}
