...
pub struct KinesisResumeFrontier {
    ...
    /// A durable checkpoint row for this stream was PRESENT but
    /// unreadable: its `kinesisSequences` value was not a non-empty
    /// object (corrupt — the shards it named cannot be enumerated), OR a
    /// checkpoint-bearing row's `kind`/`stream` attribution field was
    /// corrupt (present but wrong-typed, or missing beside a matching
    /// companion — the row can be neither attributed nor positively
    /// excluded, ship2). Rather than let those shards fall back to the
    /// spec `LATEST` default (and skip records after the last durable
    /// segment), the WHOLE frontier is condemned to `TRIM_HORIZON` — the
    /// same all-shards floor as `stream_recreated` (Codex R3 C1).
...
///   but a CHECKPOINT-BEARING row whose `kind`/`stream` attribution is
///   corrupt (wrong-typed, or absent beside a matching companion) rather
///   than positively foreign condemns the whole frontier
///   ([`checkpoint_corrupt`](KinesisResumeFrontier::checkpoint_corrupt),
///   ship2 — never a silent skip that would fall back to the spec
///   default);
...
pub fn fold_resume_frontier(
    ...
    for row in rows {
        let p = row.payload;
        let kind_field = p.get("kind");
        let stream_field = p.get(STREAM_PAYLOAD_KEY);
        let kind_matches = kind_field.and_then(Value::as_str) == Some(KINESIS_STREAMING_KIND);
        let stream_matches = stream_field.and_then(Value::as_str) == Some(stream);
        if !(kind_matches && stream_matches) {
            // ship2 (R3-C1 sibling): a CHECKPOINT-BEARING row (it carries
            // the `kinesisSequences` key) whose `kind`/`stream`
            // attribution is corrupt — a field PRESENT but wrong-typed,
            // or the companion of a positively-matching field ABSENT —
            // can be neither attributed to this stream nor positively
            // excluded from it. Silently skipping it would let the shards
            // it names fall back to the spec default (a LATEST default
            // then skips every consumed-but-unpublished record above the
            // durable coverage: silent loss). Condemn the whole frontier
            // to TrimHorizon instead, exactly like a corrupt
            // `kinesisSequences` value. A WELL-TYPED foreign `kind` or
            // `stream` is a positive exclusion and never condemns.
            let kind_is_other = kind_field
                .and_then(Value::as_str)
                .is_some_and(|k| k != KINESIS_STREAMING_KIND);
            let stream_is_other = stream_field
                .and_then(Value::as_str)
                .is_some_and(|s| s != stream);
            let kind_corrupt = kind_field.is_some_and(|v| !v.is_string());
            let stream_corrupt = stream_field.is_some_and(|v| !v.is_string());
            let attribution_corrupt = kind_corrupt
                || stream_corrupt
                || (kind_matches && stream_field.is_none())
                || (stream_matches && kind_field.is_none());
            if p.get(SEQUENCES_PAYLOAD_KEY).is_some()
                && !kind_is_other
                && !stream_is_other
                && attribution_corrupt
            {
    ...
    if checkpoint_corrupt {
        tracing::warn!(
            stream,
            "kinesis durable checkpoint row was unreadable (a non-object or \
             EMPTY kinesisSequences value, or a checkpoint-bearing row whose \
             kind/stream attribution field was corrupt): its shards cannot be \
             enumerated/attributed, so EVERY shard resumes from TRIM_HORIZON \
             rather than risk the spec LATEST default skipping records after \
             the last durable segment (bounded at-least-once duplication, \
             never loss)"
        );
    }
...
mod tests {
    ...
    /// ship2 Low (R3-C1 sibling): a durable CHECKPOINT-bearing row whose
    /// `stream` (or `kind`) attribution field is PRESENT but wrong-typed
    /// cannot be attributed to — or positively excluded from — this
    /// stream. Silently skipping it leaves the frontier EMPTY and every
    /// shard on the spec default; with the Druid-default LATEST that
    /// seeks past all consumed-but-unpublished records above the durable
    /// coverage (silent loss). It must condemn the whole frontier to
    /// TrimHorizon, exactly like a corrupt `kinesisSequences` value.
    #[test]
    fn fold_corrupt_attribution_on_checkpoint_row_floors_all_shards() {
        let mut seg = ShardSequences::new();
        seg.insert("shard-a".to_owned(), span("10", "20", None));

        // stream corrupted to a JSON number, kind + sequences intact.
        let mut p = payload("st", Some(111), sequences_to_payload(&seg));
        p["stream"] = serde_json::json!(123);
        let rows = [FrontierRowEvidence {
            payload: &p,
            loaded: true,
        }];
        ...
        assert!(
            ...
            "a corrupt `stream` attribution on a checkpoint row must be flagged"
        );
        assert_eq!(
            f.start_position_for("shard-a", StartPosition::Latest),
            StartPosition::TrimHorizon,
            "the un-attributable checkpoint must floor every shard"
        );

        // kind corrupted to a JSON number, stream + sequences intact.
        let mut p2 = payload("st", Some(111), sequences_to_payload(&seg));
        p2["kind"] = serde_json::json!(42);
        ...
        assert!(
            f2.checkpoint_corrupt,
            "a corrupt `kind` attribution on a checkpoint row must be flagged"
        );

        // POSITIVE exclusions never condemn: a WELL-TYPED foreign stream…
        let p3 = payload("other-stream", Some(111), sequences_to_payload(&seg));
        let rows3 = [FrontierRowEvidence {
            payload: &p3,
            loaded: true,
        }];
        assert!(!fold_resume_frontier(&rows3, "st", Some(111)).checkpoint_corrupt);
        // …a WELL-TYPED foreign pipeline kind…
        let mut p4 = payload("st", Some(111), sequences_to_payload(&seg));
        p4["kind"] = serde_json::json!("kafka-streaming");
        let rows4 = [FrontierRowEvidence {
            payload: &p4,
            loaded: true,
        }];
        assert!(!fold_resume_frontier(&rows4, "st", Some(111)).checkpoint_corrupt);
        // …and a corrupt-typed field WITHOUT the checkpoint key (no
        // `kinesisSequences` → the row could never contribute anyway).
        let p5 = serde_json::json!({"kind": KINESIS_STREAMING_KIND, "stream": 123});
        let rows5 = [FrontierRowEvidence {
            payload: &p5,
            loaded: true,
        }];
        assert!(!fold_resume_frontier(&rows5, "st", Some(111)).checkpoint_corrupt);
    }
