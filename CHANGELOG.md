...
## [1.5.1] — 2026-07-21 (read older-Druid segments)

A migration-reach patch: FerroDruid now reads the **v1 numeric column serdes**
(`long` / `double` / `float`) that **older Apache Druid (version 27 and earlier)**
writes for `__time` and numeric metric columns. v1.5.0 read the v2 serdes
(`longV2` / `doubleV2` / `floatV2`, written by Druid 31/35); before this patch,
`ferrodruid-migrate attach` / `import-druid-metadata` failed on segments from a
genuinely-old Druid deployment — the exact clusters a "legacy migration" targets.

### Fixed / Added

- **Legacy v1 numeric column read**: the v9 segment reader decodes the v1
  `long`/`double`/`float` column serdes. Reverse-engineered clean-room from real
  Apache Druid 27.0.0 segment bytes (verified byte-exact against Druid's own
  `dump-segment` — including `longEncoding` auto-TABLE and auto-DELTA, DOUBLE
  bit-exact, and FLOAT). A v1 numeric part is the same compressed-columnar payload
  as v2 without v2's length prefix; unobserved sub-variants (unknown version byte,
  trailing bytes, unknown compression id, non-little-endian) fail loud. End-to-end:
  a real Druid 27 deployment on S3 deep storage now attaches (previously 0
  readable), and its `hyperUnique` estimates + legacy-null answers match Druid.
  Druid 31/35 v2 reading is unchanged.

## [1.5.0] — 2026-07-21 (legacy-Druid migration compatibility)

A migration-compatibility release aimed at **older Apache Druid deployments**:
FerroDruid now reads the classic `hyperUnique` distinct-count sketch that
pre-DataSketches dashboards rely on, offers an opt-in **legacy null mode**
matching Druid ≤27 semantics, and can attach segments directly from **S3 deep
storage**. Every claim below is measured against a real Apache Druid used as a
black-box oracle (Druid 27.0.0 for legacy-null, Druid 31.0.2 for hyperUnique),
clean-room (no Druid source referenced); scope limits are in
`docs/known-limitations.md`. Additive and off by default — the ANSI-null path,
the single-binary default, and every prior query result are byte-for-byte
unchanged.

### Added — cross-engine sketch read

- **`hyperUnique` complex-column decode**: FerroDruid reads Apache Druid's
  built-in classic-HLL `hyperUnique` distinct-count metric column (present in
  every Druid version), so `hyperUnique` native queries and SQL
  `APPROX_COUNT_DISTINCT` over such a column return **estimates that match real
  Druid**. Verified against Druid 31.0.2: the native double estimate is
  bit-exact (`to_bits`-equal) across single-row, rollup-folded, and
  multi-segment/multi-shard segmentations; the rounded SQL estimate is exact.
  The on-disk sparse and dense register formats were reverse-engineered from
  Druid-written bytes (including the buffer-relative sparse register positions).
  A decoded Druid `hyperUnique` sketch is a distinct, merge-only type — it is
  never mixed with FerroDruid's own sketches or with Apache DataSketches
  `HLLSketch`; a genuine cross-type mix fails loud rather than returning a wrong
  estimate. (`HLLSketch` / `quantilesDoublesSketch` decode remain follow-ons.)

### Added — legacy null semantics

- **Opt-in `useDefaultValueForNull` legacy null mode** (default off = ANSI):
  a global, startup-latched flag (`--use-default-value-for-null`, the
  `FERRODRUID_USE_DEFAULT_VALUE_FOR_NULL` env var, or the config key) switches to
  Apache Druid ≤27 default null semantics — string `null` ≡ `''`, numeric `null`
  → `0` — across filters, aggregations (including empty-aggregate defaults),
  groupBy/topN keys, COUNT DISTINCT, SQL rendering, and ingestion (which then
  writes `''`/`0` with no null markers, as legacy Druid does). Verified with a
  legacy-vs-ANSI diff battery against real Apache Druid 27.0.0 (legacy) and
  31.0.2 (ANSI). Applies identically on the single-binary and role-split
  (broker/historical) execution paths. With the flag off, behaviour is
  byte-identical to the ANSI default.

### Added — migration from S3

- **Attach + import from S3 deep storage**: `ferrodruid-migrate attach
  --deep-storage s3://…` and `import-druid-metadata` (for an `s3_zip` loadSpec)
  now stream segment artifacts directly from an Apache Druid deployment on S3,
  wiring the existing S3 deep-storage client — no local "pull first" step.
  Untrusted keys/objects flow through the existing containment, zip, and hash
  checks; the listing is memory-bounded; a malformed `s3://` URL or key fails
  loud rather than falling back to a local path or a different object. Verified
  end-to-end against MinIO (free) and a real AWS S3 tree. Target deep storage
  stays local; HDFS/GCS/Azure sources still require a local pull first.

### Known residual limitations (documented)

- Legacy null mode covers the oracle-pinned surfaces above; string
  bound/like/regex, first/last, variance, windows, joins, and some multi-value
  corners keep ANSI behaviour under the flag, and legacy non-strict booleans are
  out of scope. hyperUnique's high-cardinality raw-HLL estimator branch is
  reproduced from public HLL literature (all captured fixtures fall in the
  linear-counting regime). S3 attach's path-derived identity assumes Druid's
  conventional key layout; the metadata-DB importer is layout-immune. Full list
  and measurement conditions in `docs/known-limitations.md`.

## [1.4.0] — 2026-07-18 (Druid migration & compatibility)

A compatibility-focused release: FerroDruid can now **attach an existing
Apache Druid deployment's on-disk segments and metadata**, ingest from
more sources, store more Druid column types faithfully, and — for the
first time — **write segments Apache Druid itself can read back**. Every
claim below is measured against a real Apache Druid 31.0.2 (and 35.0.1
where noted) or a real backend (PostgreSQL 16 / MySQL 8 / LocalStack
Kinesis); the honest scope limits of each are in
`docs/known-limitations.md`. These are additive — the single-binary
default, the durable streaming path (1.3.0), and every prior query result
are byte-for-byte unchanged.

### Added — migration & attach

- **Attach existing Druid v9 segments** (`ferrodruid-migrate attach`): an
  on-disk Apache Druid **v9** deep-storage tree is imported so its
  segments become query-visible (identity from `descriptor.json` or the
  `<ds>/<interval>/<version>/<partition>` path, gated on the real v9
  reader, transcoded with a re-computed SHA-256, marked `used=true`).
  Verified end-to-end: attach → bootstrap reload → the imported rows
  answer queries with the expected aggregates. **Scope:** single-binary +
  local deep storage on both sides; offline (rows land on the next
  startup); S3/remote source not wired.
- **Import an existing Druid metadata database**
  (`ferrodruid-migrate import-druid-metadata`): reads a Druid
  `druid_segments` table from **PostgreSQL, MySQL, or SQLite**, taking
  identity from the row columns cross-checked against the payload.
  **Scope:** local loadSpec, `used=true` rows, single-binary, offline.
- **Lenient attach** (`--allow-unreadable-columns`, default OFF): a
  segment carrying a column FerroDruid cannot yet decode (e.g. a
  `hyperUnique`/`HLLSketch`/`quantilesDoublesSketch` complex column, or a
  Concise bitmap) is by default a loud whole-segment skip; the opt-in flag
  instead **drops just the undecodable column(s)** and attaches the rest,
  loudly manifested, the pruned segment re-written and strict-re-opened
  before commit. Unblocks migrating a rollup datasource's
  dimensions/`__time`/counts/sums.
- **FerroDruid → Druid segment write-back** (reverse migration, TG-1): a
  native writer emits real Apache Druid on-disk **v9** — verified two ways
  against real Druid 31.0.2, byte-identical to Druid-written fixtures and
  read back by Druid's own `dump-segment` with a per-column SHA-256 deep
  match. **Scope:** LONG metrics/`__time` + single-value STRING dimensions
  (milestone 1); DOUBLE/FLOAT/nullable/multi-value/compressed writing is a
  documented follow-on and fails loud until then.

### Added — ingestion & sources

- **Kinesis streaming ingestion** (`kinesis-io` build): a Kinesis
  supervisor now runs a real durable consumer with an anchor-rooted
  string-sequence frontier (at-least-once, never silent loss), verified
  consume/durability/resume against LocalStack Kinesis. (This was
  previously de-claimed; it is now real. No live AWS Kinesis E2E yet.)
- **Native batch ingestion executes** (`index` / `index_parallel`, local
  and inline sources, JSON/CSV/TSV): tasks that were previously accepted
  and left PENDING now run synchronously to completion, persist durably,
  and reload — with a new `/task/{id}/status` endpoint.
- **`timestampSpec.format` on native batch**: an explicit timestamp format
  (e.g. a Joda pattern, `millis`, `iso`) is now honoured on ingest instead
  of being silently mis-parsed; a content-bearing `transformSpec` is
  rejected rather than ignored. Residual formats (posix/nano/missingValue,
  Joda edge cases) fail closed / dead-letter, documented in FG-15.

### Added — metadata backends & column types

- **Real PostgreSQL and MySQL metadata stores**: the metadata store,
  previously SQLite-only, now runs against production **PostgreSQL 16** and
  **MySQL 8** backends (per-backend dialect + execution layer), verified by
  a container end-to-end suite.
- **`thetaSketch` column decode**: FerroDruid reads a real Druid
  `thetaSketch` complex column and `APPROX_COUNT_DISTINCT_DS_THETA` matches
  Druid 31, with DataSketches seed-hash identity and no cross-space
  mixing. (FerroDruid's own sketches remain FNV-based and byte-incompatible
  with DataSketches; `hyperUnique`/`HLLSketch`/`quantiles` decode is a
  follow-on.)
- **Nullable 64-bit integer columns**: a LONG dimension can now hold NULL
  alongside values above 2^53 exactly (`>9,007,199,254,740,992`) — the
  previous degrade-to-DOUBLE path that silently lost integer precision is
  gone. Verified vs real Druid 31.
- **Multi-value string dimensions**: a dimension holding a JSON array
  ingests as a real per-row multi-value column and explodes in
  groupBy/topN, with any-element filters — Druid-faithful (verified vs real
  Druid 31, including the filtered-groupBy explosion behaviour). Reading an
  upstream Druid multi-value segment is a documented fail-loud follow-on.
- **Broader v9 reader coverage**: SQL-compatible NULL numeric columns
  (Druid 28+ default null handling) and `longEncoding: auto` (TABLE + DELTA)
  are now read, so a modern **default** Druid segment's numeric columns are
  fully readable. Verified vs real Druid 31.

### Fixed

- Vectorized executor no longer trusts a mutable `time_sorted` flag on a
  segment whose rows are not actually time-ascending (the flag is
  reconciled/re-derived; a debug assertion guards the invariant).

### Known residual limitations (documented)

- The migration paths are single-binary + local-deep-storage + offline;
  the distributed historical still speaks a JSONL format, not v9; S3/remote
  sources for attach are not wired. `thetaSketch` is the only decodable
  sketch; `hyperUnique`/`HLLSketch`/`quantiles` are loud skips (or a
  `--allow-unreadable-columns` column drop). The v9 **writer** is LONG +
  single-value STRING only. Full list and measurement conditions in
  `docs/known-limitations.md` (FG-4/10/11/12/13/14/15).
