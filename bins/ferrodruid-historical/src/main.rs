...
struct Cli {
    ...
    /// everything this process serves — including the per-segment
    /// native-query executor. Latched process-globally at startup
    /// (before serving); cannot change without a restart. Must match
    /// the value every other role in the cluster was started with.
    /// Default off (modern SQL-compatible ANSI nulls).
    #[arg(
        long,
...
fn main() -> ExitCode {
    ...
    // anything can read it, so the per-segment executor
    // (`ferrodruid-rpc::native_query`) answers with the same null
...
mod tests {
    ...
    /// H4: the historical accepts the same legacy-null-mode surface as
    /// the single binary (`--use-default-value-for-null` +
    /// `FERRODRUID_USE_DEFAULT_VALUE_FOR_NULL`), so the role-split path
    /// can latch the flag before serving.
    #[test]
    fn cli_parses_use_default_value_for_null() {
        let cli = Cli::try_parse_from(["ferrodruid-historical", "--use-default-value-for-null"])
            .expect("flag parses");
        assert!(cli.use_default_value_for_null);

        let cli = Cli::try_parse_from(["ferrodruid-historical"]).expect("default parses");
        assert!(!cli.use_default_value_for_null, "default is ANSI (off)");
    }
