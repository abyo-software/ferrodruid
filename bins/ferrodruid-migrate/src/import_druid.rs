...
//! 2. the payload `loadSpec` is resolved to a LOCAL artifact: the
//!    `local.path` as-is when it exists on this host, else
//!    longest-suffix probing under the `--deep-storage` remap base
//!    (the "Druid deep storage copied/mounted here" case).  Every
//!    non-`local` loadSpec (s3_zip / hdfs / google / azure) is a
//!    per-segment LOUD skip — the run continues;
...
//! * **`local` loadSpec only**: s3_zip / hdfs / google / azure rows are
//!   loud per-segment skips — pull those blobs to a local directory and
//!   re-run with `--deep-storage` (remote fetch is a compat-7b
//!   follow-up).
...
enum ImportStatus {
    ...
    SkippedExists,
    /// Non-`local` loadSpec — out of compat-7 scope, loudly skipped.
    SkippedLoadSpec {
...
async fn import_one(
    ...
    // 2. loadSpec resolution — compat-7 scope is `local` only; every
    //    other type is a LOUD skip and the run continues.
    let LoadSpec::Local { path } = &payload.load_spec else {
        record.status = ImportStatus::SkippedLoadSpec {
            reason: format!(
                "loadSpec type `{}` is not supported in compat-7 (scope: local deep \
                 storage) — pull the segment blob to a local directory and re-run \
                 with --deep-storage",
                load_spec_type_name(&payload.load_spec)
            ),
        };
        return record;
    };
    let resolved = match resolve_local_artifact(path, params.deep_storage.as_deref()) {
        Ok(r) => r,
        Err(reason) => {
