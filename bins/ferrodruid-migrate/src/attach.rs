...
//! segments from a Druid **local deep-storage** directory into a
//! FerroDruid single-binary deployment (compat-2).
//!
...
//! * **Local (filesystem) deep storage only**, on both sides.  For
//!   S3/HDFS/GCS Druid deep storage, pull the segments to a local dir
//!   first.
...
/// completed.
pub(crate) async fn run(params: AttachParams) -> Result<(), Box<dyn std::error::Error>> {
    ...
    //
    // A DRY RUN must write NOTHING to the local filesystem — not even
    // the empty SQLite file `create_if_missing` would leave behind — so
    // for SQLite it only opens a store that already exists (for
    // collision reporting). For the network backends a dry run still
    // connects and runs the idempotent `CREATE TABLE IF NOT EXISTS`
    // bootstrap (documented residual: empty metadata TABLES may be
    // created on the target server; no rows are ever written). A real
    // run creates + initializes the store like `serve` does.
        ...
        match parse_metadata_uri(&params.metadata_uri)? {
            MetadataUri::SqlitePath(path) => {
                if Path::new(&path).is_file() {
                    let s = MetadataStore::new_sqlite(&path).await?;
                    s.initialize().await?;
                    Some(s)
                } else {
                    None
...
fn print_report(input: &ReportInput<'_>) {
    ...
    println!(
        ...
        assess::sanitize(&params.deep_storage.display().to_string())
    );
    println!(
        "ferro deep-storage:      {}",
