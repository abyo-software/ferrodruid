...
#[derive(Subcommand)]
enum Command {
    /// Dry-run assessment of a Druid local deep-storage directory:
    /// scan for segment artifacts and report, per segment, whether it
    /// is readable by FerroDruid's v9 reader — attachable with
    /// `ferrodruid-migrate attach`. Judgment tool only: nothing is
    /// migrated or modified. Local deep storage only (for S3/HDFS/GCS,
    /// pull the segments to a local dir first).
    Assess {
        /// Path to the Druid local deep-storage root
        /// (`druid.storage.storageDirectory`). Both `index.zip`
        /// artifacts and already-unzipped smoosh directories are
        /// recognized.
        #[arg(long)]
    ...
    /// segment. Scope: single-binary deployments and local deep
    /// storage on both sides (the distributed role binaries do not
    /// serve attached v9 blobs); segments the v9 reader cannot open
    /// (e.g. unsupported encodings) are skipped loudly — run `assess`
    /// first for a readability map. Run offline: stop the instance (or
    /// attach before first start); concurrent writers to the metadata
    /// store are unsupported.
    Attach {
        /// Path to the Druid local deep-storage root
        /// (`druid.storage.storageDirectory`) to import from. Both
        /// `index.zip` artifacts and already-unzipped smoosh
        ...
        /// reader cannot decode (e.g. a complex/sketch column such as
        /// thetaSketch or hyperUnique, which FerroDruid cannot read
        /// yet), drop JUST that column and attach the rest instead of
        /// failing the whole segment. Every dropped column is listed
        /// loudly in the report and recorded in the metadata row's
    ...
    /// are imported, and only `local` loadSpecs are in scope: each
    /// row's payload identity (dataSource/interval/version/
    /// shardSpec.partitionNum) is synthesized, its local.path resolved
    /// (optionally remapped under --deep-storage), and the segment then
    /// runs the same per-segment crash-safe blob-first import as
    /// `attach` (blob upload, then metadata row). s3_zip/hdfs/google/
    /// azure loadSpecs are loudly skipped — pull those blobs to a local
    /// dir and re-run with --deep-storage. Derby sources are
    /// unsupported (no Rust driver): externalize the Druid metadata to
    /// PostgreSQL/MySQL first, or use `attach` on the deep-storage
    /// directory. Rules/supervisors are counted in the report but NOT
    /// imported. Nothing is loaded eagerly: segments become
    /// query-visible after the next `ferrodruid serve` startup
    ...
    ImportDruidMetadata {
        ...
        /// (e.g. thetaSketch/hyperUnique complex columns) instead of
        /// failing the whole segment; dropped names are reported loudly
        /// and recorded in payload.droppedUnreadableColumns, and the
