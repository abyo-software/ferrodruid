...
/// An hdfs row is a LOUD per-segment skip (with the remediation hint)
/// while the local neighbour still imports — the run never aborts.
/// (`s3_zip` rows are no longer skips: since W-C they are FETCHED —
/// see `tests/s3_source.rs`.)
#[tokio::test]
async fn non_local_loadspec_is_loud_skip_while_local_imports() {
    ...
    let zip = make_druid_blob(blobs.path(), "wiki", WIKI_VERSION, "0");
    let hdfs_payload = json!({
        "dataSource": "clicks",
        "interval": WIKI_INTERVAL,
        "version": "v1",
        "loadSpec": {"type": "hdfs", "path": "/druid/segments/clicks/0/index.zip"},
        "dimensions": "region",
        "metrics": "value",
        "shardSpec": {"type": "numbered", "partitionNum": 0, "partitions": 1},
        "binaryVersion": 9,
        "size": 1024,
        "identifier": "clicks_hdfs",
    })
    .to_string();
    let src_db = make_source_db(
        holder.path(),
        &[
            ...
            ("clicks_hdfs", "clicks", 1, hdfs_payload),
        ],
    )
    .await;
    ...
    let stdout = stdout_of(&out);
    assert!(
        stdout.contains("SKIP-LOADSPEC") && stdout.contains("hdfs"),
        "the hdfs row must be a loud skip naming the type: {stdout}"
    );
    assert!(
