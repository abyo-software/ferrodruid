...
/// Identity fields parsed from a descriptor are length-bounded so a
/// hostile descriptor cannot bloat the retained report by megabytes
/// per segment.  Codex R4 finding.
#[test]
fn descriptor_identity_fields_are_length_bounded() {
    let root = tempfile::tempdir().expect("tempdir");
    ...
    let v = parse_json(&out);
    let seg = &v["segments"][0];
    let ds = seg["data_source"].as_str().expect("data_source");
    assert!(
        ds.len() <= 1100,
        "identity field must be truncated, got {} chars",
        ds.len()
    );
    assert!(ds.starts_with("dddd"), "prefix preserved: {ds}");
}
...
/// ("readable by FerroDruid's v9 reader — attachable when the attach
/// feature ships") and must NOT imply the attach feature already
/// exists.
#[test]
fn human_output_wording() {
    ...
    assert!(
        stdout.contains("attachable when the attach feature ships"),
        "stdout: {stdout}"
    );
    assert!(
...
/// An oversized `descriptor.json` is ignored (identity falls back to
/// the path) instead of being slurped into memory.  Codex R1 finding
/// (unbounded descriptor read).
#[test]
fn oversized_descriptor_json_ignored() {
    let root = tempfile::tempdir().expect("tempdir");
    make_zip_layout(root.path(), "wiki", "iv", "v1", "0");
    let part_dir = root.path().join("wiki").join("iv").join("v1").join("0");
    // > 4 MiB of valid JSON.
    ...
    let v = parse_json(&out);
    let seg = &v["segments"][0];
    assert_eq!(seg["identity_source"], "path", "json: {v}");
    assert_eq!(seg["data_source"], "wiki");
}
