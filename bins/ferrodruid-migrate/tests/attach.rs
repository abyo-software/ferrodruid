...
/// A dry run against an EXISTING but empty (0-byte) SQLite target must
/// leave it 0 bytes with NO FerroDruid schema — it must never run
/// `initialize()` DDL against the target (re-audit Medium: the
/// read-only-probe discipline `import-druid-metadata --dry-run` already
/// uses applies to `attach --dry-run` too; the absent-file case is
/// covered by `dry_run_writes_nothing` above).
#[tokio::test]
async fn dry_run_does_not_initialize_an_existing_sqlite_target() {
    let druid_root = tempfile::tempdir().expect("druid root");
    let holder = tempfile::tempdir().expect("holder");
    let ferro_base = holder.path().join("deep-storage");
    let db_path = holder.path().join("empty-target.db");
    std::fs::File::create(&db_path).expect("create empty target file");
    make_wiki_raw_layout(druid_root.path());
    ...
    let stdout = stdout_of(&out);
    assert!(
        stdout.contains("WOULD-ATTACH") && stdout.contains(WIKI_ID),
        "the dry run still reports the artifact: {stdout}"
    );

    assert_eq!(
        std::fs::metadata(&db_path).expect("stat").len(),
        0,
        "--dry-run must not grow an existing target from 0 bytes (no initialize() DDL)"
    );
    let store = MetadataStore::open_sqlite_read_only(db_path.to_str().expect("utf8"))
        .await
        .expect("open the target read-only");
    assert!(
        !store.is_initialized().await.expect("probe target schema"),
        "--dry-run must not create the FerroDruid schema in the target"
    );
}

/// A dry run pointed (e.g. via a mistyped `--metadata-uri`) at a
/// FOREIGN SQLite database — some other application's file — must leave
/// it byte-identical: no FerroDruid CREATE TABLEs silently added inside
/// it, and no `-wal`/`-shm`/`-journal` siblings left behind.
#[tokio::test]
async fn dry_run_leaves_a_foreign_sqlite_target_byte_identical() {
    let druid_root = tempfile::tempdir().expect("druid root");
    let holder = tempfile::tempdir().expect("holder");
    let ferro_base = holder.path().join("deep-storage");
    let foreign_db = holder.path().join("other-app.db");
    make_wiki_raw_layout(druid_root.path());
    {
        ...
        .await
        .expect("create foreign db");
        sqlx::query("CREATE TABLE other_app (id INTEGER PRIMARY KEY, note TEXT)")
            .execute(&pool)
            .await
            .expect("create foreign table");
        pool.close().await;
    }
    let before = std::fs::read(&foreign_db).expect("read foreign db");
    ...
    let out = run_attach(&[
        ...
        ferro_base.to_str().expect("utf8"),
        "--metadata-uri",
        foreign_db.to_str().expect("utf8"),
        "--dry-run",
    ]);
    let stdout = stdout_of(&out);
    assert!(
        stdout.contains("WOULD-ATTACH"),
        "the dry run still reports the artifact: {stdout}"
    );

    assert_eq!(
        before,
        std::fs::read(&foreign_db).expect("re-read foreign db"),
        "--dry-run must leave a foreign SQLite target byte-identical (no DDL)"
    );
    for suffix in ["-wal", "-shm", "-journal"] {
        assert!(
            !holder.path().join(format!("other-app.db{suffix}")).exists(),
            "--dry-run must not leave a {suffix} journal next to the target"
        );
    }
}

/// A dry run against an ALREADY-initialized, populated store still
/// consults it (READ-ONLY) for accurate skip reporting — the existing
/// segment reports SKIP-EXISTS, not WOULD-ATTACH — and leaves the store
/// file byte-identical.
#[tokio::test]
async fn dry_run_against_initialized_store_reports_existing_skip_and_writes_nothing() {
    let druid_root = tempfile::tempdir().expect("druid root");
    let ferro_base = tempfile::tempdir().expect("ferro deep-storage");
    let meta_dir = tempfile::tempdir().expect("metadata dir");
    let db_path = meta_dir.path().join("ferrodruid.db");
    ...
    // A REAL attach first, to populate the target.
    let stdout = stdout_of(&run_attach(&base_args));
    assert!(stdout.contains("ATTACHED"), "first attach: {stdout}");
    let bytes_after_real = std::fs::read(&db_path).expect("read populated target");

    let mut dry_args = base_args.to_vec();
    dry_args.push("--dry-run");
    let stdout = stdout_of(&run_attach(&dry_args));
    assert!(
        stdout.contains("SKIP-EXISTS") && stdout.contains(WIKI_ID),
        "the already-attached segment is reported as a skip: {stdout}"
    );
    assert!(
        !stdout.contains("WOULD-ATTACH"),
        "an existing segment is not a would-attach: {stdout}"
    );
    assert_eq!(
        bytes_after_real,
        std::fs::read(&db_path).expect("re-read target"),
