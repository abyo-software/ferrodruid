...
async fn run_single_binary(
    ...
    let deep_storage_dir = data_dir.join("deep-storage");
    let metadata_dir = data_dir.join("metadata");
    tokio::fs::create_dir_all(&segments_dir).await?;
    tokio::fs::create_dir_all(&deep_storage_dir).await?;
    tokio::fs::create_dir_all(&metadata_dir).await?;

    // 1. Metadata store (SQLite).
    let db_path = metadata_dir.join("ferrodruid.db");
    let metadata = MetadataStore::new_sqlite(&db_path.to_string_lossy()).await?;
    metadata.initialize().await?;
    let metadata = Arc::new(metadata);
    tracing::info!("metadata store initialized");

    // 2. Deep storage (local filesystem).
    let _deep_storage = Arc::new(LocalDeepStorage::new(deep_storage_dir));
    ...
    let historical = Arc::new(Historical::with_strict_null_generation(
        segments_dir,
        1_073_741_824,
        strict_null_generation,
    )); // 1 GiB cache
    let overlord = Arc::new(Overlord::with_executor(
        Arc::clone(&metadata),
