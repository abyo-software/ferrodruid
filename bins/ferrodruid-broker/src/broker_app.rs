...
use ferrodruid_rpc::native_query::{
    Aggregation, NativeQuery, NativeQueryResult, TimeseriesBucket, legacy_fill_empty_timeseries,
    merge_group_by, merge_scan, merge_timeseries, merge_top_n,
};
use ferrodruid_rpc::sql_bridge::{
    BridgedQuery, TranslateError, default_schema_for_sql, sql_response_rows, translate_sql,
};
use ferrodruid_rpc::{
...
// The SQL response formatter (`sql_response_rows`) moved into
// `ferrodruid_rpc::sql_bridge` (W-B H1) so the role-split broker and the
// single-binary REST path share the SAME legacy null→`""` VARCHAR
// rendering rule (`ferrodruid_sql::legacy_string_cell`) instead of two
// diverging copies.
...
fn merge_fragments(query: &NativeQuery, fragments: Vec<SegmentQueryResponse>) -> NativeQueryResult {
    match query {
        NativeQuery::Timeseries(spec) => {
            ...
            // W-B H2: the legacy empty-RESULT bucket is synthesized HERE,
            // after the scatter merge — never per segment — anchored to
            // the query interval start (single-binary parity).  ANSI and
            // non-empty merges pass through unchanged.
            let merged = merge_timeseries(parts, aggs);
            NativeQueryResult::Timeseries(legacy_fill_empty_timeseries(merged, spec))
        }
