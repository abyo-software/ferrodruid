...
mod tests {
    ...
    /// H4: the broker accepts the same legacy-null-mode surface as the
    /// single binary (`--use-default-value-for-null` +
    ...
    fn cli_parses_use_default_value_for_null() {
        let cli = Cli::try_parse_from(["ferrodruid-broker", "--use-default-value-for-null"])
            .expect("flag parses");
        assert!(cli.use_default_value_for_null);

        let cli = Cli::try_parse_from(["ferrodruid-broker"]).expect("default parses");
